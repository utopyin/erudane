...
describe("DenoHttpServer", () => {
  ...
  it.effect("exports a weak ETag generator", () =>
    Effect.gen(function* () {
      ...
      const etag = yield* generator.fromFileWeb(
  ...
  it.effect("mountApp", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        yield* HttpClient.get("/child/1").pipe(Effect.flatMap((_) => _.text)),
        "/1",
      );
      assert.strictEqual(yield* HttpClient.get("/child").pipe(Effect.flatMap((_) => _.text)), "/");
      assert.strictEqual(
        yield* HttpClient.get("/child?foo=bar").pipe(Effect.flatMap((_) => _.text)),
        "?foo=bar",
      );
      assert.strictEqual(yield* HttpClient.get("/child/").pipe(Effect.flatMap((_) => _.text)), "/");
      assert.strictEqual(
        yield* HttpClient.get("/child1/", { urlParams: { foo: "bar" } }).pipe(
          Effect.map((_) => _.status),
        ),
        404,
      );
    }).pipe(Effect.provide(DenoHttpServer.layerTest)),
  ...
  it.effect("schemaBodyFormJson", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        (yield* HttpClient.post("/upload", { body: HttpBody.formData(formData) })).status,
        204,
      );
    }).pipe(Effect.provide(DenoHttpServer.layerTest)),
  );
    ...
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.post("/upload", {
        body: HttpBody.urlParams(
          UrlParams.fromInput({ json: JSON.stringify({ test: "content" }) }),
        ),
      });
      assert.strictEqual(response.status, 204);
    }).pipe(Effect.provide(DenoHttpServer.layerTest)),
  ...
  it.effect("html", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        yield* HttpClient.get("/home").pipe(Effect.flatMap((_) => _.text)),
        "<html />",
      );
      assert.strictEqual(
        yield* HttpClient.get("/about").pipe(Effect.flatMap((_) => _.text)),
        "<html><body /></html>",
      );
      assert.strictEqual(
  ...
  it.live("disposes after a client aborts a handler awaiting an upstream request", () =>
    Effect.gen(function* () {
      ...
      const upstream = yield* Effect.acquireRelease(
          ...
          const server = Deno.serve(
            ...
            (request) => {
              ...
              return new Promise<Response>((resolve) => {
                ...
                controller.signal.addEventListener("abort", () => resolve(new Response()), {
                  once: true,
                });
              });
            },
        ...
        ({ controller, server }) =>
          Effect.sync(() => controller.abort()).pipe(
            Effect.andThen(Effect.promise(() => server.finished)),
          ),
      );
      ...
      const runtime = yield* Effect.acquireRelease(
        Effect.sync(() =>
          ManagedRuntime.make(
            Layer.merge(services, HttpRouter.serve(router).pipe(Layer.provide(services))),
          ),
        ),
        (runtime) => Effect.promise(() => runtime.dispose()),
      );
      yield* Effect.promise(() => runtime.context());
      const downstreamServer = yield* Effect.promise(() =>
        runtime.runPromise(HttpServer.HttpServer),
      );
  ...
  it.effect("HttpRouter prefixed", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        yield* HttpClient.get("/todos/1").pipe(
          Effect.flatMap(HttpClientResponse.schemaBodyJson(Todo)),
        ),
        { id: 1, title: "test" },
      );
      assert.strictEqual(
        yield* HttpClient.get("/todos").pipe(Effect.flatMap((_) => _.text)),
        "root",
      );
    }).pipe(Effect.provide(DenoHttpServer.layerTest)),
  );
  ...
  it.effect("cancels a file body for HEAD requests", () =>
    Effect.gen(function* () {
      ...
      yield* fileResponse.pipe(Effect.succeed, HttpServer.serveEffect());
  ...
  it.effect("ignores cancellation errors for HEAD response bodies", () => {
    ...
    return Effect.gen(function* () {
      yield* HttpServerResponse.raw(body).pipe(Effect.succeed, HttpServer.serveEffect());
      const response = yield* HttpClient.head("/");
      assert.strictEqual(response.status, 200);
    }).pipe(
  ...
  it.effect("round trips WebSocket frames and closes cleanly", () =>
    Effect.gen(function* () {
      ...
      const messages = yield* connectWebSocket(
        `ws://127.0.0.1:${port}/`,
