...
describe("DenoSocket", () => {
    ...
    Effect.gen(function* () {
      ...
      const output = yield* Stream.make("Hello", "World").pipe(
        ...
        Stream.pipeThroughChannel(
          DenoSocket.makeTcpChannel({ hostname: address.hostname, port: address.port }),
        ),
        Stream.decodeText(),
        Stream.mkString,
      );
  ...
  it.effect("keeps reading after the write side closes", () =>
    Effect.gen(function* () {
      ...
      const runFiber = yield* socket
        .run((chunk) => Queue.offer(messages, chunk))
  ...
  it.effect("uses Deno's native WebSocket", () =>
    Effect.gen(function* () {
      const server = yield* Effect.acquireRelease(
        Effect.sync(() =>
          Deno.serve({ hostname: "127.0.0.1", port: 0, onListen: () => {} }, (request) => {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.deepStrictEqual(
          ...
          new TextEncoder().encode("Hello WebSocket"),
        );
        yield* Fiber.interrupt(runFiber);
      }).pipe(
        Effect.scoped,
        Effect.provide(
          DenoSocket.layerWebSocket(`ws://${address.hostname}:${address.port}`, {
  ...
  it.effect("adapts a TransformStream", () =>
    Effect.gen(function* () {
      ...
      const socket = yield* Socket.fromTransformStream(Effect.succeed({ readable, writable }), {
        closeCodeIsError: () => false,
      });
