...
const platformError = (method: string, path: string, cause: unknown) =>
  PlatformError.systemError({
    _tag:
      Predicate.hasProperty(cause, "name") && cause.name === "NotFound" ? "NotFound" : "Unknown",
    module: "FileSystem",
    method,
...
describe("DenoChildProcessSpawner options", () => {
  it.effect("rejects detached", () =>
    Effect.gen(function* () {
      const error = yield* ChildProcess.make("echo", ["test"], { detached: false }).pipe(
        Effect.flip,
      );
  ...
  it.effect("uses a custom shell", () =>
    Effect.gen(function* () {
      const result = yield* output(
        ChildProcess.make("printf", ["custom-shell"], { shell: "/bin/sh" }),
      );
