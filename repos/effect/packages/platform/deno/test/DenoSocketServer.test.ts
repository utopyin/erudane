...
describe("DenoSocketServer", () => {
  ...
  it.effect("does not surface BadResource when the listener is closed", () =>
    Effect.gen(function* () {
      ...
      const server = yield* DenoSocketServer.make({ hostname: "127.0.0.1", port: 0 }).pipe(
        Scope.provide(scope),
      );
  ...
  it.effect("logs accept failures and continues accepting", () =>
    Effect.gen(function* () {
      ...
      const listener = makeTestListener(
        [() => Promise.reject(failure), () => Promise.resolve(makeTestConn()), neverAccept],
        called,
      );
      const server = fromListener(listener);
      yield* server
        .run(() => Deferred.succeed(handled, undefined))
        .pipe(
  ...
  it.effect("backs off consecutive accept failures", () =>
    Effect.gen(function* () {
      ...
      yield* server
        .run(() => Effect.void)
        .pipe(Effect.provideService(References.UnhandledLogLevel, undefined), Effect.forkChild);
