...
PersistedCacheTest.suite("DenoRedis", Persistence.layerRedis.pipe(Layer.provide(RedisLayer)));
...
it.effect("closes the connection when interrupted during acquisition", () =>
  Effect.gen(function* () {
    ...
    const layerFiber = yield* Layer.build(
      DenoRedis.layer({
...
const read = (connection: Deno.TcpConn) =>
  Effect.promise(() => {
    ...
    return connection
      .read(buffer)
      .then((size) => (size === null ? null : decoder.decode(buffer.subarray(0, size))));
  });

const write = (connection: Deno.TcpConn, value: string) =>
  Effect.promise(() => connection.write(encoder.encode(value)));
