...
export const make = Effect.fnUntraced(function* (
  ...
  const notFound: Handler = (_request, _info) =>
    Promise.resolve(new Response("not found", { status: 404 }));
  const handlerStack: Array<Handler> = [notFound];
  let currentHandler = notFound;
  const { disablePreemptiveShutdown, gracefulShutdownTimeout, websocket, ...serveOptions } =
    options;
  const server = Deno.serve(serveOptions as Deno.ServeTcpOptions, (request, info) =>
    currentHandler(request, info),
  ) as Deno.HttpServer<Deno.NetAddr | Deno.UnixAddr>;

  const shutdown = yield* Effect.promise(() => server.shutdown()).pipe(Effect.cached);
  const preemptiveShutdown = disablePreemptiveShutdown
    ? Effect.void
    ...
    serve: Effect.fnUntraced(function* (httpApp, middleware) {
      ...
      const httpEffect = HttpEffect.toHandled(
        ...
        (request, response) => {
          ...
          return Effect.flatMap(makeResponse(request, response, services, scope), (response) =>
            Effect.sync(() => denoRequest.resolve(response)),
          );
        },
        middleware,
      );
...
> = Layer.mergeAll(Platform.layer, Etag.layerWeak, DenoServices.layer);
...
class DenoServerRequest extends Inspectable.Class implements ServerRequest.HttpServerRequest {
  ...
  get remoteAddress(): Option.Option<string> {
    return (
      ...
      (this.remoteAddr.transport === "tcp" ? Option.some(this.remoteAddr.hostname) : Option.none())
    );
  }
  ...
  get text(): Effect.Effect<string, Error.HttpServerError> {
    return (this.textEffect ??= Effect.runSync(
      Effect.cached(
        Effect.tryPromise({
  ...
  get multipart(): Effect.Effect<
    ...
    Scope.Scope | FileSystem.FileSystem | Path.Path
  > {
    return (this.multipartEffect ??= Effect.runSync(
      Effect.cached(DenoMultipart.persisted(this.source)),
    ));
  }
          ...
          const onOpen = () => {
            ...
            resume(
              Socket.fromWebSocket(
                Effect.acquireRelease(Effect.succeed(upgrade.socket), (socket) =>
