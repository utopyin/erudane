    ...
    const run = <R, E, _>(
      ...
      Effect.scopedWith(
        Effect.fnUntraced(function* (scope) {
          ...
          const read = Effect.tryPromise(() => reader.read()).pipe(
            Effect.catchIf(
              ...
              () =>
                Effect.succeed({
                  ...
                  value: undefined,
                } as ReadableStreamReadDoneResult<Uint8Array>),
            ),
            Effect.mapError(
              (error) =>
 ...
 * @since 4.0.0
 */
export const layerTcp: (options: ConnectOptions) => Layer.Layer<Socket.Socket, Socket.SocketError> =
...
const isBadResource = (cause: unknown): cause is Deno.errors.BadResource =>
  cause instanceof Deno.errors.BadResource;

const isTeardownError = (cause: unknown): boolean =>
  cause instanceof Deno.errors.Interrupted || isBadResource(cause);
