...
const make = Effect.gen(function* () {
  ...
  const inputToStdioOption = (input: ChildProcess.CommandInput): "piped" | "inherit" | "null" =>
    ...
    Effect.try({
      try: () => {
        ...
        childProcess.status.then(
          ...
          (cause) =>
            Deferred.doneUnsafe(exitSignal, Exit.fail(toPlatformError("status", cause, command))),
        );
      ...
      switch (cmd._tag) {
        ...
        case "PipedCommand": {
          ...
          const kill = (options?: ChildProcess.KillOptions | undefined) =>
            Effect.forEach(
              [...handles].reverse(),
              (handle) => Effect.ignore(handle.kill(options)),
              { discard: true },
            );
