...
export const layerHttpServer: Layer.Layer<
  HttpPlatform | Etag.Generator | DenoServices | HttpServer,
  ServeError,
...
> = Effect.gen(function* () {
  ...
  if (Option.isNone(listenAddress)) {
    return yield* Effect.die(
      "DenoClusterHttp.layerHttpServer: ShardingConfig.runnerAddress is None",
    );
  }
  ...
  const layer: Layer.Layer<any, any, any> = options.clientOnly
    ...
    : options.transport === "http"
      ...
      : Layer.provide(HttpRunner.layerWebsocket, [
          layerHttpServer,
          DenoSocket.layerWebSocketConstructor,
        ]);
          ...
          Layer.provide(
            options.transport === "http"
              ...
              : HttpRunner.layerClientProtocolWebsocketDefault.pipe(
                  Layer.provide(DenoSocket.layerWebSocketConstructor),
                ),
          ),
        );
