...
const makeTempDirectoryFactory =
  (method: string): FileSystem.FileSystem["makeTempDirectory"] =>
  (options) =>
...
const removeFactory =
  ...
  (path, options) => {
    const effect = tryPromise(method, path, () =>
      Deno.remove(path, { recursive: options?.recursive ?? false }),
    );
    return options?.force
      ? Effect.catchTag(effect, "PlatformError", (error) =>
...
const makeTempDirectoryScoped: FileSystem.FileSystem["makeTempDirectoryScoped"] = (options) =>
  Effect.acquireRelease(makeTempDirectoryFactory("makeTempDirectoryScoped")(options), (directory) =>
    Effect.orDie(removeFactory("makeTempDirectoryScoped")(directory, { recursive: true })),
  );
...
class FileImpl implements FileSystem.File {
  ...
  constructor(file: Deno.FsFile, append: boolean) {
    this.file = file;
    this.append = append;
  }
  ...
  private readChunk(method: string, buffer: Uint8Array) {
    return Effect.suspend(() => {
      ...
      return Effect.map(
        tryPromise(method, undefined, async () => {
  ...
  readAlloc(size: FileSystem.SizeInput) {
    ...
    return Effect.suspend(() => {
      ...
      return Effect.map(this.readChunk("readAlloc", buffer), (bytesRead) => {
        ...
        return Option.some(
          bytesRead === BigInt(sizeNumber) ? buffer : buffer.subarray(0, Number(bytesRead)),
        );
      });
    });
  }
...
const makeTempFileFactory =
  (method: string): FileSystem.FileSystem["makeTempFile"] =>
  (options) =>
    tryPromise(method, options?.directory, () =>
      Deno.makeTempFile({
        ...(options?.directory === undefined ? {} : { dir: options.directory }),
...
const makeTempFileScoped: FileSystem.FileSystem["makeTempFileScoped"] = (options) =>
  Effect.acquireRelease(makeTempFileFactory("makeTempFileScoped")(options), (file) =>
    Effect.orDie(removeFactory("makeTempFileScoped")(file, { force: true })),
  );
...
const truncate: FileSystem.FileSystem["truncate"] = (path, length) =>
  tryPromise("truncate", path, () =>
    Deno.truncate(path, length === undefined ? undefined : Number(length)),
  );
    ...
    Effect.map(
      ...
      (watcher) =>
        Stream.fromAsyncIterable(watcher, handleError("FileSystem", "watch", path)).pipe(
          Stream.flatMap(
            (event): Stream.Stream<FileSystem.WatchEvent, PlatformError.PlatformError> => {
              switch (event.kind) {
                case "create":
                  return Stream.map(Stream.fromIterable(event.paths), (path) => ({
