...
export interface IndexedDbQueryBuilder<Source extends IndexedDbVersion.AnyWithProps>
  ...
  ) => Effect.Effect<A, E | IndexedDbQueryError, Exclude<R, IndexedDbTransaction>>;
}
...
const applyDelete = (query: IndexedDbQuery.Delete<any, never>) =>
  Effect.callback<any, IndexedDbQueryError>((resume) => {
    ...
    const store =
      query.delete.index === undefined ? objectStore : objectStore.index(query.delete.index);
...
const applySelect = Effect.fnUntraced(function* (
  ...
  const data =
    predicate || keyPath === undefined || query.offsetValue !== undefined || query.reverseValue
