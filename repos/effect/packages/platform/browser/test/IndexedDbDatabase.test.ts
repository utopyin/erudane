...
describe.sequential("IndexedDbDatabase", () => {
  ...
  it.effect("aborts the versionchange transaction when a migration fails", () => {
    ...
    class Database extends IndexedDbDatabase.make(Version, (transaction) =>
      Effect.andThen(transaction.createObjectStore("entries"), Effect.fail("migration failed")),
    ) {}
    const open = Effect.callback<IDBDatabase, Error>((resume) => {
      const request = indexedDB.open(databaseName);
      request.onerror = () =>
        resume(Effect.fail(request.error ?? new Error("IndexedDB open request failed")));
