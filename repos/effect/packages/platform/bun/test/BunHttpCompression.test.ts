...
const get = (handler: (request: Request) => Promise<Response>, algorithm: Algorithm) =>
    ...
    it(`compresses stream bodies with ${algorithm}`, () =>
      withHandler(
        ...
        async (handler) => {
          ...
          assert.strictEqual(
            decompress(algorithm, new Uint8Array(await response.arrayBuffer())),
            bigJson,
          );
        },
      ));
  ...
  it("compresses Bun.file responses", async () => {
    ...
    await withHandler(
      ...
      async (handler) => {
        ...
        assert.strictEqual(
          decompress("gzip", new Uint8Array(await response.arrayBuffer())),
          contents,
        );
      },
    );
