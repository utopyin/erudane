...
const fetchText = (url: string) =>
  Effect.promise(() =>
    fetch(url, { headers: { connection: "close" } }).then((response) => response.text()),
  );
...
describe("BunHttpServer", () => {
  it.effect("closing an older serve scope keeps the newer handler active", () =>
    Effect.gen(function* () {
      ...
      yield* server
        .serve(Effect.succeed(HttpServerResponse.text("first")))
        .pipe(Scope.provide(firstScope));
      yield* server
        .serve(Effect.succeed(HttpServerResponse.text("second")))
