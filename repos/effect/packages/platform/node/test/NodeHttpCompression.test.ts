...
const bigJson = JSON.stringify({
  text: "All work and no play makes Jack a dull boy. ".repeat(100),
});
const bigJsonApp = Effect.succeed(
  HttpServerResponse.text(bigJson, { contentType: "application/json" }),
);
...
const get = (handler: (request: Request) => Promise<Response>, headers?: Record<string, string>) =>
...
describe("NodeHttpCompression", () => {
  ...
  it.skipIf(!zstdSupported)("compresses with zstd when opted in", () =>
  ...
  it("compresses stream bodies without a Content-Length", () =>
    withHandler(
      Effect.succeed(
        HttpServerResponse.stream(Stream.fromArray([new TextEncoder().encode(bigJson)]), {
      ...
      async (handler) => {
        ...
        assert.strictEqual(
          Zlib.gunzipSync(new Uint8Array(await response.arrayBuffer())).toString(),
          bigJson,
        );
      },
    ));
  ...
  it.effect("closes compressed file bodies for HEAD requests", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => Fs.rmSync(directory, { recursive: true })),
      );
      ...
      const response = yield* HttpServerResponse.file(path, {
      ...
      yield* HttpRouter.add("GET", "/file", Effect.succeed(response)).pipe(
        (self) =>
          HttpRouter.serve(self, { middleware: HttpMiddleware.compression({ minSize: 0 }) }),
        Layer.build,
      );
