...
describe("HttpStaticServer", () => {
  ...
  it("handles range requests for valid, invalid, and malformed headers", async () => {
    await withStaticFiles(async ({ handler }) => {
      const fullBody = await handler(new Request("http://localhost/range.txt")).then((response) =>
        response.text(),
      );
      const fileSize = fullBody.length;

      const first = await handler(
        new Request("http://localhost/range.txt", { headers: { Range: "bytes=0-10" } }),
      );
      ...
      const openEnded = await handler(
        new Request("http://localhost/range.txt", { headers: { Range: "bytes=5-" } }),
      );
      ...
      const suffix = await handler(
        new Request("http://localhost/range.txt", { headers: { Range: "bytes=-10" } }),
      );
      ...
      const invalid = await handler(
        new Request("http://localhost/range.txt", { headers: { Range: "bytes=100-200" } }),
      );
      assert.strictEqual(invalid.status, 416);
      assert.strictEqual(invalid.headers.get("content-range"), `bytes */${fileSize}`);

      const malformed = await handler(
        new Request("http://localhost/range.txt", { headers: { Range: "bytes=abc" } }),
      );
