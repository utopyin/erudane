...
const LocalServerRoutes = HttpRouter.serve(
  HttpRouter.addAll([
    HttpRouter.route(
      "GET",
      "/todos/1",
    ...
    HttpRouter.route(
      "GET",
      "/redirect",
      Effect.succeed(HttpServerResponse.redirect("/redirected")),
    ),
...
[
  ...
  describe(`NodeHttpClient - ${name}`, () => {
    it.effect("text", () =>
      Effect.gen(function* () {
        const response = yield* HttpClient.get("/text").pipe(Effect.flatMap((_) => _.text));
