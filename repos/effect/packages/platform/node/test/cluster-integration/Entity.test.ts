...
const addressString = (address: { readonly host: string; readonly port: number }) =>
...
const GroupEntityLayer = GroupEntity.toLayer(
  Effect.gen(function* () {
    const runner = yield* Entity.CurrentRunnerAddress;
...
const ResourceEntityLayer = ResourceEntity.toLayer(
  Effect.gen(function* () {
...
const RegistrationEntityLayer = RegistrationEntity.toLayer(
  Effect.gen(function* () {
    const runner = yield* Entity.CurrentRunnerAddress;
    return { Runner: () => Effect.succeed(addressString(runner)) };
  }),
...
describe("cluster entity integration", () => {
  for (const backend of ["pg", "mysql"] satisfies ReadonlyArray<Backend>) {
    it.live(
      `${backend}: fails an unroutable reply during runner shutdown without hanging teardown`,
      () =>
        Effect.gen(function* () {
                    ...
                    yield* Effect.addFinalizer(() =>
                      Effect.uninterruptible(
                        Effect.gen(function* () {
                          ...
                          discardExit = yield* receiver("unroutable")
                            .Ping(void 0, { discard: true })
                            .pipe(Effect.exit);
                        }),
                      ),
                    );
    ...
    it.live(
      `${backend}: rebuilds an entity after a fatal defect and replays in-flight requests`,
      () =>
        Effect.gen(function* () {
          ...
          const ReplayEntityLayer = ReplayEntity.toLayer(
            Effect.sync(() => {
              const generation = ++builds;
              return ReplayEntity.of({
                Crash: () =>
                  generation === 1 ? Effect.die("fatal replay defect") : Effect.succeed(generation),
                Hold: () =>
                  Rpc.fork(
                    Effect.gen(function* () {
                      replayAttempts++;
          ...
          const held = yield* client("replay")
            .Hold()
            .pipe(Effect.forkChild({ startImmediately: true }));
          yield* cluster.waitUntil(
    ...
    it.live(
      `${backend}: force-interrupts a stuck handler after the graceful termination timeout`,
      () =>
        Effect.gen(function* () {
          ...
          const request = yield* client("stuck")
            .Run()
            .pipe(Effect.forkChild({ startImmediately: true }));
          yield* cluster.waitUntil(
          ...
          yield* cluster.waitUntil(
            ...
            Effect.map(
              cluster.ownerOfEntity(StuckEntity, "stuck"),
              (next) => next !== undefined && next !== owner,
            ),
          );
    ...
    it.live(`${backend}: holds persisted messages until a slow entity layer registers`, () =>
      Effect.gen(function* () {
        ...
        const starting = yield* cluster
          .start(1, { entities: delayedEntities })
          .pipe(Effect.forkChild({ startImmediately: true }));
        const client = yield* cluster.getClient(RegistrationEntity);
        const request = yield* client(`${backend}-slow-registration`)
          .Runner()
          .pipe(Effect.forkChild({ startImmediately: true }));
        ...
        yield* cluster.waitForEntityOwner(
          RegistrationEntity,
          `${backend}-slow-registration`,
          runner,
        );
    ...
    it.live(`${backend}: defects a durable request whose owner never registers the entity`, () =>
      Effect.gen(function* () {
        ...
        const client = yield* cluster.getClient(RegistrationEntity);
        const request = yield* client(entityId)
          .Runner()
          .pipe(Effect.forkChild({ startImmediately: true }));
        ...
        if (Exit.isFailure(exit)) {
          assert.include(
            ...
            `Entity type '${RegistrationEntity.type}' not registered`,
          );
        }
        assert.deepStrictEqual(yield* cluster.messageCounts(), {
          failed: 1,
    ...
    it.live(`${backend}: routes by entity id, isolates state, and preserves mailbox order`, () =>
      Effect.gen(function* () {
        ...
        for (const [runner, id] of ids) {
          ...
          assert.isFalse(
            cluster.clientSharding.hasShardId(yield* cluster.shardOfEntity(StateEntity, id)),
          );
        }

        const orderedId = ids.values().next().value!;
        const first = yield* client(orderedId)
          .Ordered(new Request({ id: `${backend}-ordered-1`, sequence: 1 }))
          .pipe(Effect.forkChild({ startImmediately: true }));
        yield* cluster.waitUntil(
          "The first ordered request did not start",
          Effect.as(orderEntered.await, true),
        );
        const second = yield* client(orderedId)
          .Ordered(new Request({ id: `${backend}-ordered-2`, sequence: 2 }))
          .pipe(Effect.forkChild({ startImmediately: true }));
    ...
    it.live(
      `${backend}: reports mailbox saturation and revives idle entities with fresh state`,
      () =>
        Effect.gen(function* () {
          generations.clear();
          resetMailbox();
          ...
          const held = yield* mailbox("full")
            .Hold(new Request({ id: `${backend}-held`, sequence: 1 }))
            .pipe(Effect.forkChild({ startImmediately: true }));
          yield* cluster.waitUntil(
            "The mailbox request did not start",
            Effect.as(mailboxEntered.await, true),
          );
          const error = yield* mailbox("full")
            .Hold(new Request({ id: `${backend}-rejected`, sequence: 2 }))
            .pipe(Effect.flip);
    ...
    it.live(`${backend}: rebalances on runner addition, graceful stop, and abrupt death`, () =>
      Effect.gen(function* () {
        ...
        const request = yield* client(stopId)
          .Ordered(new Request({ id: `${backend}-stop`, sequence: 1 }))
          .pipe(Effect.forkChild({ startImmediately: true }));
        yield* cluster.waitUntil(
          "The handover request did not start",
          Effect.as(orderEntered.await, true),
        );
        const stopping = yield* cluster
          .stop(initial[0])
        ...
        yield* cluster.waitUntil(
          ...
          Effect.map(
            cluster.ownerOfEntity(StateEntity, stopId),
            (owner) => owner !== undefined && owner !== initial[0],
          ),
        );
        orderGate.openUnsafe();
    ...
    it.live(`${backend}: transfers frozen row locks after expiry`, () =>
      Effect.gen(function* () {
        ...
        yield* cluster.waitUntil(
          ...
          Effect.map(
            cluster.ownerOfEntity(StateEntity, id),
            (owner) => owner !== undefined && owner !== oldOwner,
          ),
          "12 seconds",
        );
        ...
        const client = yield* cluster.getClient(StateEntity);
        const reply = yield* client(id).Increment(
          new Request({ id: `${backend}-freeze`, sequence: 0 }),
        );
  ...
  it.live("runs one singleton and migrates it after owner death", () =>
    Effect.gen(function* () {
      ...
      yield* cluster.waitForStableAssignments();
      yield* cluster.waitUntil(
        "The singleton did not start",
        Effect.sync(() => singleton.active === 1),
      );
  ...
  it.live("keeps EntityResource alive during movement and releases it explicitly", () =>
    Effect.gen(function* () {
      ...
      yield* cluster.waitUntil(
        ...
        Effect.map(
          cluster.ownerOfEntity(ResourceEntity, "resource"),
          (next) => next !== undefined && next !== owner,
        ),
      );
    ...
    it.live(`${backend}: isolates clusters with different table prefixes`, () =>
      Effect.gen(function* () {
        const first = yield* make({
          backend,
          entities: StateEntityLayer,
          config: { shardsPerGroup: 30 },
        });
        ...
        assert.deepStrictEqual(
          firstRegistrations.map((row) => row.address),
          [addressString(firstRunner.address)],
        );
        assert.deepStrictEqual(
          secondRegistrations.map((row) => row.address),
