...
describe("cluster transport integration", () => {
  for (const backend of ["pg", "mysql"] satisfies ReadonlyArray<Backend>) {
    it.live(
      `${backend}: treats a directional socket cut as RunnerUnavailable for volatile and recovers persisted`,
      () =>
        Effect.gen(function* () {
          ...
          const volatile = yield* client("volatile")
            .Volatile(new Request({ id: `${backend}-volatile` }))
            .pipe(Effect.forkChild({ startImmediately: true }));
          const persisted = yield* client("persisted")
            .Persisted(new Request({ id: `${backend}-persisted` }))
            .pipe(Effect.forkChild({ startImmediately: true }));
