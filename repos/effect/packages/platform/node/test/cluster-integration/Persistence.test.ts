...
describe("cluster message persistence integration", () => {
  for (const backend of ["pg", "mysql"] satisfies ReadonlyArray<Backend>) {
    it.live(
      `${backend}: delivers a persisted request sent while its runner is down exactly once`,
      () =>
        Effect.gen(function* () {
          resetState();
          ...
          const replyFiber = yield* client("restart")
            .Persisted(new KeyedPayload({ id: `${backend}-restart` }))
    ...
    it.live(`${backend}: does not lose an uninterruptible request during runner shutdown`, () =>
      Effect.gen(function* () {
        ...
        const replyFiber = yield* client("uninterruptible")
          .Uninterruptible(new KeyedPayload({ id }))
          .pipe(Effect.forkChild({ startImmediately: true }));
        yield* cluster.waitUntil(
    ...
    it.live(`${backend}: does not store or redeliver a volatile request after runner failure`, () =>
      Effect.gen(function* () {
        ...
        const requestFiber = yield* client("volatile")
          .Volatile(new KeyedPayload({ id }), { discard: true })
          .pipe(Effect.forkChild({ startImmediately: true }));
        yield* cluster.waitUntil(
    ...
    it.live(
      `${backend}: completes a discarded volatile caller while the handler is still blocked`,
      () =>
        Effect.gen(function* () {
          resetState();
          const cluster = yield* make({
          ...
          const caller = yield* client("discarded-volatile")
            .Volatile(new KeyedPayload({ id }), { discard: true })
            .pipe(Effect.forkChild({ startImmediately: true }));
          yield* cluster.waitUntil(
    ...
    it.live(
      `${backend}: stores an unencodable persisted reply as a defect and does not hang the caller`,
      () =>
        Effect.gen(function* () {
          resetState();
          const cluster = yield* make({ backend, entities: PersistenceEntityLayer });
          ...
          yield* assertStoredAsDefect(
            client("unencodable").Unencodable(payload),
            "Unencodable",
            id,
          );
          assert.deepStrictEqual(yield* cluster.messageCounts(), {
    ...
    it.live(
      `${backend}: isolates a reply-encoding failure so a sibling request on the same connection succeeds`,
      () =>
        Effect.gen(function* () {
          resetState();
          const cluster = yield* make({ backend, entities: PersistenceEntityLayer });
          ...
          const healthy = yield* client(siblingEntityId)
            .Healthy(new KeyedPayload({ id: healthyId }))
            .pipe(Effect.forkChild({ startImmediately: true }));
          yield* cluster.waitUntil(
          ...
          const bad = yield* client(badEntityId)
            .Unencodable(new KeyedPayload({ id: badId }))
            .pipe(Effect.exit);
      ...
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          yield* client("failures")
            .TypedFailure(new KeyedPayload({ id: typedId }))
            .pipe(Effect.flip),
          `typed:${typedId}`,
        );
        assert.strictEqual(
        ...
        const firstDefect = yield* client("failures")
          .Defect(new KeyedPayload({ id: defectId }))
          .pipe(Effect.exit);
        ...
        const storedDefect = yield* client("failures")
          .Defect(new KeyedPayload({ id: defectId }))
          .pipe(Effect.exit);
        ...
        yield* cluster.waitUntil(
          ...
          Effect.map(
            ...
            (counts) => counts.failed === 2 && counts.replied === 1,
          ),
        );
        assert.deepStrictEqual(yield* cluster.messageCounts(), {
          failed: 2,
    ...
    it.live(`${backend}: defects a malformed persisted envelope without wedging the mailbox`, () =>
      Effect.gen(function* () {
        ...
        const shardId = cluster.clientSharding.getShardId(
          entityId,
          PersistenceEntity.getShardGroup(entityId),
        );
    ...
    it.live(
      `${backend}: abandons a stream chunk acknowledgement during shutdown without stranding either runner`,
      () =>
        Effect.gen(function* () {
          resetState();
          const cluster = yield* make({ backend, entities: PersistenceEntityLayer });
          ...
          const values = yield* client(entityId)
            .Streamed(new KeyedPayload({ id }))
    ...
    it.live(`${backend}: resumes a persisted stream after its runner is killed`, () =>
      Effect.gen(function* () {
        ...
        const valuesFiber = yield* client("stream-restart")
          .Streamed(new KeyedPayload({ id }))
          .pipe(
    ...
    it.live(
      `${backend}: resumes a terminal stream on the replacement owner and persists exactly one reply`,
      () =>
        Effect.gen(function* () {
          resetState();
          yield* Effect.addFinalizer(() =>
            Effect.sync(() => state.streamTerminalGate.openUnsafe()),
          );
          const cluster = yield* make({ backend, entities: PersistenceEntityLayer });
          const runners = yield* cluster.start(2);
          yield* cluster.waitForStableAssignments();
    ...
    it.live(`${backend}: delivers scheduled messages only after their deadline`, () =>
      Effect.gen(function* () {
        ...
        const replyFiber = yield* client("scheduled")
          .Scheduled(new ScheduledPayload({ deliverAt, id }))
          .pipe(Effect.forkChild({ startImmediately: true }));
