...
describe("cluster lock-storage integration", () => {
  for (const backend of ["pg", "mysql"] satisfies ReadonlyArray<Backend>) {
      ...
      () =>
        Effect.gen(function* () {
          const cluster = yield* make({
            backend,
            entities: Layer.empty,
            config: { shardsPerGroup: 4 },
          });
          ...
          yield* cluster.waitUntil(
            ...
            Effect.gen(function* () {
              ...
              assert(
                assignments.every((owners) => owners.length <= 1),
                "A shard had multiple owners",
              );
              return (
      ...
      Effect.gen(function* () {
        ...
        yield* cluster.waitUntil(
          ...
          Effect.sync(() => {
            ...
            assert(
              owners.every((shardOwners) => shardOwners.length <= 1),
              "A shard had multiple owners",
            );
            const complete =
              owners.every((shardOwners) => shardOwners.length === 1) &&
    ...
    it.live(`${backend}: keeps peers progressing when a lock query is permanently stuck`, () =>
      Effect.gen(function* () {
        ...
        yield* cluster.waitUntil(
          ...
          Effect.sync(() =>
            Object.values(cluster.assignmentMap()).every(
              (owners) => !owners.includes(faultedAddress),
            ),
          ),
        );
    ...
    it.live(
      `${backend}: converges after repeated lock-connection rebuilds and a hung release`,
      () =>
        Effect.gen(function* () {
          const cluster = yield* make({
            backend,
          ...
          yield* cluster.waitUntil(
            ...
            Effect.gen(function* () {
              ...
              assert(
                assignments.every(
                  (owners) => owners.length === 1 && owners.includes(faultedAddress),
                ),
          ...
          yield* cluster.waitUntil(
            ...
            Effect.sync(() =>
              Object.values(cluster.assignmentMap()).some((owners) => owners.includes(peerAddress)),
            ),
          );
