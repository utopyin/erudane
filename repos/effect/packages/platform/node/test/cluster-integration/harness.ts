...
const makeLockFaultController = (sql: SqlClient.SqlClient) => {
  ...
  const wrapConnection = (
    ...
    const execute = <A, E extends SqlError.SqlError, R>(
      sql: string,
      ...
      Effect.suspend((): Effect.Effect<A, E | SqlError.SqlError, R> => {
        ...
        if (sql.includes("pg_advisory_unlock_all") || sql.includes("RELEASE_ALL_LOCKS"))
          return effect;
    ...
    return {
      ...
      executeValuesUnprepared: (...args) =>
        execute(args[0], connection.executeValuesUnprepared(...args)),
        ...
        return Effect.gen(function* () {
          ...
          yield* Effect.addFinalizer(() =>
            Effect.uninterruptible(
              Effect.suspend(() =>
                persistentFault === "hangRelease" ? releaseGate.await : Effect.void,
              ),
            ),
          );
...
const makeRunnerStorageController = (storage: RunnerStorage.RunnerStorage["Service"]) => {
  ...
  const waitWhileFrozen = <A, E>(
    ...
    Effect.suspend(() => {
      ...
      return gate.await.pipe(
        Effect.uninterruptible,
        Effect.andThen(
          Effect.suspend(() => (mode === "running" ? effect : Effect.succeed(onKilled()))),
        ),
      );
    });
  ...
  const controlled = RunnerStorage.RunnerStorage.of({
    ...
    release: (address, shardId) =>
      mode === "killed" ? Effect.void : storage.release(address, shardId),
...
export const make = Effect.fnUntraced(function* (options: MakeOptions) {
  ...
  const entities =
    typeof options.entities === "function" ? options.entities({ prefix }) : options.entities;
  ...
  const clientBase = yield* clientLayer(
    config,
    trackedClientProtocolLayer(socketController, clientPeer),
  ).pipe(Layer.buildWithScope(clientScope), Effect.provide(Context.merge(shared, clientStorage)));
  ...
  const startRunner = Effect.fnUntraced(function* (index: number, startOptions?: StartOptions) {
    ...
    const controller = makeRunnerStorageController(
      Context.get(rawStorageContext, RunnerStorage.RunnerStorage),
    );
    ...
    ).pipe(Layer.buildWithScope(scope), Effect.provide(Context.mergeAll(shared, storage)));
  ...
  const faultLock = (runner: ClusterRunner, mode: LockFaultMode) =>
    entryFor(runner).lockFault.set(mode);
  ...
  const cutSocket = (runner: ClusterRunner, options: CutSocketOptions) => {
    ...
    const [source, target] =
      options.direction === "inbound" ? [peerAddress, runnerAddress] : [runnerAddress, peerAddress];
  ...
  const ownersOfShard = (shard: ShardId.ShardId, includeInactive = false) =>
    runners.filter(
      (runner) =>
        (includeInactive || runner.state() === "running") && runner.sharding.hasShardId(shard),
    );
  ...
  const waitForStableAssignments = Effect.fnUntraced(function* (timeout?: Duration.Input) {
    ...
    yield* waitUntil(
      ...
      Effect.sync(() => {
        ...
        const complete =
          Object.values(current).every((owners) => owners.length === 1) &&
          runners.every(
            (runner) =>
              runner.state() !== "running" ||
              owners.has(`${runner.address.host}:${runner.address.port}`),
          );
  ...
  const ownerOfEntity = Effect.fnUntraced(function* <Type extends string, Rpcs extends Rpc.Any>(
    ...
    return runners.find(
      (runner) => runner.state() === "running" && runner.sharding.hasShardId(shardId),
    ) as ClusterRunner | undefined;
  });
  ...
  const getClient = <Type extends string, Rpcs extends Rpc.Any>(
    entity: Entity.Entity<Type, Rpcs>,
