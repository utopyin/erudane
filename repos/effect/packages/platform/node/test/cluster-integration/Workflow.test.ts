...
const ReplayWorkflowLayer = ReplayWorkflow.toLayer(
...
class CompensationError extends Schema.Error<CompensationError>(
  "ClusterIntegrationCompensationError",
)({
...
const CompensationWorkflowLayer = CompensationWorkflow.toLayer(
  Effect.fnUntraced(function* ({ id }) {
    yield* Activity.make({
...
const SuspendFailureWorkflowLayer = SuspendFailureWorkflow.toLayer(
  Effect.fnUntraced(function* ({ id }) {
...
const RaceBoundaryWorkflowLayer = RaceBoundaryWorkflow.toLayer(
  Effect.fnUntraced(function* ({ id }) {
...
const QueueWorkerLayer = Layer.effectDiscard(
  Effect.forkScoped(
    Effect.suspend(() => queueWorkerGate.await).pipe(
      Effect.andThen(
        DurableQueue.makeWorker(Queue, ({ id }) =>
          Effect.sync(() => {
...
const InterruptWorkflowLayer = InterruptWorkflow.toLayer(() =>
  DurableDeferred.await(InterruptGate),
);
...
const DeferredControlLayer = DeferredControl.toLayer(
  Effect.gen(function* () {
  ...
  for (const backend of ["pg", "mysql"] satisfies ReadonlyArray<Backend>) {
    it.live(`${backend}: executes end to end and deduplicates concurrent callers`, () =>
      Effect.gen(function* () {
        ...
        const first = yield* withWorkflow(
          cluster,
          EndToEndWorkflow.execute({ id, value: 41 }),
        ).pipe(Effect.forkChild({ startImmediately: true }));
        yield* cluster.waitUntil(
          "The end-to-end activity did not start",
          Effect.as(endToEndEntered.await, true),
        );
        const second = yield* withWorkflow(
          cluster,
          EndToEndWorkflow.execute({ id, value: 41 }),
        ).pipe(Effect.forkChild({ startImmediately: true }));
    ...
    it.live(`${backend}: replays completed activities after the owner dies`, () =>
      Effect.gen(function* () {
        ...
        const executionId = yield* withWorkflow(
          cluster,
          ReplayWorkflow.execute({ id }, { discard: true }),
        );
        ...
        yield* cluster.waitForStableAssignments();
        const token = DurableDeferred.tokenFromExecutionId(ReplayGate, {
          workflow: ReplayWorkflow,
          executionId,
        });
        yield* withWorkflow(
          cluster,
          DurableDeferred.succeed(ReplayGate, { token, value: "resumed" }),
        );
    ...
    it.live(
      `${backend}: resumes deferred workflows across a whole-cluster restart from another runner`,
      () =>
        Effect.gen(function* () {
          ...
          const executionId = yield* withWorkflow(
            cluster,
            RestartWorkflow.execute({ id }, { discard: true }),
          );
          ...
          const token = DurableDeferred.tokenFromExecutionId(RestartGate, {
            workflow: RestartWorkflow,
            executionId,
          });
          const completedBy = yield* control(controlId).CompleteDeferred({
            token,
            value: "after-restart",
          });
          assert.strictEqual(
            completedBy,
            `${controlOwner.address.host}:${controlOwner.address.port}`,
          );
    ...
    it.live(
      `${backend}: wakes a DurableDeferred raceAll and replays the winner after owner death`,
      () =>
        Effect.gen(function* () {
          ...
          const executionId = yield* withWorkflow(
            cluster,
            RaceReplayWorkflow.execute({ id }, { discard: true }),
          );
          yield* cluster.waitUntil(
    ...
    it.live(`${backend}: runs compensation and SuspendOnFailure after the owner is lost`, () =>
      Effect.gen(function* () {
        ...
        yield* Effect.forEach(new Set([compensationOwner!, suspendOwner!]), cluster.kill, {
          discard: true,
        });
        yield* cluster.waitForStableAssignments();
    ...
    it.live(`${backend}: persists queued work across restart and consumes it once`, () =>
      Effect.gen(function* () {
        ...
        const executionId = yield* withWorkflow(
          cluster,
          QueueWorkflow.execute({ id }, { discard: true }),
        );
