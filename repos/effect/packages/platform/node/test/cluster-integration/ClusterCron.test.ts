...
describe("cluster cron integration", () => {
  for (const backend of ["pg", "mysql"] satisfies ReadonlyArray<Backend>) {
    ...
    it.live(
      `${backend}: calculates the next run from the previous instant or the current time`,
      () =>
        Effect.gen(function* () {
          ...
          const previousSecond = DateTime.toEpochMillis(
            DateTime.makeUnsafe(previousTicks[1].scheduled),
          );
          const currentSecond = DateTime.toEpochMillis(
            DateTime.makeUnsafe(currentTicks[1].scheduled),
          );
    ...
    it.live(
      `${backend}: catches up or skips stale runs and preserves the schedule across restart`,
      () =>
        Effect.gen(function* () {
          ...
          const firstAfterSkipScheduledAt = DateTime.toEpochMillis(
            DateTime.makeUnsafe(firstAfterSkip.scheduled),
          );
          assert.strictEqual(
          ...
          assert.isAtLeast(
            firstCatchUp.at - DateTime.toEpochMillis(DateTime.makeUnsafe(firstCatchUp.scheduled)),
            2_000,
          );
    ...
    it.live(
      `${backend}: resumes without duplicate or missing ticks after the singleton owner dies`,
      () =>
        Effect.gen(function* () {
          ...
          assert.isTrue(
            ticks.slice(afterKill).every((tick) => tick.runner !== addressString(owner!.address)),
          );
        }),
    );
    ...
    it.live(`${backend}: assigns cron singletons and executions to their shard groups`, () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          yield* cluster.ownerOfEntity(cronProbe(defaultName), defaultName),
          defaultRunner,
        );
        assert.strictEqual(
        ...
        assert.isTrue(
          defaultTicks.every((tick) => tick.runner === addressString(defaultRunner.address)),
        );
        assert.isTrue(
          specialTicks.every((tick) => tick.runner === addressString(specialRunner.address)),
        );
      }),
    );
  }
