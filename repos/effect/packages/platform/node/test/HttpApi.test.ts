...
describe("HttpApi", () => {
  it.effect("catch all path", () => {
    ...
    const ApiLayer = HttpRouter.serve(HttpApiBuilder.layer(Api).pipe(Layer.provide(GroupLayer)), {
  ...
  describe("middleware", () => {
    it.effect("error", () => {
      ...
      const MLayer = Layer.succeed(M, () => Effect.fail("error"));
    ...
    it.effect("error array", () => {
      ...
      const MLayer = Layer.succeed(M, (_, { endpoint }) =>
    ...
    it.effect("addHttpApi + middleware works across merged groups", () => {
      ...
      const UsersLayer = HttpApiBuilder.group(Api, "users", (handlers) =>
        handlers.handle("list", () => Effect.succeed("ok")),
      );
      const M1Layer = Layer.succeed(M1, (effect, { endpoint, group }) =>
        Effect.sync(() => calls.push(`m1:${group.identifier}.${endpoint.identifier}`)).pipe(
          Effect.flatMap(() => effect),
        ),
      );
      const M2Layer = Layer.succeed(M2, (effect, { endpoint, group }) =>
  ...
  it.effect("missing addHttpApi group layer has actionable error", () => {
    ...
    const ApiLayer = HttpRouter.serve(HttpApiBuilder.layer(Api).pipe(Layer.provide(UsersLayer)), {
      disableListenLog: true,
      disableLogger: true,
    }).pipe(Layer.provideMerge(NodeHttpServer.layerTest));
  ...
  describe("success option", () => {
    ...
    it("no content via toWebHandler", async () => {
      ...
      const { handler, dispose } = HttpRouter.toWebHandler(Layer.mergeAll(ApiLayer), {
        disableLogger: true,
      });
      try {
...
const AuthorizationLayer = Layer.succeed(Authorization)({
  cookie: (effect, opts) =>
    Effect.provideService(
      effect,
      CurrentUser,
...
).pipe(Layer.provide([UserRepo.layer, AuthorizationLayer]));

const HttpGroupsLayer = HttpApiBuilder.group(Api, "groups", (handlers) =>
  handlers
...
const TopLevelLayer = HttpApiBuilder.group(Api, "root", (handlers) =>
