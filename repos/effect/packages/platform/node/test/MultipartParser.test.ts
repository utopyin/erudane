...
type Expected = Array<
  ...
  | [type: "file", name: string, bytesReceived: number, filename: string, contentType: string]
>;
...
const cases: ReadonlyArray<MultipartCase> = [
  ...
  },
  {
    ...
    expected: [
      ["field", "file_name_0", "super alpha file", "text/plain"],
      ["field", "file_name_1", "super beta file", "text/plain"],
      ["file", "upload_file_0", 1024 * 1024 * 10, "1k_a.dat", "application/octet-stream"],
    ],
  ...
  },
  {
    ...
    boundary: "---------------------------paZqsnEHRufoShdX6fh0lUhXBP4k",
    expected: [
      ["file", "upload_file_0", 26, "/absolute/1k_a.dat", "application/octet-stream"],
      ["file", "upload_file_1", 26, "C:\\absolute\\1k_b.dat", "application/octet-stream"],
      ["file", "upload_file_2", 26, "relative/1k_c.dat", "application/octet-stream"],
    ],
  ...
  },
  {
    ...
    boundary: "---------------------------paZqsnEHRufoShdX6fh0lUhXBP4k",
    expected: [["field", "upload_file_0", "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "application/json"]],
  ...
  },
  {
    ...
    boundary: "---------------------------paZqsnEHRufoShdX6fh0lUhXBP4k",
    expected: [
      ["field", "upload_file_0", "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "application/json"],
  ...
  },
  {
    ...
    expected: [
      ...
      ["field", "file", "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "application/octet-stream"],
    ],
      ...
      onFile: (info) => {
        ...
        return (chunk) => {
          if (chunk) {
            ...
            parts.push(["file", info.name, size, info.filename!, info.contentType]);
          }
        };
      },
      onField: (info, value) => {
        parts.push(["field", info.name, Multipart.decodeField(info, value), info.contentType]);
      },
      onError: (error) => {
...
describe("node api", () => {
  ...
  test.each(cases)("$name", (opts) => {
    ...
    parser.on("file", (file) => {
      ...
      file.on("end", () => {
        parts.push(["file", file.info.name, size, file.info.filename!, file.info.contentType]);
      });
    });
    ...
    async function read() {
      for await (const part of parser) {
        if (part._tag === "Field") {
          ...
          parts.push(["file", part.info.name, size, part.info.filename!, part.info.contentType]);
        }
      }
    }
    ...
    for (let i = 0; i < 100; i++) {
      ...
      while (cursor < buffer.length) {
        ...
        const chunkSize = Math.max(
          1,
          Math.min(Math.round(random() * 128 * 1024 * 1024), maxChunkSize),
        );
