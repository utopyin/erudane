...
describe("SqlRunnerStorage", () => {
  it.effect(
    "bounds shard lock operations and rebuilds an unresponsive reserved connection",
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const expectDeadline = Effect.fnUntraced(function* (
          operation: Effect.Effect<unknown, unknown>,
        ) {
          const [elapsed, exit] = yield* operation.pipe(Effect.exit, Effect.timed);
        ...
        const expectRecovery = Effect.fnUntraced(function* () {
          ...
          expect(
            yield* storage
              .refresh(runnerAddress1, shards)
              .pipe(Effect.retry({ times: 5, schedule: Schedule.spaced(20) })),
          ).toEqual(shards);
        });
  ...
  it.effect(
    "recovers when a blackholed query cannot resume after the partition clears",
    () => {
      const partitioned = makePartitionState();
      const layer = StorageLayer.pipe(
  ...
  it.effect(
    "rebuilds the reserved connection again when a rebuilt connection stops responding",
    () => {
      const partitioned = makePartitionState();
      const layer = StorageLayer.pipe(
        Layer.provideMerge(blackholeReservedConnection(partitioned, false)),
  ...
  it.effect(
    "isolates advisory shard locks by prefix",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "excludes other storages using the same prefix",
    () =>
      Effect.gen(function* () {
        const storageA = yield* SqlRunnerStorage.make({ prefix: "cluster" });
    ...
    .flatMap(
      ([label, layer]) =>
        [
          [
            label,
            StorageLayer.pipe(Layer.provideMerge(layer), Layer.provide(ShardingConfig.layer())),
          ],
          [
            label + " (no advisory)",
            StorageLayer.pipe(
      ...
      const wrapConnection = (connection: SqlConnection.Connection): SqlConnection.Connection => {
        ...
        const execute = <A, E extends SqlError.SqlError, R>(effect: Effect.Effect<A, E, R>) =>
          Effect.suspend((): Effect.Effect<A, E | SqlError.SqlError, R> => {
            ...
            partitioned.maxActiveQueries = Math.max(
              partitioned.maxActiveQueries,
              partitioned.activeQueries,
            );
            return (
              !Latch.isOpen(partitioned.connectionAvailable)
                ? resumePending
        ...
        return {
          ...
          executeValuesUnprepared: (...args) =>
            execute(connection.executeValuesUnprepared(...args)),
            ...
            return Effect.andThen(
              ...
              Effect.addFinalizer(() =>
                Effect.uninterruptible(
                  Effect.suspend(function waitForRelease(): Effect.Effect<void> {
