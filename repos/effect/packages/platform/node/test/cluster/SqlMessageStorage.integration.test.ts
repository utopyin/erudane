...
describe("SqlMessageStorage", () => {
    ...
    it.layer(StorageLayer.pipe(Layer.provideMerge(layer)), {
      ...
      if (label === "sqlite") {
        ...
        it.effect("detects duplicates for legacy long-key plaintext rows", () =>
          Effect.gen(function* () {
            ...
            yield* sql`UPDATE cluster_messages SET message_id = ${plaintext} WHERE id = ${String(
      ...
      it.effect("unprocessedMessages honors the limit and claims only returned rows", () =>
        Effect.gen(function* () {
          ...
          for (let i = 1; i <= 5; i++) {
            yield* storage.saveRequest(
              yield* makeRequest({ payload: { id: i }, entityId: String(i) }),
            );
          }
      ...
      it.effect("encoded unprocessedMessages fails closed for empty addresses", () =>
        Effect.gen(function* () {
          ...
          const request = yield* makeRequest();
          yield* storage.saveRequest(request);
          const encoded = yield* SqlMessageStorage.makeEncoded().pipe(
  ...
  it.effect(
    "keeps held messages readable while entity layers build",
    () =>
      Effect.gen(function* () {
      ...
      }).pipe(Effect.provide(StorageLayer.pipe(Layer.provideMerge(SqliteLayer)))),
    { timeout: 15_000 },
  );
});
