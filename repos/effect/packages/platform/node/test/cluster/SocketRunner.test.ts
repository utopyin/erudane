...
const IsolationEntity = Entity.make("IsolationEntity", [
  Rpc.make("BadReply", {
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          const volatileFiber = yield* client
            .ProcessVolatile(TestPayload.make({ id: "req-2", amount }), { discard: true })
  ...
  it.live(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Layer.launch(makeRunnerLayer(ISOLATION_PORT, IsolationEntityLayer)).pipe(
          Effect.forkScoped,
        );
        yield* Effect.sleep("2 seconds");
        ...
        yield* Effect.gen(function* () {
          ...
          assert.include(
            failure,
            "MalformedMessage",
            "the caller must receive the real encode error",
          );
          assert.notInclude(
            failure,
        ...
        }).pipe(Effect.provide(makeClientLayer(ISOLATION_PORT)), Effect.scoped);
      }).pipe(Effect.provide(SharedStorage)),
    30_000,
  );
});
