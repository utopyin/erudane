...
const isStatement = (node: Node): boolean =>
  node.type.endsWith("Statement") || statementTypes.has(node.type);

const collect = (
  program: ESTree.Program,
): { readonly candidates: Array<Candidate>; readonly names: Set<string> } => {
...
const assertionTarget = (
  ...
  const target = candidates
    .filter(
      ({ node }) =>
        node.end <= comment.start && /^[ \t]*$/.test(source.slice(node.end, comment.start)),
    )
...
const parseExpected = (
  ...
  const parsed = parseSync(`${file}?doctest-expected`, `const __expected = (${expected})`, {
    lang: "ts",
  });
...
export const transform = (source: string, file: string, line: number): string => {
  ...
  if (parseError !== undefined) {
    fail(
      ...
      file,
      line,
      `invalid doctest source: ${parseError.message}`,
    );
  }
  ...
  for (const { comment, expected } of markers) {
    ...
    if (node.type === "VariableDeclaration" && parent?.type !== "ExportNamedDeclaration") {
      ...
      const declaration =
        Array.isArray(declarations) && declarations.length === 1 ? declarations[0] : undefined;
