...
const referenceResolution = (name: ts.EntityName, context: SerializationContext): unknown => {
  ...
  const publicApiId =
    context.publicSymbols.get(symbolIdentity(symbol, "type")) ??
  ...
  if (nodeModules !== -1) {
    const remainder = file
      .slice(nodeModules + `${separator}node_modules${separator}`.length)
      .split(separator);
    return {
      externalPackage: remainder[0]?.startsWith("@")
        ? remainder.slice(0, 2).join("/")
        : remainder[0],
    };
  }
...
const serializeTypeParameters = (
  ...
  readonly parameters?: ReadonlyArray<TypeParameterModel>;
...
const serializeSignature = (
  ...
  return {
    ...
    returnType:
      node.type === undefined ? { kind: "unknown" } : serializeType(node.type, generic.context),
    modifiers: modifiers(node),
  };
};

const serializeTypeMember = (
  node: ts.TypeElement,
  context: SerializationContext,
): DeclarationModel => {
...
const serializeObjectMembers = (
  ...
  members
    .map((member) => serializeTypeMember(member, context))
    .sort((left, right) =>
...
export const serializeType = (node: ts.TypeNode, context: SerializationContext): TypeModel => {
  ...
  if (ts.isTupleTypeNode(node)) {
    return {
      kind: "tuple",
      elements: node.elements.map((element) => serializeType(element, context)),
    };
  }
  ...
  if (ts.isMappedTypeNode(node)) {
    const generic = serializeTypeParameters(
      ...
      optional:
        node.questionToken?.kind === ts.SyntaxKind.MinusToken
          ...
          : node.questionToken === undefined
            ...
            : "add",
      nameType:
        node.nameType === undefined ? undefined : serializeType(node.nameType, generic.context),
      ...
      if (
        ...
        if (ts.isVariableStatement(statement)) {
          return statement.declarationList.declarations.map((declaration) =>
            serializeDeclaration(declaration, context),
          );
        }
...
const symbolBuckets = (symbol: ts.Symbol): ReadonlyArray<Bucket> => {
  ...
  if (
    ...
    (symbol.flags & ts.SymbolFlags.Namespace) !== 0
  ) {
    output.push("value");
  }
  return output;
...
const symbolIdentity = (symbol: ts.Symbol, bucket: Bucket): string => {
  const declarations = symbol.declarations ?? [];
  return `${bucket}:${declarations
    .map((node) => `${node.getSourceFile().fileName}:${node.pos}:${node.end}`)
    .sort()
    .join("|")}`;
};
...
const snapshotExtractionError = (
  diagnostics: ReadonlyArray<SnapshotDiagnostic>,
): SnapshotExtractionError =>
...
const extractSnapshot = (
  ...
  const compilerDiagnostics = program
    .getSyntacticDiagnostics()
    .filter((diagnostic) => diagnostic.category === ts.DiagnosticCategory.Error);
  if (compilerDiagnostics.length > 0) {
    throw snapshotExtractionError(
  ...
  for (const pendingEntity of pending.values()) {
    ...
    entities.push({
      ...
      importRoutes: [...pendingEntity.routes].sort(
        (left, right) =>
          left.module.localeCompare(right.module) ||
          left.path.join(".").localeCompare(right.path.join(".")),
      ),
  ...
  const packages = [
    ...new Set(discovered.entrypoints.map((entrypoint) => entrypoint.packageName)),
  ].sort();
  return {
    version: 1,
...
export const snapshotCacheKey = (sha: string, modules?: ReadonlyArray<string>): string =>
  fingerprint([
    "snapshot-v4",
    sha,
    ts.version,
...
export class Snapshotter extends Context.Service<
  Snapshotter,
  {
    readonly extract: (
      options: ExtractSnapshotOptions,
    ) => Effect.Effect<ApiSnapshot, ApiDiffError | SnapshotExtractionError>;
  }
>()("@effect/api-diff/Snapshotter") {
  ...
  static readonly layer = this.layerNoDependencies.pipe(Layer.provide(Discovery.layer));
}
