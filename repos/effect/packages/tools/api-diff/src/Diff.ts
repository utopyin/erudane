...
const groupSimilarity = (base: ApiGroup, head: ApiGroup): number => {
  ...
  const sameKind =
    facets.filter(({ base, head }) => base.declarationKind === head.declarationKind).length /
    facets.length;
  const sameFingerprint =
    facets.filter(({ base, head }) => base.fingerprint === head.fingerprint).length / facets.length;
  const structure =
    facets.reduce(
  ...
  const documentation =
    facets.reduce(
      (total, { base, head }) =>
        total + setSimilarity(documentationTokens(base), documentationTokens(head)),
      0,
    ) / facets.length;
  const sameCategory = facets.some(
    ({ base, head }) =>
  ...
  const mentionsHeadModule =
    headModuleName !== undefined &&
...
const changeId = (
  ...
  `change-${fingerprint([classification, base?.id ?? null, head?.id ?? null, suffix]).slice(0, 16)}`;
...
const classifyStructure = (match: Match): ReadonlyArray<ApiChange> => {
  ...
  if (base.declarationKind !== head.declarationKind) {
    changes.push(
      makeChange("declaration-kind-changed", match, {
  ...
  const baseSignatures = baseDeclarations.filter(
    (declaration) => declaration.kind === "function" || declaration.kind === "method",
  );
  const headSignatures = headDeclarations.filter(
    (declaration) => declaration.kind === "function" || declaration.kind === "method",
  );
  if (headSignatures.length > baseSignatures.length) {
    changes.push(
      makeChange("overload-added", match, {
    ...
    changes.push(
      makeChange("overload-removed", match, {
        before: baseSignatures.length,
        after: headSignatures.length,
      }),
    );
    ...
    stableJson(baseSignatures.map(declarationHash).sort()) ===
      stableJson(headSignatures.map(declarationHash).sort()) &&
    stableJson(baseSignatures.map(declarationHash)) !==
  ...
  if (stableJson(baseHeritage) !== stableJson(headHeritage)) {
    changes.push(
      makeChange("heritage-changed", match, { before: baseHeritage, after: headHeritage }),
    );
  }
  ...
  const baseKinds = new Set(
    baseTypes.flatMap((type) => (type?.kind === undefined ? [] : [type.kind])),
  );
  const headKinds = new Set(
    headTypes.flatMap((type) => (type?.kind === undefined ? [] : [type.kind])),
  );
  if (
    (baseKinds.has("union") || headKinds.has("union")) &&
    stableJson(baseTypes) !== stableJson(headTypes)
  ) {
    changes.push(
      makeChange("union-member-changed", match, { before: baseTypes, after: headTypes }),
    );
  }
  if (
    ...
    changes.push(
      makeChange("intersection-member-changed", match, { before: baseTypes, after: headTypes }),
    );
  }
  if (stableJson(base.documentation) !== stableJson(head.documentation)) {
    changes.push(
      makeChange("documentation-changed", match, {
  ...
  if (base.fingerprint !== head.fingerprint && changes.length === 0) {
    changes.push(
      makeChange("structural-change", match, {
...
const movementChange = (match: Match): ApiChange | undefined => {
  ...
  if (
    match.base.module !== match.head.module ||
    match.base.path.join(".") !== match.head.path.join(".")
  ) {
...
export const diffSnapshots = (base: ApiSnapshot, head: ApiSnapshot): ApiDiff => {
  ...
  const addMatch = (
    ...
    if (
      ...
      !unmatchedBase.has(baseEntity.id) ||
      !unmatchedHead.has(headEntity.id)
    ) {
      return false;
    }
  ...
  for (const baseGroup of groupEntities(unmatchedBase.values())) {
    ...
    if (
      best === undefined ||
      best.score < 0.5 ||
      (next !== undefined && best.score - next.score < 0.05)
    ) {
      continue;
    }
    for (const baseEntity of baseGroup.entities) {
      const headEntity = best.group.entities.find(
        (candidate) => candidate.bucket === baseEntity.bucket,
      );
