...
export class Worktrees extends Context.Service<
  Worktrees,
  {
    ...
    ) => Effect.Effect<ApiSnapshot, ApiDiffError>;
  }
>()("@effect/api-diff/Worktrees") {
  static readonly layerNoDependencies = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const runCommandScoped = Effect.fnUntraced(function* (
        ...
        const handle = yield* spawner.spawn(ChildProcess.make(command, args, { cwd })).pipe(
          Effect.mapError(
            (cause) =>
              new ApiDiffError({
      ...
      const runCommand = (
        ...
        ApiDiffError
      > => Effect.scoped(runCommandScoped(command, args, cwd));
      ...
      const hasProductionStripInternal = Effect.fnUntraced(function* (worktree: string) {
        ...
        for (const candidate of candidates) {
          if (
            ...
            /"stripInternal"\s*:\s*true/.test(yield* fs.readFileString(candidate))
          ) {
            return true;
          }
        }
      ...
      const prepareSnapshotInternal = Effect.fnUntraced(function* (
        options: PrepareSnapshotOptions,
      ) {
        ...
        return yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            return yield* Effect.acquireUseRelease(
              runChecked(
                "git",
                ["worktree", "add", "--detach", worktree, options.sha],
                options.repoRoot,
              ),
              () =>
                Effect.gen(function* () {
                  yield* buildWorktree(worktree);
                  const snapshot = yield* snapshotter
                    .extract({
                      repoRoot: worktree,
                      ref: options.ref,
                      sha: options.sha,
  ...
  static readonly layer = this.layerNoDependencies.pipe(Layer.provide(Snapshotter.layer));
}
