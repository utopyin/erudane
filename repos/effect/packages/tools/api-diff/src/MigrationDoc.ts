...
const codeSpan = (value: string): string => {
  const delimiter = "`".repeat(
    Math.max(1, ...(value.match(/`+/g)?.map((run) => run.length + 1) ?? [])),
  );
...
const isBreakingChange = (change: ApiChange): boolean => {
  ...
  return (
    delta?.after?.slice(beforeLength).some((parameter) => !parameter.optional && !parameter.rest) ??
    true
  );
};
...
const addedApiSignatures = (diff: ApiDiff): ReadonlyMap<string, ReadonlyArray<string>> => {
  ...
  for (const change of diff.changes) {
    if (
      change.classification !== "api-added" ||
...
const migrationEntries = (
  ...
  return entries.sort(
    (left, right) => compareStrings(left.module, right.module) || compareStrings(left.id, right.id),
  );
};
...
const renderCompactEntry = (
  ...
  if (annotation !== undefined) {
    return `- ${codeSpan(displayApiId(entry.id))} -> ${codeSpan(annotation.replacement)}: ${escapeAnnotationText(
      annotation.note,
    )}`;
  }
...
export const renderMigrationDocument = (
  ...
  for (const module of modulesRemoved) {
    ...
    if (annotation !== undefined) {
      ...
      lines.push(
        `- ${codeSpan(module)}: No single module replacement; follow the curated per-API guidance below.`,
      );
    } else {
...
export const unannotatedModuleIds = (
  ...
  const replacements = importMapReplacements(importMapSections);
  return removedModules(diff).filter(
    (module) =>
...
export const renderMissingAnnotations = (
  ...
  return `${[
    ...(missingModules.length === 0
      ? []
      : ["Removed modules:", ...missingModules.map((module) => `  - ${module}`)]),
    ...[...missingApis].flatMap(([module, ids]) => [`${module}:`, ...ids.map((id) => `  - ${id}`)]),
  ].join("\n")}\n`;
};
