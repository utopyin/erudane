  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const packagesIn = Effect.fnUntraced(function* (repoRoot: string) {
        const manifestPaths = yield* walk(path.join(repoRoot, "packages"), (location) =>
          location.endsWith(`${path.sep}package.json`),
        );
      ...
      const targetToDeclaration = (
        ...
        return declaration.endsWith(".d.ts")
          ? path.resolve(packageInfo.root, declaration)
          : undefined;
      };
      ...
      const expandPattern = Effect.fnUntraced(function* (
        ...
        for (const declarationFile of candidates) {
          ...
          const excluded = Object.entries(exportsMap).some(
            ([excludedKey, excludedTarget]) =>
      ...
      const discoverEntrypointsInternal = Effect.fnUntraced(function* (
        ...
        for (const packageInfo of packages) {
          ...
          for (const [keyPattern, rawTarget] of Object.entries(exportsMap)) {
            ...
            if (
              ...
              (requested === undefined ||
                requested.has(
                  keyPattern === "."
                    ? packageInfo.name
                    : `${packageInfo.name}${keyPattern.slice(1)}`,
                ))
            ) {
              const module =
            ...
            if (keyPattern.includes("*") && typeof target === "string" && target.includes("*")) {
              for (const entry of yield* expandPattern(
                packageInfo,
                keyPattern,
                target,
                requested,
        ...
        return {
          entrypoints: [...output.values()].sort((left, right) =>
