...
export class ApiDiff extends Context.Service<
  ApiDiff,
  {
...
>()("@effect/api-diff/ApiDiff") {
    ...
    Effect.gen(function* () {
      ...
      const runInternal = Effect.fnUntraced(function* (options: ApiDiffOptions) {
        ...
        const headSha = yield* options.headRef === "main"
          ? worktrees
              .resolveRef(repoRoot, "origin/main")
              .pipe(Effect.catch(() => worktrees.resolveRef(repoRoot, options.headRef)))
        ...
        if (options.output !== undefined) {
          ...
          yield* Console.log(
            `Wrote ${path.relative(repoRoot, output)} (${diff.changes.length} changes)`,
          );
        }

        if (options.writeDoc !== undefined || options.check) {
          const annotations = yield* loadAnnotations(
            path.join(repoRoot, "migration", "annotations"),
          ).pipe(
            Effect.provideService(FileSystem.FileSystem, fs),
            Effect.provideService(Path.Path, path),
          );
          const documentPath =
            options.writeDoc === undefined
              ? path.join(repoRoot, "migration", "v3-to-v4.md")
              : absolute(repoRoot, options.writeDoc);
          const existing = (yield* fs.exists(documentPath))
            ? yield* fs.readFileString(documentPath)
            : "";
          ...
          if (options.writeDoc !== undefined) {
            ...
            yield* Console.log(
              `Wrote ${path.relative(repoRoot, documentPath)} (${diff.changes.length} changes)`,
            );
          }
          if (options.check) {
            ...
            yield* Console.log(
              renderMissingAnnotations(diff, annotations, importMapSections).trimEnd(),
            );
  ...
  static readonly layer = this.layerNoDependencies.pipe(Layer.provide(Worktrees.layer));
}
