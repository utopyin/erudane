...
const parseAnnotation = (id: string, value: unknown, file: string): MigrationAnnotation => {
  if (
    ...
    (Reflect.get(value, "example") !== undefined &&
      typeof Reflect.get(value, "example") !== "string")
  ) {
  ...
  return {
    ...
    ...(Reflect.get(value, "example") === undefined
      ? {}
      : { example: Reflect.get(value, "example") }),
  };
};
  ...
  for (const file of files) {
    ...
    const document = yield* Effect.try({
      try: () => Yaml.parse(source) as unknown,
        ...
        new ApiDiffError({ message: `Could not parse annotation file ${file}`, cause }),
    });
    if (typeof document !== "object" || document === null || Array.isArray(document)) {
      return yield* new ApiDiffError({ message: `Annotation file ${file} must contain an object` });
    }
    for (const [id, value] of Object.entries(document).sort(([left], [right]) =>
