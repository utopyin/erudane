...
describe("canonical snapshot", () => {
  it.effect(
    "extracts declarations, overloads, namespaces, re-exports, and canonical types deterministically",
    () =>
      Effect.gen(function* () {
        ...
        const overloaded = first.entities.find(
          (entity) => entity.id === "@fixture/sample/Foo#overloaded#value",
        );
        ...
        assert.deepStrictEqual(
          client.map((entity) => entity.bucket),
          ["type", "value"],
        );
        assert(
          first.entities.some(
            (entity) => entity.path.join(".") === "merged.Options" && entity.bucket === "type",
          ),
        );
        assert(
          first.entities.some((entity) =>
            JSON.stringify(entity.declarations).includes('"kind":"conditional"'),
          ),
        );
        assert(
          first.entities.some((entity) =>
            JSON.stringify(entity.declarations).includes('"kind":"mapped"'),
          ),
        );
...
`,
      ...
      assert(
        failure.diagnostics.some(
          (diagnostic) => diagnostic.code === "unsupported-public-declaration",
        ),
      );
    }).pipe(Effect.provide(MainLayer)),
  );
  ...
  it.effect("normalizes union order in structural fingerprints", () =>
    Effect.gen(function* () {
      ...
      yield* writeFixturePackage(leftRoot, {
        "Choice.d.ts": "export type Choice = string | number\n",
      });
      yield* writeFixturePackage(rightRoot, {
        "Choice.d.ts": "export type Choice = number | string\n",
      });
