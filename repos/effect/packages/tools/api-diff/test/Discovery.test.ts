...
const MainLayer = Discovery.layer.pipe(Layer.provideMerge(NodeServices.layer));
  ...
  it.effect("expands wildcards and respects null exclusions", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        result.entrypoints.map((entrypoint) => entrypoint.module),
        ["@fixture/sample", "@fixture/sample/Foo"],
      );
      assert.deepStrictEqual(result.missing, ["@fixture/sample/internal/Secret"]);

      const all = yield* discovery.discoverEntrypoints(root);
      assert.deepStrictEqual(
        all.entrypoints.map((entrypoint) => entrypoint.module),
        ["@fixture/sample", "@fixture/sample/Foo", "@fixture/sample/index"],
      );
