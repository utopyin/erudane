...
describe("snapshot diff", () => {
  it("matches renames, classifies signature changes, and separates suggestions", () => {
    ...
    const head = snapshot("b", [
      ...
      entity(
        "old/A",
        "added",
        { kind: "variable", name: "added", type: { kind: "primitive", name: "string" } },
        "a",
      ),
    ]);
    ...
    assert(
      diff.changes.some(
        (change) => change.baseApiId?.includes("similarName") && !change.authoritative,
      ),
    );
    assert(
      diff.changes.some(
    ...
    assert(
      diff.changes.some(
        (change) =>
          change.classification === "api-added" && change.headApiId?.includes("similarNames"),
      ),
    );
  ...
  it("suggests replacements across modules and preserves class facets", () => {
    ...
    const contextServiceValue = entity(
      "effect/Context",
      "Service",
      variable("Service"),
      "service-value",
    ...
    assert.strictEqual(
      diff.changes.filter((change) => change.classification === "api-removed").length,
      3,
    );
    assert.strictEqual(
      diff.changes.filter((change) => change.classification === "api-added").length,
      3,
    );
  });
  ...
  it("classifies overload and parameter reordering", () => {
    const signature = (
      name: string,
      parameters: ReadonlyArray<"left" | "right">,
    ): DeclarationModel => ({
      kind: "function",
      name,
    ...
    const diff = diffSnapshots(snapshot("a", [before]), snapshot("b", [after]));
