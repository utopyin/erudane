...
const TestFileSystem = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const docgenJson = Option.getOrElse(
      Context.getOption(context, DocgenJsonTag),
      () => ({}) as DocgenJson,
    );
    ...
    const stat: FileSystem.FileSystem["stat"] = (filePath) =>
      path.basename(filePath) === "invalid-json.txt"
        ? Effect.succeed(fileInfo)
        : Effect.die("file not found");
    ...
    Effect.gen(function* () {
      const result = yield* Effect.exit(
        testCliFor(Effect.void)([]).pipe(
          Effect.provide(
            makeTestLayer().pipe(Layer.provide(makeDocgenJson({ projectHomepage: 1 } as any))),
          ),
        ),
      );
      ...
      for (const flag of ["--parse-compiler-options", "--examples-compiler-options"]) {
        for (const value of ["null", "[]", "1", '"text"']) {
          ...
          if (result.failure._tag === "ShowHelp") {
            assert.isTrue(
              result.failure.errors.some(
                (error) => error._tag === "InvalidValue" && error.expected.includes("JSON record"),
              ),
            );
          }
        }
