...
const sortModulesByPath: <A extends Domain.Module>(self: Iterable<A>) => Array<A> = Array.sort(
  Domain.ByPath,
);
...
const parseInterfaceDeclaration = (id: ast.InterfaceDeclaration) =>
  Effect.gen(function* () {
    ...
    return [new Domain.Interface(name, doc, signature, position)];
  });

const parseInterfaceDeclarations = (interfaces: ReadonlyArray<ast.InterfaceDeclaration>) => {
  const exportedInterfaces = Array.filter(interfaces, (id) => id.isExported());
 ...
 * @since 0.6.0
 */
export const parseInterfaces = Effect.flatMap(Source, (source) =>
  parseInterfaceDeclarations(source.sourceFile.getInterfaces()),
);
...
const parseType = (node: ast.Node) => {
  ...
  for (const property of node.getDescendantsOfKind(ast.ts.SyntaxKind.PropertySignature)) {
    ...
    const readonly = property.getFirstModifierByKind(ast.ts.SyntaxKind.ReadonlyKeyword)
      ? "readonly "
      : "";
...
const parseFunctionDeclaration = (fd: ast.FunctionDeclaration) =>
  Effect.gen(function* () {
    ...
    return [new Domain.Function(name ?? "", doc, signature, position)];
  });
...
const getFunctionDeclarations = Effect.gen(function* () {
  const source = yield* Source;
  const functions = Array.filter(source.sourceFile.getFunctions(), (fd) => fd.isExported());
  const arrows = pipe(
    Array.filter(source.sourceFile.getVariableDeclarations(), (vd) => {
...
export const parseFunctions = Effect.gen(function* () {
  ...
  const functionVariableDeclarations = yield* Effect.forEach(
    arrows,
...
const parseTypeAliasDeclaration = (ta: ast.TypeAliasDeclaration) =>
  Effect.gen(function* () {
    ...
    return [new Domain.TypeAlias(name, doc, signature, position)];
  });

const parseTypeAliasDeclarations = (typeAliases: ReadonlyArray<ast.TypeAliasDeclaration>) => {
  const exportedTypeAliases = Array.filter(typeAliases, (tad) => tad.isExported());
 ...
 * @since 0.6.0
 */
export const parseTypeAliases = Effect.flatMap(Source, (source) =>
  parseTypeAliasDeclarations(source.sourceFile.getTypeAliases()),
);
...
const parseConstantVariableDeclaration = (vd: ast.VariableDeclaration) =>
  Effect.gen(function* () {
    ...
    return [new Domain.Constant(name, doc, signature, position)];
  });
...
const parseExportSpecifier = (es: ast.ExportSpecifier) =>
  Effect.gen(function* () {
    ...
    return new Domain.Export(name, doc, signature, position, false);
  });
...
const parseModuleDeclaration = (
  ...
  return Effect.gen(function* () {
    ...
    const position = yield* parsePosition(ed);
    return [new Domain.Namespace(name, doc, position, interfaces, typeAliases, namespaces)];
  });
};
...
const parseMethod = (md: ast.MethodDeclaration) =>
  Effect.gen(function* () {
    ...
    return Option.some(new Domain.DocEntry(name, doc, signature, position));
  });
...
const parseProperty = (pd: ast.PropertyDeclaration) =>
  Effect.gen(function* () {
    ...
    return [new Domain.DocEntry(name, doc, signature, position)];
  });
    ...
    Effect.map((typeParameters) =>
      pipe(
        ...
        Array.matchLeft({
          ...
          onNonEmpty: (head) =>
            `declare class ${name}${typeParameters} { ${getConstructorDeclarationSignature(
              head,
            )} }`,
        }),
      ),
...
const parseClass = (c: ast.ClassDeclaration) =>
  Effect.gen(function* () {
    ...
    return [new Domain.Class(name, doc, signature, position, methods, staticMethods, properties)];
  });
...
export const parseFile =
  ...
  (
    file: Domain.File,
  ): Effect.Effect<Domain.Module, Array<string>, Configuration.Configuration | Path.Path> => {
    return Effect.gen(function* () {
...
export const parseFiles = (files: ReadonlyArray<Domain.File>) =>
  createProject(files).pipe(
    Effect.flatMap((project) =>
      pipe(files, Effect.validate(parseFile(project)), Effect.map(sortModulesByPath)),
    ),
  );
