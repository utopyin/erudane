...
export const ConfigurationSchema = Schema.Struct({
  ...
  parseCompilerOptions: Schema.optional(
    compilerOptionsSchema.annotate({
...
const readJsonFile = (path: string): Effect.Effect<unknown, never, FileSystem.FileSystem> =>
  Effect.gen(function* () {
...
/** @internal */
export const configProviderLayer = Layer.effect(ConfigProvider.ConfigProvider)(
  Effect.gen(function* () {
    // Extract the requisite services
    const process = yield* Domain.Process;
    const cwd = yield* process.cwd;
