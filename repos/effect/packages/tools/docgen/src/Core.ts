    ...
    Effect.mapError(
      () =>
        new Domain.DocgenError({
          message:
            `[Core.glob] Unable to execute glob pattern '${pattern}' ` +
...
const readSourceFiles = Effect.gen(function* () {
  ...
  return yield* Effect.forEach(
    paths,
    (path) =>
      Effect.map(fs.readFileString(path), (content) => new Domain.File(path, content, false)),
    { concurrency: "unbounded" },
  );
});
  ...
  Parser.parseFiles(files).pipe(
    Effect.mapError(
      (errors) =>
        new Domain.DocgenError({
          message:
            "[Core.parseModules] The following error(s) occurred while " +
...
const filterJoin = (self: Array<string>) =>
  pipe(self, Array.filter(String.isNonEmpty), Array.join("-"));
...
const extractPrefixedNestedNamespaces = (
  ...
  const namespaces = Array.flatMap(doc.namespaces, (namespace) =>
    extractPrefixedNestedNamespaces(namespace, newPrefix),
  );
 ...
 * @internal
 */
export const extractFencedCode = (
  content: string,
): [examples: Array<string>, warnings: Array<string>] => {
    ...
    const files = Array.flatMap(modules, (module) => {
      ...
      const getFiles =
        ...
        (namedDoc: {
          ...
          return Array.map(examples, (example, i) => {
      ...
      const classExamples = Array.flatMap(module.classes, (c) =>
        Array.flatten([
          getFiles("class")(c),
          Array.flatMap(c.methods, getFiles(`${c.name}-method`)),
          Array.flatMap(c.staticMethods, getFiles(`${c.name}-staticmethod`)),
        ]),
      );
      const allPrefixedInterfaces = [
        ...module.interfaces.map((iface) => ["" as string, iface] as const),
        ...Array.flatMap(allPrefixedNamespaces, ([prefix, namespace]) =>
          namespace.interfaces.map(
            (iface) => [filterJoin([prefix, namespace.name]), iface] as const,
          ),
        ),
      ];
      const interfacesExamples = Array.flatMap(allPrefixedInterfaces, ([ns, doc]) =>
      ...
      const allPrefixedTypeAliases = [
        ...
        ...Array.flatMap(allPrefixedNamespaces, ([prefix, namespace]) =>
          namespace.typeAliases.map(
            (typeAlias) => [filterJoin([prefix, namespace.name]), typeAlias] as const,
          ),
        ),
      ];
      const typeAliasesExamples = Array.flatMap(allPrefixedTypeAliases, ([ns, doc]) =>
        getFiles(filterJoin(["typealias", ns]))(doc),
      );
      const constantsExamples = Array.flatMap(module.constants, getFiles("constant"));
      const functionsExamples = Array.flatMap(module.functions, getFiles("function"));
      const exportsExamples = Array.flatMap(module.exports, getFiles("export"));
      const namespacesExamples = Array.flatMap(allPrefixedNamespaces, ([ns, doc]) =>
...
const getExamplesEntryPoint = (examples: ReadonlyArray<Domain.File>) =>
  Effect.gen(function* () {
    ...
    const content = examples
      .map((example) => `import './${path.basename(example.path, ".ts")}'`)
      .join("\n");
    return new Domain.File(
...
const getModuleMarkdownOutputPath = (module: Domain.Module) => {
  return Effect.gen(function* () {
    ...
    return path.normalize(
      path.join(config.outDir, "modules", `${module.path.slice(1).join(path.sep)}.md`),
    );
  });
};
...
const getModuleMarkdownFiles = (modules: ReadonlyArray<Domain.Module>) =>
  Effect.forEach(modules, (module, i) =>
    Effect.gen(function* () {
      ...
      const tocgen = yield* Effect.promise(() =>
        import("@effect/markdown-toc").then((module) => module.default),
      ).pipe(Effect.orDie);
