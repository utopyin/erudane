...
const printClass = (model: Domain.Class) => {
  return Effect.gen(function* () {
    ...
    const staticMethods = yield* Effect.forEach(model.staticMethods, (method) =>
      printStaticMethod(method),
    );
    const methods = yield* Effect.forEach(model.methods, (method) => printMethod(method));
    const properties = yield* Effect.forEach(model.properties, (property) =>
...
const printNamespace = (
  ...
  return Effect.gen(function* () {
    ...
    const interfaces = yield* Effect.forEach(model.interfaces, (inter) =>
      printInterface(inter, indentation + 1),
    );
...
export const printModule = (module: Domain.Module) => {
  return Effect.gen(function* () {
    ...
    const strings = yield* Effect.forEach(printables, ([category, printables]) =>
      Effect.gen(function* () {
        const out = `\n\n# ${category}`;
        const strings = yield* Effect.forEach(sortByName(printables), (printable) =>
          print(printable),
        );
