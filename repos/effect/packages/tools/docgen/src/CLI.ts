...
const theme = Flag.string("theme").pipe(
  Flag.withFallbackConfig(
    Config.string("theme").pipe(Config.withDefault(Configuration.DEFAULT_THEME)),
  ),
...
export const loadConfiguration = Effect.fnUntraced(function* (
  ...
  const configuredEnableSearch = yield* Config.boolean("enableSearch").pipe(
  ...
  const configuredEnforceVersion = yield* Config.boolean("enforceVersion").pipe(
    Effect.orElseSucceed(() => true),
  );
  const configuredRunExamples = yield* Config.boolean("runExamples").pipe(
    Effect.orElseSucceed(() => false),
  );
  return yield* Configuration.load({
    ...
    examplesCompilerOptions: Option.orElse(
      examplesCompilerOptionsFile,
      () => examplesCompilerOptionsInline,
    ),
  });
