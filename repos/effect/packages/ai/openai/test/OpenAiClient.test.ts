  ...
  describe("error mapping", () => {
    ...
    it.effect("maps OpenAI-compatible 402 errors to QuotaExhaustedError", () =>
      Effect.gen(function* () {
        const client = yield* OpenAiClient.OpenAiClient;
        const result = yield* client
          .createResponse({ model: "grok-4", input: "test" })
          .pipe(Effect.flip);
...
const makeResponse = (
  ...
  const contentType = response._tag === "Json" ? "application/json" : "text/event-stream";
  const body =
    response._tag === "Json"
  ...
  const stream = Stream.concat(Stream.succeed(new TextEncoder().encode(body)), Stream.never);
