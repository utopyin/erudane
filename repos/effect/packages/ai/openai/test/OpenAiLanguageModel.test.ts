      ...
      describe("tool messages", () => {
        ...
        it.effect("emits only the specialized output for apply_patch results", () =>
          Effect.gen(function* () {
            ...
            const outputs = body.input.filter(
              (item: any) => item.call_id === "call_apply_patch" && item.type.endsWith("_output"),
            );

            deepStrictEqual(
              outputs.map((item: any) => item.type),
        ...
        it.effect("emits only the specialized output for shell results", () =>
          Effect.gen(function* () {
            ...
            const body = yield* getRequestBody(requests[0]);
            const outputs = body.input.filter(
              (item: any) => item.call_id === "call_shell" && item.type.endsWith("_output"),
            );
        ...
        it.effect("emits only the specialized output for local_shell results", () =>
          Effect.gen(function* () {
            ...
            const body = yield* getRequestBody(requests[0]);
            const outputs = body.input.filter(
              (item: any) => item.call_id === "call_local_shell" && item.type.endsWith("_output"),
            );
