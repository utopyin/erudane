...
describe("Iterable", () => {
  it("groupBy", () => {
    ...
    const bySign = Iterable.groupBy([1, 2, 3], (n) =>
      n > 0 ? ("positive" as const) : ("negative" as const),
    );
    expect(bySign).type.toBe<
      Partial<Record<"positive" | "negative", Array.NonEmptyArray<number>>>
    >();
    expect(
      pipe(
        [1, 2, 3],
    ...
    expect(Iterable.groupBy(["a", "b"], Symbol.for)).type.toBe<
      Record<symbol, Array.NonEmptyArray<string>>
    >();
    expect(
      Iterable.groupBy(["a", "b"], (s) => (s === "a" ? symA : s === "b" ? symB : symC)),
    ).type.toBe<
