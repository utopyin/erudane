...
describe("Record", () => {
  describe("ReadonlyRecord.GroupByResult", () => {
    it("returns a record for open key types", () => {
      expect<EffectRecord.ReadonlyRecord.GroupByResult<string, number>>().type.toBe<
        Record<string, number>
      >();
      expect<EffectRecord.ReadonlyRecord.GroupByResult<symbol, number>>().type.toBe<
      ...
      >();
      expect<
        EffectRecord.ReadonlyRecord.GroupByResult<`operation-${string}` | symbol, number>
      >().type.toBe<Record<`operation-${string}` | symbol, number>>();
    });

    it("returns a partial record for finite string key types", () => {
      expect<EffectRecord.ReadonlyRecord.GroupByResult<"", number>>().type.toBe<
        Partial<Record<"", number>>
      >();
      expect<EffectRecord.ReadonlyRecord.GroupByResult<"0", number>>().type.toBe<
    ...
    it("returns a partial record for unique symbol key types", () => {
      ...
      >();
      expect<
        EffectRecord.ReadonlyRecord.GroupByResult<typeof symbolA | typeof symbolB, number>
      >().type.toBe<Partial<Record<typeof symbolA | typeof symbolB, number>>>();
    });
    ...
    it("returns a partial record when an open key type is mixed with a finite key type", () => {
      ...
      >();
      expect<
        EffectRecord.ReadonlyRecord.GroupByResult<string | typeof symbolA, number>
      >().type.toBe<Partial<Record<string | typeof symbolA, number>>>();
      expect<
        EffectRecord.ReadonlyRecord.GroupByResult<"create" | typeof symbolA, number>
      >().type.toBe<Partial<Record<"create" | typeof symbolA, number>>>();
    });

    it("returns an empty record when there are no keys", () => {
      expect<EffectRecord.ReadonlyRecord.GroupByResult<never, number>>().type.toBe<
        Record<never, number>
      >();
    });
