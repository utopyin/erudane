...
describe("Optic", () => {
  describe("standalone functions", () => {
    ...
    it("supports data-first usage", () => {
      ...
      expect(Optic.getResult({ value: 1 }, value)).type.toBe<
        Result.Result<number, SchemaIssue.Issue>
      >();
      expect(Optic.set({ _tag: "Circle", radius: 1 }, circle)).type.toBe<Shape>();
      expect(Optic.replace(user, profile, { city: "Rome" })).type.toBe<User>();
      expect(Optic.replaceResult({ value: 1 }, value, 2)).type.toBe<
        Result.Result<Record<string, number>, SchemaIssue.Issue>
      >();
      ...
      expect(Optic.modifyAll([1, 2, 3], numbers, (value) => value + 1)).type.toBe<
    ...
    it("supports data-last usage", () => {
      ...
      expect(Optic.modifyAll(numbers, (value) => value + 1)([1, 2, 3])).type.toBe<
        ReadonlyArray<number>
      >();
    });
    ...
    it("supports pipe usage", () => {
      ...
      expect(pipe({ value: 1 }, Optic.getResult(value))).type.toBe<
        Result.Result<number, SchemaIssue.Issue>
      >();
      ...
      expect(pipe({ value: 1 }, Optic.replaceResult(value, 2))).type.toBe<
        Result.Result<Record<string, number>, SchemaIssue.Issue>
      >();
      expect(
        pipe(
          user,
          Optic.modify(city, (value) => value.toUpperCase()),
        ),
      ).type.toBe<User>();
      ...
      expect(
        pipe(
          [1, 2, 3],
          Optic.modifyAll(numbers, (value) => value + 1),
        ),
  ...
  describe("compose", () => {
    const iso = Optic.makeIso<number, number>(
      (n) => n,
      (n) => n,
    );
    const lens = Optic.makeLens<number, number>(
      (n) => n,
      (n) => n,
    );
    ...
    it("uses structured issues for failures", () => {
      expect(prism.getResult).type.toBe<
        (source: number) => Result.Result<number, SchemaIssue.Issue>
      >();
      expect(optional.getResult).type.toBe<
        (source: number) => Result.Result<number, SchemaIssue.Issue>
      >();
      expect(optional.replaceResult).type.toBe<
        (value: number, source: number) => Result.Result<number, SchemaIssue.Issue>
      >();
