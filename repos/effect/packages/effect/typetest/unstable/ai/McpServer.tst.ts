...
describe("McpServer", () => {
  describe("protocol configuration", () => {
    ...
    it("should expose each historical protocol through the structural adapter interface", () => {
      expect(McpProtocol.v2024_11_05).type.toBeAssignableTo<
        McpProtocol.ProtocolAdapter<"2024-11-05">
      >();
      expect(McpProtocol.v2025_03_26).type.toBeAssignableTo<
        McpProtocol.ProtocolAdapter<"2025-03-26">
      >();
      expect(McpProtocol.v2025_06_18).type.toBeAssignableTo<
        McpProtocol.ProtocolAdapter<"2025-06-18">
      >();
      expect(McpProtocol.v2025_11_25).type.toBeAssignableTo<
        McpProtocol.ProtocolAdapter<"2025-11-25">
      >();
      ...
      expect(protocols[0].clientRpcs.requests).type.toBeAssignableTo<
        ReadonlyMap<string, Rpc.Any>
      >();
    });

    it("should preserve the decoded tool JSON Schema shape", () => {
      expect<
        Schema.Schema.Type<typeof McpSchema.ToolJsonSchema>
      >().type.toBe<McpSchema.ToolJsonSchema>();
    });
  ...
  describe("request context", () => {
    ...
    it("should expose initialization data and the generated reverse RPC client", () => {
      expect(
        McpSchema.McpServerClient.useSync((client) => client.initializePayload.capabilities),
      ).type.toBe<Effect.Effect<McpSchema.ClientCapabilities, never, McpSchema.McpServerClient>>();
      expect(
        McpSchema.McpServerClient.useSync((client) => client.initializePayload.clientInfo),
      ).type.toBe<Effect.Effect<McpSchema.Implementation, never, McpSchema.McpServerClient>>();
