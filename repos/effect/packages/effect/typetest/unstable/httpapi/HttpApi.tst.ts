...
describe("HttpApi", () => {
  describe("groups", () => {
    ...
    it("keeps groups distinct when endpoint identifiers overlap", () => {
      ...
      expect(Prefixed.groups.users.endpoints.find).type.toBe<
        HttpApiEndpoint.AddPrefix<typeof FindUser, "/v1">
      >();
      expect(Prefixed.groups.admins.endpoints.find).type.toBe<
        HttpApiEndpoint.AddPrefix<typeof FindAdmin, "/v1">
      >();
    });
  });
});
