...
describe("HttpApiGroup", () => {
  ...
  describe("endpoints", () => {
    ...
    it("preserves endpoint types by identifier after group transformations", () => {
      ...
      expect(Prefixed.endpoints.getUser).type.toBe<
        HttpApiEndpoint.AddPrefix<typeof GetUser, "/v1">
      >();
      expect(Prefixed.endpoints.createUser).type.toBe<
        HttpApiEndpoint.AddPrefix<typeof CreateUser, "/v1">
      >();
      expect(WithMiddleware.endpoints.getUser).type.toBe<
        HttpApiEndpoint.AddMiddleware<typeof GetUser, M>
      >();
      expect(WithMiddleware.endpoints.createUser).type.toBe<
        HttpApiEndpoint.AddMiddleware<typeof CreateUser, M>
      >();
    });
  ...
  describe("Endpoints", () => {
    it("extracts the endpoint union from one group", () => {
      ...
      expect<HttpApiGroup.Endpoints<typeof Group>>().type.toBe<
        typeof GetUser | typeof CreateUser
      >();
    });
