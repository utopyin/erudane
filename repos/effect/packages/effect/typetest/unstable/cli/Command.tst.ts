...
describe("Command", () => {
  describe("withSubcommands", () => {
    it("unions errors and requirements across multiple subcommands", () => {
      ...
      const childA = Command.make(
        "child-a",
        {},
        () => Effect.void as Effect.Effect<void, "err-a", _ServiceA>,
      );
      const childB = Command.make(
        "child-b",
        {},
        () => Effect.void as Effect.Effect<void, "err-b", _ServiceB>,
      );
      const childC = Command.make(
        "child-c",
        {},
        () => Effect.void as Effect.Effect<void, "err-c", _ServiceC>,
      );

      const root = Command.make("root").pipe(Command.withSubcommands([childA, childB, childC]));
