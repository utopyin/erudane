...
describe("HttpRouter", () => {
  describe("middleware", () => {
    it("provides handled request errors", () => {
      ...
      expect<Layer.Success<typeof middleware.layer>>().type.toBeAssignableFrom<
        HttpRouter.Request<"Error", MyError>
      >();
    });
  });
  ...
  describe("toHttpEffect", () => {
    it("includes errors from global middleware", () => {
      ...
      expect<Effect.Error<Effect.Success<typeof result>>>().type.toBe<
        MyError | HttpServerError.HttpServerError
      >();
    });
  });
