...
describe("Match", () => {
  it("fn infers the selected value and original arguments", () => {
    ...
    expect(filterTodos).type.toBe<(todos: Todos, filter: Filter) => Todos>();
    ...
    expect(withoutExtraParameters).type.toBe<
      (todos: Todos, filter: Filter) => "all" | "filtered"
    >();
  ...
  it("fn threads arguments through handlers and terminals", () => {
    ...
    const whenAnd = Match.fn(
      (_context: boolean, value: { readonly kind: "a"; readonly active: boolean }) => value,
    ).pipe(
      Match.whenAnd({ kind: "a" }, { active: true }, (_value, context) => context),
      Match.orElse((_value, context) => context),
    );
    expect(whenAnd).type.toBe<
      (context: boolean, value: { readonly kind: "a"; readonly active: boolean }) => boolean
    >();
  ...
  it("value matchers resolve terminals for generic inputs", () => {
    ...
    expect(option<{ readonly _tag: "Open" }>({ _tag: "Closed" })).type.toBe<
      Option.Option<{ readonly _tag: "Open" }>
    >();
    ...
    const freePipe = <Open extends { readonly _tag: "Open" }>(state: Closed | Open): string =>
      pipe(
        Match.value(state),
        Match.tag("Closed", () => "closed"),
        Match.orElse(() => "open"),
  ...
  it("exhaustiveness checking is preserved", () => {
    const incomplete = Match.value(stringOrNumber).pipe(Match.when(Match.number, (n) => n));
