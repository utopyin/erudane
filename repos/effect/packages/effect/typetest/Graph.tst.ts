...
describe("Graph", () => {
  ...
  it("path edge indexes", () => {
    ...
    expect(allPairs.edges).type.toBe<
      Map<Graph.NodeIndex, Map<Graph.NodeIndex, Array<Graph.EdgeIndex>>>
    >();
  });
  ...
  it("minimumSpanningForest", () => {
    ...
    expect(
      pipe(
        mutableUndirected,
        Graph.minimumSpanningForest((edge) => edge),
      ),
    ).type.toBe<Graph.UndirectedGraph<string, number>>();
  ...
  it("lazy path enumeration", () => {
    ...
    >();
    expect(
      Graph.allShortestPaths(directed, { source: 0, target: 1, cost: (edge) => edge }),
