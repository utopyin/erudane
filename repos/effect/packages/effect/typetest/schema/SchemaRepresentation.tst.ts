...
describe("SchemaRepresentation persisted wire", () => {
  ...
  it("uses native literal values", () => {
    ...
    expect({
      ...
      literal: "value",
      checks: [],
    } as const).type.toBeAssignableTo<SchemaRepresentation.Literal>();

    expect({
      _tag: "Literal",
      literal: { type: "string", value: "value" },
      checks: [],
    } as const).type.not.toBeAssignableTo<SchemaRepresentation.Literal>();
  });
  ...
  it("uses native property keys", () => {
    ...
    expect(Symbol.for("key")).type.toBeAssignableTo<
