...
describe("Schema.Graph", () => {
  it("infers directed and undirected graph kinds", () => {
    ...
    >();
    expect<
      Schema.Schema.Type<Schema.Graph<"undirected", Schema.String, Schema.Number>>
    >().type.toBe<Graph.UndirectedGraph<string, number>>();
    expect<Schema.Schema.Type<typeof generic>>().type.toBe<
      Graph.Graph<string, number, Graph.Kind>
    >();
  });
