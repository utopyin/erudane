...
async function assertMissing(provider: ConfigProvider.ConfigProvider, path: ConfigProvider.Path) {
  const r = Effect.result(provider.load(path));
...
describe("ConfigProvider", () => {
  describe("make", () => {
    it.effect("exposes lookup absence and input transformation as provider behavior", () =>
      Effect.gen(function* () {
        ...
        deepStrictEqual(yield* nested.load(["PORT"]), ConfigProvider.makeValue("3000"));
    ...
    it("creates a provider from a lookup function", async () => {
      const provider = ConfigProvider.make((path) =>
        Effect.succeed(path.join(".") === "A.B" ? ConfigProvider.makeValue("value") : undefined),
      );
  ...
  describe("fromEnvRecord", () => {
    ...
    it("preserves empty strings when requested", async () => {
      const provider = ConfigProvider.fromEnvRecord({ EMPTY: "" }, { preserveEmptyStrings: true });
...
DB_PASS=$PASSWORD`),
  ...
  describe("fromDir", () => {
    ...
    it("treats empty files as missing by default", async () => {
      const Fs = FileSystem.layerNoop({
        readFileString(path) {
          return path === "/empty" ? Effect.succeed("") : Effect.fail(notFound("readFileString"));
        },
        readDirectory(path) {
          return path === "/" ? Effect.succeed(["empty"]) : Effect.fail(notFound("readDirectory"));
        },
      });
      const provider = await Effect.runPromise(
    ...
    it("reads directory entries as a record", async () => {
      ...
      const Fs = FileSystem.layerNoop({
        ...
        readDirectory(path) {
          ...
          return entries ? Effect.succeed([...entries]) : Effect.fail(notFound("readDirectory"));
        },
      });
      const provider = await Effect.runPromise(
        ConfigProvider.fromDir({ rootPath: "/" }).pipe(
  ...
  describe("layers", () => {
    ...
    it("layer accepts an Effect that produces a provider", async () => {
      const result = await Effect.runPromise(
        loadFromContext(["KEY"]).pipe(
          Effect.provide(
            ConfigProvider.layer(Effect.succeed(ConfigProvider.fromUnknown({ KEY: "value" }))),
          ),
        ),
      );
