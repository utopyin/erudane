...
describe("Stream", () => {
  ...
  describe("destructors", () => {
    ...
    it.effect("mkArrayBuffer - concatenates Uint8Array chunks", () =>
      Effect.gen(function* () {
        const buffer = yield* Stream.make(new Uint8Array([1, 2]), new Uint8Array([3, 4])).pipe(
    ...
    it.effect("toAsyncIterable - return interrupts an in-flight pull", () =>
      Effect.gen(function* () {
        ...
        const iterator = Stream.toAsyncIterable(
          Stream.fromEffect(
            Effect.andThen(Deferred.succeed(started, void 0), Effect.never).pipe(
              Effect.onInterrupt(() =>
                Effect.sync(() => {
                  interrupted = true;
                }),
      ...
      Effect.gen(function* () {
        ...
        const iterable = Stream.toAsyncIterable(
          Stream.callback<number>((queue) =>
            Effect.andThen(Queue.offer(queue, 1), Effect.never).pipe(
              Effect.onInterrupt(() =>
                Effect.sync(() => {
                  interrupted = true;
                }),
    ...
    it.effect("toAsyncIterable - natural completion closes the scope with a successful exit", () =>
      Effect.gen(function* () {
        ...
        const stream = withScopeFinalizer(Stream.make(1, 2), (exit) =>
          Effect.sync(() => {
    ...
    it.effect("toAsyncIterable - stream failure is forwarded to the scope", () =>
      Effect.gen(function* () {
        ...
        const stream = withScopeFinalizer(Stream.fail(streamError), (exit) =>
          Effect.andThen(
            Effect.sync(() => {
              finalizerExits.push(exit);
            }),
    ...
    it.effect("toAsyncIterable - throw forwards a failure exit and preserves its error", () =>
      Effect.gen(function* () {
        ...
        const stream = withScopeFinalizer(Stream.make(1), (exit) =>
          Effect.andThen(
            Effect.sync(() => {
              finalizerExits.push(exit);
            }),
  ...
  describe("constructors", () => {
    ...
    it.effect("range - zero chunk size does not change the emitted range", () =>
      Effect.gen(function* () {
        const result = yield* Stream.range(1, 3, 0).pipe(Stream.take(4), Stream.runCollect);
  ...
  const testOuterFailure = (combinator: "flatMap" | "switchMap", inner: "never" | "slow") =>
    Effect.gen(function* () {
      ...
      const makeInner = () => {
        started.openUnsafe();
        return (
          inner === "never" ? Stream.never : Stream.fromEffect(Effect.sleep(Duration.hours(1)))
      ...
      const fiber = yield* stream.pipe(Stream.runDrain, Effect.forkChild);
  ...
  describe("flatMap", () => {
    ...
    it.effect("releases a permit after a successful saturated pull", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Stream.flatMap(
          outer,
          (n) =>
            n === 3 ? Stream.make(3) : Stream.fromEffect(Deferred.await(gate).pipe(Effect.as(n))),
          { concurrency: 2 },
        ).pipe(Stream.runCollect);
    ...
    it.effect("fails promptly and interrupts slow inner streams when the outer stream fails", () =>
      testOuterFailure("flatMap", "slow"),
    );
  });

  describe("switchMap", () => {
    it.effect("interrupts a never-ending inner stream when the outer stream fails", () =>
      testOuterFailure("switchMap", "never"),
    );

    it.effect("fails promptly and interrupts a slow inner stream when the outer stream fails", () =>
