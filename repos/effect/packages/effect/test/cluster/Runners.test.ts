...
describe.concurrent("Runners.makeRpc", () => {
  ...
  it.effect("uses the client protocol codec for simulated remote serialization", () => {
    ...
    }).pipe(Effect.provide(layerRunners(layerFakeProtocol(() => Effect.void, codecFor))));
  });
  ...
  it.effect("a server-delivered defect resolves the request instead of RunnerUnavailable", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(
        Exit.isSuccess(exit),
        "send must not fail with RunnerUnavailable for a delivered defect",
      );
      assert.strictEqual(replies.length, 1);
  ...
  it.effect("persisted notification transport failures are ignored", () =>
    Effect.gen(function* () {
      const runners = yield* Runners.Runners;
      const snowflakeGen = yield* Snowflake.Generator;
      const message = makeOutgoingRequest(
        TestRpcPersisted,
        snowflakeGen.nextUnsafe(),
        () => Effect.void,
      );
  ...
  it.effect(
    "a delivered defect for a persisted request maps to RunnerUnavailable for storage recovery",
    () =>
      Effect.gen(function* () {
        const runners = yield* Runners.Runners;
        const snowflakeGen = yield* Snowflake.Generator;
  ...
  it.effect(
    "encodes the entity payload with the protocol codec and leaves the hole untouched",
    () => {
      const sent: Array<FromClientEncoded> = [];
      const protocol = layerFakeProtocol((request, write) => {
        sent.push(request);
