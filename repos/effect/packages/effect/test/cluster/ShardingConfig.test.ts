...
describe("ShardingConfig", () => {
    ...
    Effect.gen(function* () {
      ...
      const fromEnv = yield* ShardingConfig.config.parse(
        ConfigProvider.fromUnknown({
          maxResidentEntities: "500",
          unprocessedMessageBatchSize: "128",
        }),
      );
      assert.strictEqual(fromEnv.maxResidentEntities, 500);
      assert.strictEqual(fromEnv.unprocessedMessageBatchSize, 128);

      const invalid = yield* ShardingConfig.config
        .parse(ConfigProvider.fromUnknown({ maxResidentEntities: "unbounded" }))
        .pipe(Effect.flip);
  ...
  it.effect("treats the optional listen address as an atomic group", () =>
    Effect.gen(function* () {
      ...
      const missingHost = yield* ShardingConfig.config
        .parse(ConfigProvider.fromUnknown({ listenPort: "8080" }))
        .pipe(Effect.flip);
      assert.strictEqual(
  ...
  at ["listenHost"]`,
      ...
      const invalidPort = yield* ShardingConfig.config
        .parse(ConfigProvider.fromUnknown({ listenHost: "0.0.0.0", listenPort: "invalid" }))
        .pipe(Effect.flip);
      assert.strictEqual(
  ...
  it.effect("rejects non-positive residency and batch limits", () =>
    Effect.gen(function* () {
      for (const values of [
        ...
        const error = yield* ShardingConfig.config
