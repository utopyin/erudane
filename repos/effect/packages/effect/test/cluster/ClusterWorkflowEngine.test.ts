...
describe.concurrent("ClusterWorkflowEngine", () => {
  ...
  it.effect(
    "DurableDeferred.raceAll lets a deferred win while another branch is active",
    () =>
      Effect.gen(function* () {
        const sharding = yield* Sharding.Sharding;
  ...
  it.effect(
    "DurableDeferred.raceAll wakes the active run by replaying it",
    () =>
      Effect.gen(function* () {
        const flags = yield* Flags;
        const sharding = yield* Sharding.Sharding;
  ...
  it.effect(
    "DurableDeferred.raceAll wakes a branch wrapped in DurableDeferred.into",
    () =>
      Effect.gen(function* () {
        const sharding = yield* Sharding.Sharding;
  ...
  it.effect(
    "DurableDeferred.raceAll runs a branch's transformations on a deferred wake",
    () =>
      Effect.gen(function* () {
        const sharding = yield* Sharding.Sharding;
  ...
  it.effect(
    "DurableDeferred.raceAll re-runs a multi-await branch once per completion",
    () =>
      Effect.gen(function* () {
        const flags = yield* Flags;
        const sharding = yield* Sharding.Sharding;
  ...
  it.effect("propagates trace context to persisted workflow requests", () => {
    ...
    }).pipe(Effect.provideService(Tracer.Tracer, tracer), Effect.provide(TestWorkflowEngine));
  });
...
const LosingDeferredWorkflowLayer = LosingDeferredWorkflow.toLayer(
  Effect.fnUntraced(function* () {
    const flags = yield* Flags;
...
const InPlaceWakeWorkflowLayer = InPlaceWakeWorkflow.toLayer(
  Effect.fnUntraced(function* () {
    const flags = yield* Flags;
...
const SlowUnwindWorkflowLayer = SlowUnwindWorkflow.toLayer(
  Effect.fnUntraced(function* () {
    const flags = yield* Flags;
    ...
    return yield* DurableDeferred.raceAll({
      ...
      success: Schema.String,
      error: Schema.Never,
      effects: [DurableDeferred.await(SlowUnwindGateA), DurableDeferred.await(SlowUnwindGateB)],
    }).pipe(
...
const TwoGateWorkflowLayer = TwoGateWorkflow.toLayer(() =>
  DurableDeferred.raceAll({
    ...
    success: Schema.String,
    error: Schema.Never,
    effects: [DurableDeferred.await(TwoGateA), DurableDeferred.await(TwoGateB)],
  }),
);
