...
describe.concurrent("Sharding", () => {
  ...
  it.live("defects instead of hanging when persisted failures contain non-JSON Error values", () =>
    Effect.gen(function* () {
      ...
      const cause = yield* client
        .Fail()
        .pipe(Effect.timeout("2 seconds"), Effect.sandbox, Effect.flip);
      ...
      it.live(
        ...
        () =>
          Effect.gen(function* () {
            ...
            const SenderLayer = Sender.toLayer(
              Effect.gen(function* () {
            ...
            assert(
              ...
              "Sharding scope-close hung during shutdown (deadlock)",
            );
            assert(
              Exit.isSuccess(yield* Deferred.await(discardExit)),
              "discard send failed during shutdown",
            );
    ...
    it.live(
      ...
      () =>
        Effect.gen(function* () {
          ...
          assert(
            Option.isSome(completed),
            "Sharding scope-close hung during shutdown (stranded caller)",
          );
  ...
  it.live(
    "fails a stream when its chunk acknowledgement is abandoned during shutdown",
    () =>
      Effect.gen(function* () {
        ...
        const Receiver = Entity.make("ShutdownStreamReceiver", [
          Rpc.make("Values", { success: Schema.Number, stream: true }).annotate(
        ...
        const ReceiverLayer = Receiver.toLayer({
          Values: () =>
            Stream.fromQueue(chunks).pipe(Stream.onStart(Deferred.succeed(streamStarted, void 0))),
        });
        ...
        const SenderLayer = Sender.toLayer(
          Effect.gen(function* () {
            const receiver = yield* Receiver.client;
            const streamFiber = yield* receiver("peer")
              .Values()
              .pipe(
                Stream.runDrain,
                Effect.uninterruptible,
        ...
        assert(
          Option.isSome(completed),
          "Sharding scope-close hung after abandoning a stream chunk acknowledgement",
        );
        ...
        assert(failure.value instanceof ClusterError.EntityNotAssignedToRunner);
        assert.strictEqual(
          driver.journal.filter((envelope) => envelope._tag === "AckChunk").length,
          1,
        );
      }),
    20_000,
  ...
  it.effect("defects durable messages when no entity ever registers", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* TestClock.adjust(1);
      }).pipe(Effect.provide(registeredEnv), Effect.scoped);
      ...
      assert.isTrue(
        warnings.some(
          (message) =>
            globalThis.Array.isArray(message) &&
            message.includes("Could not find entity manager for address, retrying"),
        ),
      );
      assert.strictEqual(driver.replyIds.size, 1);
      assert.strictEqual(driver.unprocessed.size, 0);
  ...
  it.effect("client volatile discard retries a failed delivery", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const client = makeClient("1");
        const fiber = yield* client
          .NeverVolatile(void 0, { discard: true })
          .pipe(Effect.forkChild({ startImmediately: true }));
...
describe.concurrent("Sharding residency cap", () => {
  ...
  it.effect("keeps delivering to resident entities at the cap", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* makeClient("1")
        .GetUser({ id: 1 })
        .pipe(Effect.forkChild({ startImmediately: true }));
      yield* TestClock.adjust(5000);
  ...
  it.effect("advances past in-flight memory requests when reading bounded batches", () =>
    Effect.gen(function* () {
      ...
      yield* makeClient("2")
        .Never()
        .pipe(Effect.forkChild({ startImmediately: true }));
      const fiber = yield* makeClient("3")
        .GetUser({ id: 3 })
        .pipe(Effect.forkChild({ startImmediately: true }));
    ...
    Effect.forEach(
      ...
      (unprocessedMessageBatchSize) =>
        Effect.gen(function* () {
          ...
          assert.deepStrictEqual(yield* Fiber.join(fiber), new User({ id: 1, name: "User 1" }));
        }).pipe(Effect.provide(CappedSharding({ unprocessedMessageBatchSize }))),
      { discard: true },
    ),
  );
  ...
  it.effect("does not busy-poll storage at the cap", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        Effect.provide(
          CappedSharding({ maxResidentEntities: 1 }, (storage) => ({
            ...storage,
            unprocessedMessages(shardIds, options) {
              reads++;
              return storage.unprocessedMessages(shardIds, options);
  ...
  it.effect("interrupts entities and reacquires shards after lock storage recovers", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const entityFiber = yield* client
          .NeverVolatile()
          .pipe(Effect.forkChild({ startImmediately: true }));
        yield* TestClock.adjust(1);
        ...
        const failedRefreshes = storageState.refreshCalls.filter(
          (call) => call.at >= partitionedAt && call.shards.length > 0,
        );
        ...
        assert(
          storageState.refreshCalls.some(
            (call) => call.at >= partitionedAt && call.shards.length === 0,
          ),
        );
        assert(
        ...
        assert.deepStrictEqual(
          yield* client.GetUserVolatile({ id: 2 }),
          new User({ id: 2, name: "User 2" }),
        );
...
const makeFailoverStorage = (state: FailoverStorageState, clock: Clock.Clock) =>
  RunnerStorage.RunnerStorage.of({
    getRunners: Effect.sync(() => {
      ...
      return state.otherRunnerHealthy
        ? [
            [state.runner, true],
            [otherRunner, true],
          ]
...
const CappedSharding = (
  ...
  return layer.pipe(Layer.provideMerge(MessageStorage.layerMemory), Layer.provide(configLayer));
};
