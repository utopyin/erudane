...
describe("Latch", () => {
  ...
  it.effect("open then close does not resume waiters registered after close", () =>
    Effect.gen(function* () {
      ...
      const before = yield* Effect.forkChild(Latch.await(latch), { startImmediately: true });

      yield* latch.open;
      yield* latch.close;
      const after = yield* Effect.forkChild(Latch.await(latch), { startImmediately: true });
  ...
  it.effect("release while a flush is pending covers the new waiters", () =>
    Effect.gen(function* () {
      ...
      const first = yield* Effect.forkChild(Latch.await(latch), { startImmediately: true });
      yield* latch.release.pipe(Effect.provideService(Scheduler.Scheduler, scheduler));
      assert.lengthOf(tasks, 1);

      const second = yield* Effect.forkChild(Latch.await(latch), { startImmediately: true });
      yield* latch.release.pipe(Effect.provideService(Scheduler.Scheduler, scheduler));
      assert.lengthOf(tasks, 1);
  ...
  it.effect("release does not resume waiters registered after the release", () =>
    Effect.gen(function* () {
      const latch = yield* Latch.make(false);
      const covered = yield* Effect.forkChild(Latch.await(latch), { startImmediately: true });

      yield* latch.release;
      const late = yield* Effect.forkChild(Latch.await(latch), { startImmediately: true });
