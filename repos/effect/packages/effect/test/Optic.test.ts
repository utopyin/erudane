...
describe("Optic", () => {
  ...
  describe("compose", () => {
    ...
    it("sets through composed prisms without reading a source", () => {
      ...
      const text = Optic.makePrism<number, string>((n) => Result.succeed(String(n)), Number);
      const optic = value.compose(text);
    ...
    it("preserves capabilities and behavior across every optic kind", () => {
      const optics = {
        Iso: Optic.makeIso<number, number>(
          (n) => n,
          (n) => n,
        ),
        Lens: Optic.makeLens<number, number>(
          (n) => n,
          (n) => n,
        ),
    ...
    it("preserves optional setter failures", () => {
      ...
      const optic = Optic.makeOptional<number, number>(Result.succeed, () =>
        Result.fail(issue),
      ).compose(
        Optic.makeIso<number, number>(
          (n) => n,
          (n) => n,
        ),
      );
...
Expected a value greater than 0`,
    ...
    it("combines checks across successive calls", () => {
      const optic = Optic.id<number>().check(Schema.isInt()).check(Schema.isGreaterThan(0));
...
Expected a value greater than 0`,
  ...
  describe("forEach", () => {
    ...
    it("fails when an inner setter fails", () => {
      ...
      const optic = Optic.id<ReadonlyArray<number>>().forEach(() =>
        Optic.makeOptional<number, number>(Result.succeed, () => Result.fail(issue)),
      );
  ...
  describe("modifyAll", () => {
    ...
    it("returns the original source when the traversal fails", () => {
      ...
      const optic: Optic.Traversal<S, number> = Optic.makeOptional(
        ...
        (values, s) =>
          s.values === undefined ? Result.fail(issue) : Result.succeed({ ...s, values }),
      );
      const source: S = {};
  ...
  it("getAll returns an empty array when the traversal fails", () => {
    ...
    const optic: Optic.Traversal<number, number> = Optic.makeOptional<
      number,
