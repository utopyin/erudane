...
it("should sanitize the url - query", () => {
  ...
  assert.deepStrictEqual(router.find("GET", "/test?hello=world")?.searchParams, { hello: "world" });
});
...
it("handles path and query separated by ;", () => {
  const router = Router.make<boolean>();
  router.on("GET", "/test", true);
  assert.deepStrictEqual(router.find("GET", "/test;jsessionid=123456")?.searchParams, {
    jsessionid: "123456",
  });
});
