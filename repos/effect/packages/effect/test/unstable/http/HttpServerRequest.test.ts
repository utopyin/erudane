...
describe("HttpServerRequest", () => {
  it.effect("preserves FormData through client-server-client conversion", () =>
    Effect.gen(function* () {
      ...
      const roundTrip = HttpServerRequest.toClientRequest(
        HttpServerRequest.fromClientRequest(clientRequest),
      );
      ...
      deepStrictEqual(
        {
          multipartMime:
            webRequest.headers.get("content-type")?.startsWith("multipart/form-data; boundary=") ??
            false,
  ...
  it.effect("reads raw BodyInit bytes after conversion from a client request", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        new Uint8Array(yield* server.arrayBuffer),
  ...
  it.effect("streams a raw URLSearchParams after conversion from a client request", () =>
    Effect.gen(function* () {
      ...
      const server = HttpServerRequest.fromClientRequest(client);
      assert.strictEqual(
        yield* server.stream.pipe(Stream.decodeText(), Stream.mkString),
        "a=1&b=two",
      );
    }),
  );
