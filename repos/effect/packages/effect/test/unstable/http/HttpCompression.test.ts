...
): Context.Context<HttpPlatform.HttpPlatform> =>
  Context.make(
...
const makeHandler = (
  ...
  const self = app as Effect.Effect<
  ...
  if (context !== undefined) {
    return HttpEffect.toWebHandlerWith<HttpPlatform.HttpPlatform, HttpServerRequest | Scope.Scope>(
      context,
    )(self, middleware);
  }
  const { dispose, handler } = HttpEffect.toWebHandlerLayer(self, HttpServer.layerServices, {
    middleware,
  });
...
describe("HttpCompression", () => {
  ...
  it("server preference order picks br over gzip when supported", async () => {
    const response = await get(
      makeHandler(
        bigJsonApp,
        undefined,
        negotiationPlatformContext(["gzip", "deflate", "br", "zstd"]),
      ),
  ...
  it("falls through to a supported algorithm when the preferred one is unavailable", async () => {
    const response = await get(makeHandler(bigJsonApp, { algorithms: ["zstd", "br", "gzip"] }), {
