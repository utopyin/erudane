...
const makeRedirectClient = Effect.fnUntraced(function* (
  status: number,
  location: string | ReadonlyArray<string>,
) {
...
describe("HttpClient", () => {
  ...
  describe("tracer", () => {
    it.effect("includes request and response headers by default", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          clientSpan.attributes.get("http.request.header.x-request-default"),
          "request",
        );
        assert.strictEqual(
          clientSpan.attributes.get("http.response.header.x-response-default"),
          "response",
        );
      }),
    );
    ...
    it.effect("filters request and response header span attributes", () =>
      Effect.gen(function* () {
        ...
        assert(clientSpan !== undefined);
        assert.strictEqual(
          clientSpan.attributes.get("http.request.header.x-request-drop"),
          undefined,
        );
        assert.strictEqual(clientSpan.attributes.get("http.request.header.x-request-keep"), "keep");
        assert.strictEqual(
          clientSpan.attributes.get("http.response.header.x-response-drop"),
          undefined,
        );
        assert.strictEqual(
          clientSpan.attributes.get("http.response.header.x-response-keep"),
          "keep",
        );
      }),
    );
    ...
    it.effect("filters the same header name independently by phase", () =>
      Effect.gen(function* () {
        ...
        assert(clientSpan !== undefined);
        assert.strictEqual(
          clientSpan.attributes.get("http.request.header.x-phase-filter"),
          undefined,
        );
        assert.strictEqual(
          clientSpan.attributes.get("http.response.header.x-phase-filter"),
          "response",
        );
      }),
    );
  ...
  describe("followRedirects", () => {
    it.effect("preserves credential headers on same-origin redirects", () =>
      Effect.gen(function* () {
        const { client, requests } = yield* makeRedirectClient(
          302,
          "https://origin.test/destination",
        );
    ...
    it.effect("strips credential headers on cross-origin redirects", () =>
      Effect.gen(function* () {
        const { client, requests } = yield* makeRedirectClient(
          302,
          "https://redirect.test/destination",
        );
        yield* client.get("https://origin.test/start", {
          headers: {
            Authorization: "Bearer secret",
    ...
    it.effect("resolves relative locations against the current hop", () =>
      Effect.gen(function* () {
        const { client, requests } = yield* makeRedirectClient(302, [
          "../next/step",
          "destination",
        ]);
