...
describe("Multipart", () => {
  ...
  it.effect("collects file content across pulls and a split trailing boundary", () =>
    Effect.gen(function* () {
      ...
      const contents = yield* Stream.fromArray(chunks).pipe(
        ...
        Stream.mapEffect((part) =>
          part._tag === "File" ? part.contentEffect : Effect.die("expected file"),
        ),
        Stream.runCollect,
      );
  ...
  it.effect("propagates Parse when the body ends mid-file", () =>
    Effect.gen(function* () {
      ...
      const error = yield* Stream.make(encoder.encode(fileStart), encoder.encode("bbbb")).pipe(
        Stream.pipeThroughChannel(
          Multipart.makeChannel({ "content-type": `multipart/form-data; boundary=${boundary}` }),
        ),
        Stream.mapEffect((part) => {
  ...
  it.each<{
    ...
    "stops delivering fields when $description is exceeded",
    ({ expectedFields, limit, options }) => {
      const boundary = "----testboundary";
      const encoder = new TextEncoder();
      ...
      parser.write(
        encoder.encode(part("a") + part("b") + part("c") + part("d") + `--${boundary}--\r\n`),
      );
      parser.end();
  ...
  it.effect("returns distinct persisted file paths for files with the same client filename", () =>
    Effect.gen(function* () {
      ...
      const persisted = yield* Multipart.toPersisted(request.multipartStream, (path) =>
        Effect.sync(() => writes.push(path)),
      ).pipe(
