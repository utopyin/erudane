...
describe("HttpClientRequest", () => {
  ...
  describe("setBody", () => {
    it("removes stale content length when the replacement body has no known length", () => {
      const request = HttpClientRequest.bodyText(
        ...
        "abc",
      ).pipe(HttpClientRequest.setBody(HttpBody.stream(Stream.empty)));
  ...
  describe("removeHeader", () => {
    ...
    it("no-ops on a missing header", () => {
      const request = HttpClientRequest.get("/").pipe(HttpClientRequest.setHeader("X-Test", "ok"));
    ...
    it("preserves the request prototype", () => {
      const request = HttpClientRequest.get("/").pipe(HttpClientRequest.removeHeader("X-Test"));
  ...
  describe("updateHeaders", () => {
    it("transforms the header collection", () => {
      const request = HttpClientRequest.get("/").pipe(
        ...
        HttpClientRequest.updateHeaders((headers) =>
          Headers.set(Headers.remove(headers, "X-A"), "X-C", "c"),
        ),
      );
