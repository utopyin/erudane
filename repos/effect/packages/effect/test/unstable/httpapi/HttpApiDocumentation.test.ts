...
describe("HttpApiScalar", () => {
  it.effect("escapes OpenAPI metadata in its HTML contexts", () =>
    Effect.gen(function* () {
      ...
      const escapedDescription = `&quot;quoted&quot; &#39;single&#39; &lt;/script &gt;&lt;script id=&quot;script-data-injected&quot;&gt;`;
  ...
  it.effect("encodes CDN versions before interpolating the script source", () =>
    Effect.gen(function* () {
      ...
      assert.ok(
        html.includes(
          `src="https://cdn.jsdelivr.net/npm/@scalar/api-reference@${encodeURIComponent(
            version,
...
describe("HttpApiSwagger", () => {
  it.effect("escapes script end-tag variants in OpenAPI data", () =>
    Effect.gen(function* () {
      const injectedTag = `<script id="script-data-injected">`;
      const Api = HttpApi.make("Docs").annotate(OpenApi.Description, `</script/>${injectedTag}`);
