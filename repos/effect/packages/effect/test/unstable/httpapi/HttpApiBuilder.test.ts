  ...
  it.effect("round trips array query parameters with one or more values", () =>
    Effect.gen(function* () {
      ...
      const GroupLayer = HttpApiBuilder.group(Api, "test", (handlers) =>
        handlers.handle("list", ({ query }) => Effect.succeed(query)),
      );
...
it.layer(TestServices)("HttpApiBuilder WithHeaders responses", (it) => {
  it.effect("encodes WithHeaders using the schema for the response status", () =>
    Effect.gen(function* () {
      ...
      const Ok = HttpApiSchema.WithHeaders(Schema.TaggedStruct("Ok", {}), {
  ...
  it.effect(
    "encodes a branded response against the header-carrying member when body shapes overlap",
    () =>
      Effect.gen(function* () {
        const Api = HttpApi.make("Api").add(
          HttpApiGroup.make("test").add(
            HttpApiEndpoint.get("result", "/test", {
              success: [
                ...
                HttpApiSchema.WithHeaders(Schema.Struct({ value: Schema.String }), {
  ...
  it.effect(
    "fails with a Body schema error when a branded response body matches no header-carrying member",
    () =>
      Effect.gen(function* () {
        const Api = HttpApi.make("Api").add(
          HttpApiGroup.make("test").add(
            HttpApiEndpoint.get("result", "/test", {
              success: [
                HttpApiSchema.WithHeaders(Schema.TaggedStruct("A", {}), {
                  "x-source": Schema.String,
                }),
    ...
    Effect.gen(function* () {
      const Api = HttpApi.make("Api").add(
        HttpApiGroup.make("test").add(
          HttpApiEndpoint.get("result", "/test", {
            disableCodecs: true,
            success: HttpApiSchema.WithHeaders(Schema.String.pipe(HttpApiSchema.asText()), {
  ...
  it.effect("passes through string-shaped WithHeaders when endpoint codecs are disabled", () =>
    Effect.gen(function* () {
      ...
      const client = yield* HttpApiTest.groups(Api, ["test"]).pipe(Effect.provide(GroupLayer));
      const [success, successResponse] = yield* client.test.success({
  ...
  it.effect("encodes and decodes response headers with codecs enabled", () =>
    Effect.gen(function* () {
      ...
      const client = yield* HttpApiTest.groups(Api, ["test"]).pipe(Effect.provide(GroupLayer));
      const [value, response] = yield* client.test.created({
        responseMode: "decoded-and-response",
      });
  ...
  it.effect("does not unwrap an unbranded value in a mixed success union", () =>
    Effect.gen(function* () {
      ...
      const GroupLayer = HttpApiBuilder.group(Api, "test", (handlers) =>
        handlers.handle("mixed", () =>
          Effect.succeed({ body: "plain", headers: { "x-source": "body" } }),
        ),
      );
  ...
  it.effect("decodes a plain value in a mixed WithHeaders success union", () =>
    Effect.gen(function* () {
      ...
      const GroupLayer = HttpApiBuilder.group(Api, "test", (handlers) =>
        handlers.handle("mixed", () => Effect.succeed({ _tag: "Plain" as const, value: "plain" })),
      );
  ...
  it.effect("encodes error headers through encodeToWithHeaders", () =>
    Effect.gen(function* () {
      ...
      const GroupLayer = HttpApiBuilder.group(Api, "test", (handlers) =>
        handlers.handle("limited", () => Effect.fail(new RateLimited({ retryAfter: 30 }))),
      );
  ...
  it.effect("encodes annotation headers from their declared encoded shape", () =>
    Effect.gen(function* () {
      ...
      const GroupLayer = HttpApiBuilder.group(Api, "test", (handlers) =>
        handlers.handle("limited", () => Effect.fail(new RateLimited({ expiresAt }))),
      );
  ...
  it.effect("encodes and decodes an error wrapped in WithHeaders", () =>
    Effect.gen(function* () {
      const RateLimited = HttpApiSchema.WithHeaders(
        Schema.TaggedStruct("RateLimited", { message: Schema.String }).pipe(
      ...
      const GroupLayer = HttpApiBuilder.group(Api, "test", (handlers) =>
        handlers.handle("limited", () => Effect.fail(expected)),
      );
  ...
  it.effect(
    "passes through string-shaped encodeToWithHeaders values when endpoint codecs are disabled",
    () =>
      Effect.gen(function* () {
        class RateLimited extends Schema.TaggedError<RateLimited>()("RateLimited", {
    ...
    Effect.gen(function* () {
      ...
      const GroupLayer = HttpApiBuilder.group(Api, "test", (handlers) =>
        handlers.handle("limited", () =>
          Effect.fail(new RateLimited({ message: "slow down", retryAfter: 30 })),
        ),
      );
...
it.layer(TestServices)("HttpApiBuilder streaming success responses", (it) => {
    ...
    Effect.gen(function* () {
      ...
      const Api = HttpApi.make("Api").add(
        HttpApiGroup.make("test").add(
          HttpApiEndpoint.get("result", "/test", {
            success: [
              HttpApiSchema.WithHeaders(HttpApiSchema.StreamUint8Array(), {
                "x-source": Schema.String,
              }),
              Buffered,
            ],
      ...
      const GroupLayer = HttpApiBuilder.group(Api, "test", (handlers) =>
        handlers.handle("result", () => Effect.succeed({ message: "done" })),
      );
  ...
  it.effect("rejects duplicate handle and handleAll registrations", () =>
    Effect.gen(function* () {
      ...
      const HandleLayer = HttpApiBuilder.group(Api, "users", (handlers) => {
        handlers.handle("getUser", () => Effect.succeed("first"));
        return handlers.handle("getUser", () => Effect.succeed("second"));
      });
      const HandleAllLayer = HttpApiBuilder.group(Api, "users", (handlers) => {
  ...
  it.effect("does not try another security scheme after the handler fails", () =>
    Effect.gen(function* () {
      class HandlerFailure extends Schema.TaggedError<HandlerFailure>()(
        "HandlerFailure",
        {
          message: Schema.String,
        },
