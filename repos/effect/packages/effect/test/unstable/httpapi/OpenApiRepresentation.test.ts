...
describe("OpenApi representation v2 consumer", () => {
  ...
  it("applies the reference policy without removing additional schema components", () => {
    ...
    const Api = HttpApi.make("Api")
      .annotate(HttpApi.AdditionalSchemas, [AdditionalDate])
      .add(
  ...
  it("uses canonical JSON codecs for response declaration schemas", () => {
    const Api = HttpApi.make("Api").add(
      HttpApiGroup.make("test").add(HttpApiEndpoint.get("date", "/date", { success: Schema.Date })),
    );

    assert.deepStrictEqual(
      OpenApi.fromApi(Api).paths["/date"]?.get?.responses[200]?.content?.["application/json"]
        ?.schema,
      { type: "string" },
    );
  });
  ...
  it("uses custom JSON Schema compiler annotations", () => {
    const CustomString = Schema.String.check(
      Schema.makeFilter<string>((value) => value.length >= 2, {
        representation: {
  ...
  it("shares definitions and returns cached copies by API identity", () => {
    const Shared = Schema.Struct({ value: Schema.FiniteFromString }).annotate({
      identifier: "Shared",
    });
    const Api = HttpApi.make("Api").add(
      HttpApiGroup.make("test").add(
