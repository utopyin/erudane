...
describe("HttpApiEndpoint WithHeaders schemas", () => {
      ...
      () =>
        HttpApiEndpoint.get("get", "/", {
          success: [
            ...
            HttpApiSchema.WithHeaders(Schema.Struct({ wrapped: Schema.String }), {
  ...
  it("rejects an encodeToWithHeaders error sharing its status and content type", () => {
    ...
    assert.throws(
      () =>
        HttpApiEndpoint.get("get", "/", {
          error: [Schema.Struct({ plain: Schema.String }), ErrorWithHeaders],
        }),
      ...
      () =>
        HttpApiEndpoint.get("get", "/", {
          success: [
            ...
            HttpApiSchema.WithHeaders(Schema.String.pipe(HttpApiSchema.asText()), {
  ...
  it("keeps the wrapper in the success set with codec-transformed parts", () => {
    ...
    if (HttpApiSchema.isWithHeaders(schema)) {
      assert.deepStrictEqual(Schema.encodeSync(schema.headers as any)({ "x-count": 3 }), {
        "x-count": "3",
      });
    }
  });
  ...
  it("rejects wrapped and bare streams at distinct statuses", () => {
    ...
    assert.throws(
      () =>
        HttpApiEndpoint.get("events", "/events", {
          success: [HttpApiSchema.WithHeaders(sse(), { "x-count": Schema.Int }), sse()],
        }),
      "Multiple streaming success responses are not supported",
    );
  });
  ...
  it("keeps WithHeaders in the error set with codec-transformed parts", () => {
    ...
    if (HttpApiSchema.isWithHeaders(schema)) {
      assert.deepStrictEqual(Schema.encodeSync(schema.headers as any)({ "retry-after": 30 }), {
        "retry-after": "30",
      });
    }
  });
