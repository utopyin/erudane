...
describe("HttpApiSchema", () => {
  ...
  describe("WithHeaders", () => {
    ...
    it("keeps the wrapper intact through annotate", () => {
      ...
      const schema = HttpApiSchema.WithHeaders(Schema.String, headers).annotate({
    ...
    it("resolves the status from the wrapper first, falling through to the inner schema", () => {
      ...
      const onInner = HttpApiSchema.WithHeaders(
      ...
      const onBoth = HttpApiSchema.WithHeaders(
        Schema.String.pipe(HttpApiSchema.status(201)),
        headers,
      ...
      assert.strictEqual(
        HttpApiSchema.getStatusSuccessSchema(Schema.String.pipe(HttpApiSchema.status(201))),
        201,
      );
    });
  ...
  describe("encodeToWithHeaders", () => {
    ...
    it("decodes a pair back to the source type", () => {
      assert.deepStrictEqual(
        Schema.decodeUnknownSync(WrappedUserNotFound)({
          body: undefined,
          headers: { "x-user-id": 1 },
        }),
  ...
  describe("status", () => {
    it("accepts status literals", () => {
      assert.strictEqual(
        HttpApiSchema.getStatusSuccess(Schema.String.pipe(HttpApiSchema.status("Created")).ast),
        201,
      );
      assert.strictEqual(
        HttpApiSchema.getStatusError(Schema.String.pipe(HttpApiSchema.status("Conflict")).ast),
        409,
      );
    });
  });
