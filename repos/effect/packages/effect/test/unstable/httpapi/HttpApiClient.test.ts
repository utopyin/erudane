  ...
  describe("streaming responses", () => {
    ...
    it.effect("fails invalid WithHeaders stream headers before returning the body", () =>
      Effect.gen(function* () {
        ...
        if (exit._tag === "Failure") {
          assert.strictEqual(
            (Cause.squash(exit.cause) as { readonly _tag?: string })._tag,
            "SchemaError",
          );
        }
      }),
        ...
        const Api = HttpApi.make("Api").add(
          HttpApiGroup.make("test").add(
            HttpApiEndpoint.get("created", "/created", {
              success: HttpApiSchema.WithHeaders(Schema.Struct({ id: Schema.Int }), {
                "x-count": Schema.Int,
              }),
            }),
          ),
...
class EndpointError extends Schema.TaggedError<EndpointError>()(
  "EndpointError",
  {
