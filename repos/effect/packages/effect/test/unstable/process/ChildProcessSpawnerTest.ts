    ...
    it.layer(layer)((it) => {
          ...
          it.effect("should force kill a process after the initial signal times out", () =>
            Effect.gen(function* () {
              ...
              const handle = yield* ChildProcess.make(
                "sh",
                ["-c", "trap '' TERM; : > \"$1\"; while :; do sleep 1; done", "force-kill", ready],
                {
                  killSignal: "SIGKILL",
                  stdout: "ignore",
                  stderr: "ignore",
              ...
              const completed = yield* Effect.scoped(
                Effect.gen(function* () {
                  yield* ChildProcess.make(
                    "sh",
                    ["-c", "trap '' TERM; : > \"$1\"; sleep 2", "force-kill", ready],
                    {
                      killSignal: "SIGTERM",
                      forceKillAfter: "50 millis",
                      stdout: "ignore",
      ...
      describe("pipeline pipe options", () => {
        ...
        it.effect("should pipe combined output with { from: 'all' }", () =>
          Effect.gen(function* () {
            ...
            assert.deepStrictEqual(
              lines.filter((line) => line.startsWith("out")),
              ["out1", "out2"],
            );
            assert.deepStrictEqual(
              lines.filter((line) => line.startsWith("err")),
              ["err1"],
            );
          }),
        );
      ...
      describe.sequential("process supervision", () => {
        ...
        it.effect.skipIf(!options.processGroups)(
          "should kill all child processes in process group",
          () =>
            Effect.gen(function* () {
              const path = yield* Path.Path;
              const cwd = yield* path.fromFileUrl(new URL("./fixtures/bash/", import.meta.url));
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              Effect.ensuring(
                Effect.ignore(
                  Effect.all(
                    [killMatchingProcesses(rootMarker), killMatchingProcesses(tailMarker)],
                    { discard: true },
                  ),
                ),
              ),
