...
describe("Redis", () => {
  ...
  it.effect("clearing an empty persistence store succeeds", () => {
    const redis = Redis.Redis.of({
      send: <A>(command: string, ...args: ReadonlyArray<string>) => {
        ...
        if (command.toUpperCase() === "DEL" && args.length === 0) {
          return Effect.fail(
            new Redis.RedisError({ cause: "ERR wrong number of arguments for 'del' command" }),
          );
        }
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        Persistence.layerBackingRedis.pipe(Layer.provide(Layer.succeed(Redis.Redis, redis))),
      ),
    );
  });
