...
describe("Prompt.date", () => {
  it.effect("renders two-digit years, teen ordinals, and noon meridiem correctly", () =>
    Effect.gen(function* () {
      const initial = DateTime.toDateUtc(
...
describe("Prompt.select", () => {
  ...
  it.effect("aligns choices when paging arrows have different widths", () =>
    Effect.gen(function* () {
      ...
      const frames = toFrames(yield* MockTerminal.displayLines);
      assert.isTrue(
        frames.some((frame) => frame.includes("❯   Alpha") && frame.includes("vvv Beta")),
      );
      assert.isTrue(
        frames.some((frame) => frame.includes("    Beta") && frame.includes("❯   Gamma")),
      );
    }).pipe(Effect.provide(TestLayer)),
  );
});
...
describe("Prompt.password", () => {
  ...
  it.effect("keeps cursor movement aligned with a multi-character password mask", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(
        frames.some((frame) => frame.includes("******") && frame.endsWith(`${escape}[2D`)),
      );
    }).pipe(Effect.provide(TestLayer)),
  );
...
describe("Prompt.file", () => {
  ...
  const JkFilePromptLayer = Layer.mergeAll(
    FileSystem.layerNoop({
      ...
      readDirectory: (directory) =>
        Effect.succeed(
          directory === "/workspace" ? ["jest.config.ts", "package.json", "readme.md"] : [],
        ),
...
describe("Prompt.multiSelect", () => {
  ...
  it.effect("aligns choices when checkbox symbols have different widths", () =>
    Effect.gen(function* () {
      ...
      const frame = toFrames(yield* MockTerminal.displayLines).find(
        (frame) => frame.includes("[x] Alpha") && frame.includes("Beta"),
      );
