...
describe("Param", () => {
  ...
  it.effect("recognizes the alternate flag declared by orElse", () =>
    Effect.gen(function* () {
      const command = Command.make("app", {
        config: Flag.string("config").pipe(Flag.orElse(() => Flag.string("config-url"))),
      });

      const parsed = yield* Parser.parseArgs(
        Lexer.lex(["--config-url", "https://example.com"]),
        command,
      );
  ...
  it.effect("registers and parses the alternate flag declared by orElseResult", () =>
    Effect.gen(function* () {
      const flag = Flag.string("config").pipe(Flag.orElseResult(() => Flag.string("config-url")));

      assert.deepStrictEqual(
        Param.extractSingleParams(flag).map((param) => param.name),
        ["config", "config-url"],
      );
