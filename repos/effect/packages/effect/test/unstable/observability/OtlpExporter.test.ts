...
const makeControlledHttpClient = Effect.fnUntraced(function* (requestCount: number) {
  const started = yield* Effect.forEach(Array.from({ length: requestCount }), () =>
    Deferred.make<void>(),
  );
  const releases = yield* Effect.forEach(Array.from({ length: requestCount }), () =>
    Deferred.make<void>(),
  );
    ...
    it.effect("shares one registry across traces, logs and metrics", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.deepStrictEqual([...(yield* Ref.get(urls))].sort(), [
