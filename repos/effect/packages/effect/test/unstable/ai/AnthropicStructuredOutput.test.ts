  ...
  describe("Canonical JSON and mechanical invariants", () => {
    ...
    describe("Arrays", () => {
      it("encodes post-rest elements as object properties", () => {
        assertJsonSchema(Schema.TupleWithRest(Schema.Tuple([]), [Schema.String, Schema.String]), {
          type: "object",
          properties: {
    ...
    it("Record with properties and an index signature", async () => {
      ...
      assert.deepStrictEqual(result.jsonSchema, {
        ...
        items: {
          ...
          properties: {
            "0": {
              anyOf: [{ type: "string", enum: ["fixed"] }, { type: "string" }],
            },
            "1": { type: "string" },
          },
          required: ["0", "1"],
      ...
      await asserts.encoding().succeed({ fixed: "required", dynamic: "value" }, [
        { 0: "fixed", 1: "required" },
        { 0: "dynamic", 1: "value" },
      ]);
