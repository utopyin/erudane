...
describe("OpenAiStructuredOutput representation v2", () => {
  ...
  it("keeps supported custom JSON Schema filters with a description", () => {
    const schema = Schema.String.check(
      Schema.makeFilter<string>((value) => value.startsWith("a"), {
  ...
  it("keeps property names that match unsupported keywords", () => {
    const unsupported = Schema.Any.check(
      Schema.makeFilter<unknown>(() => true, {
        representation: {
  ...
  it("drops patternProperties returned by custom callbacks", () => {
    const schema = Schema.Struct({ fixed: Schema.String }).check(
      Schema.makeFilter<object>(() => true, {
        representation: {
  ...
  it("merges repeated lower bounds conservatively", () => {
    assert.deepStrictEqual(
      ...
      ),
      {
        type: "number",
        description:
          "a value greater than 1 and a value greater than or equal to 2 and a value greater than 2",
        exclusiveMinimum: 2,
      },
    );
  });
