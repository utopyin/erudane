...
const decodeResourceUpdated = Schema.decodeUnknownEffect(
  ...
  it.layer(layer)(`Mcp Conformance (${protocol.protocolVersion})`, (it) => {
    describe("Resources", () => {
      ...
      describe("Capabilities", () => {
        ...
        it.effect(
          "MUST advertise listChanged when resource list change notifications are supported",
          () =>
            Effect.gen(function* () {
              const test = yield* McpConformance;
              const initialized = yield* test.initialize({ server: "features" });
              ...
              assert.strictEqual(
                initialized.message.result.capabilities.resources?.listChanged,
                true,
              );
            }),
        );
        ...
        it.effect("MUST list every resource visible to the initialized client", () =>
          Effect.gen(function* () {
            ...
            const result = yield* test
              ...
              .pipe(Effect.flatMap((message) => decodeResources(message.result)));
      ...
      describe("Reading Resources", () => {
        it.effect("MUST read text resource contents", () =>
          Effect.gen(function* () {
            ...
            const result = yield* test
              .decodeResult(response)
              .pipe(Effect.flatMap((message) => decodeReadResource(message.result)));
        ...
        it.effect("MUST return multiple resource contents in order", () =>
          Effect.gen(function* () {
            ...
            assert.deepStrictEqual(
              result.contents.map((content) => content.uri),
              ["file:///multiple#first", "file:///multiple#second"],
            );
          }),
        );
      ...
      describe("Resource Templates", () => {
        it.effect("MUST list every registered resource template", () =>
          Effect.gen(function* () {
            ...
            const result = yield* test
              .decodeResult(response)
              .pipe(Effect.flatMap((message) => decodeResourceTemplates(message.result)));
      ...
      describe("List Changed Notification", () => {
        it.effect(
          "SHOULD send a resource list changed notification when the advertised list changes",
          () =>
            Effect.gen(function* () {
              ...
              const initializeResult = yield* Schema.decodeUnknownEffect(
              ...
              const notification = yield* fixture.awaitOutboundMethod(
              ...
              assert.isTrue(
                result.resources.some(
                  (resource) => resource.uri === "file:///dynamic-list-changed",
                ),
              );
            }),
        );
      ...
      describe("Subscriptions", () => {
        it.effect("MUST subscribe to a resource when subscriptions are advertised", () =>
          Effect.gen(function* () {
            const fixture = yield* makeMcpStdioHarness(protocol);
            yield* fixture.server.addResource(
              makeResource("file:///subscription-target", "subscription-target"),
            );
            const initialized = yield* fixture.initialize();
            const initializeResult = yield* Schema.decodeUnknownEffect(McpSchema.InitializeResult)(
              initialized.result,
            );
        ...
        it.effect("MUST unsubscribe from resource updates", () =>
          Effect.gen(function* () {
            ...
            yield* fixture.server.addResource(
              makeResource("file:///subscription-sentinel", "subscription-sentinel"),
            );
            yield* fixture.initialize();
            yield* fixture.sendRequest("resources/subscribe", {
              uri: "file:///subscription-target",
