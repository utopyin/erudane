...
}).annotate(McpSchema.EnabledWhen, (client) => client.protocolVersion === "2025-06-18");
...
const CapabilityGatedTool = Tool.make("capability-gated", {
  ...
  (client) =>
    client.clientInfo.name === "allowed-client" && client.capabilities.roots !== undefined,
);
    ...
    Layer.provideMerge(
      TestToolkit.toLayer(
        TestToolkit.of({
          ...
          "initialize-metadata": () =>
            McpSchema.McpServerClient.useSync((client) =>
              JSON.stringify(client.initializePayload._meta),
            ),
...
const listedTools = (message: JsonRpcResponse): ReadonlyArray<Record<string, unknown>> => {
  ...
  return Schema.decodeUnknownSync(Schema.Array(Schema.Record(Schema.String, Schema.Unknown)))(
    tools,
  );
};
...
describe("McpServer protocol adapters", () => {
  ...
  it.effect("should project canonical notifications when encoding each protocol revision", () =>
    Effect.gen(function* () {
      for (const protocol of [
        ...
        const progress = yield* protocol.projectNotification(
          McpCore.ServerNotification.Progress({
        ...
        for (const [notification, expected] of [
          [
            McpCore.ServerNotification.Cancelled({ requestId: 1, reason: "done", metadata }),
            {
              ...
              payload: { _meta: metadata, requestId: 1, reason: "done" },
            },
          ],
          [
      ...
      const elicitationComplete = McpCore.ServerNotification.ElicitationComplete({
        elicitationId: "elicitation-1",
      });
      assert.deepStrictEqual(
        yield* McpProtocol.v2025_11_25.projectNotification(elicitationComplete),
        {
  ...
  it.effect(
    "should reject elicitation before transport when the protocol predates v2025-06-18",
    () =>
      Effect.gen(function* () {
        let sends = 0;
        ...
        for (const protocol of [McpProtocol.v2024_11_05, McpProtocol.v2025_03_26]) {
          const client = yield* protocol
            ...
            .pipe(Effect.provideService(RpcClient.Protocol, reverseProtocol));
  ...
  it.effect("should omit June fields when projecting a March tool descriptor", () =>
    Effect.gen(function* () {
      ...
      const shared = listedTools(yield* client.request("tools/list")).find(
        (tool) => tool.name === "shared",
      );
  ...
  it.effect("should prefer the canonical title when projecting March annotations", () =>
    Effect.gen(function* () {
      ...
      const marchClient = yield* initialize(fixture.post, "2025-03-26");
      const tool = listedTools(yield* marchClient.request("tools/list")).find(
        (tool) => tool.name === "title-precedence",
      );
  ...
  it.effect(
    "should route each session through its negotiated RPC group when revisions coexist",
    () =>
      Effect.gen(function* () {
        const fixture = yield* makeFixture();
        const oldClient = yield* initialize(fixture.post, "2024-11-05");
        ...
        const oldShared = listedTools(yield* oldClient.request("tools/list")).find(
          (tool) => tool.name === "shared",
        );
        const currentShared = listedTools(yield* currentClient.request("tools/list")).find(
          (tool) => tool.name === "shared",
        );
  ...
  for (const protocolVersion of ["2025-06-18", "2024-11-05"] as const) {
    it.effect(
      `should expose the negotiated client profile when serving ${protocolVersion} requests`,
      () =>
        Effect.gen(function* () {
          const fixture = yield* makeFixture();
          ...
          const observed = JSON.parse(
            textResult(yield* allowed.request("tools/call", { name: "capability" })),
          );
  ...
  it.effect("should encode binary content for every revision that can represent it", () =>
    Effect.gen(function* () {
      ...
      for (const protocolVersion of ["2024-11-05", "2025-03-26", "2025-06-18"] as const) {
        ...
        const embedded = resultOf(yield* client.request("tools/call", { name: "binary-resource" }));
  ...
  it.effect("should project structured content according to the negotiated revision", () =>
    Effect.gen(function* () {
      ...
      assert.match(
        scalarError.message,
        /non-object structured tool content is not supported by MCP 2025-06-18/,
      );
    }),
  );

  it.effect(
    "should preserve only supported metadata when projecting embedded resource content",
    () =>
      Effect.gen(function* () {
        const fixture = yield* makeLowLevelFixture();
        ...
        for (const protocolVersion of [
          ...
          const resource = Schema.decodeUnknownSync(Schema.Record(Schema.String, Schema.Unknown))(
            content.resource,
          );
          assert.deepStrictEqual(
            content.annotations,
  ...
  it.effect(
    "should preserve result metadata while omitting June-only metadata for older revisions",
    () =>
      Effect.gen(function* () {
        const fixture = yield* makeLowLevelFixture();
        for (const protocolVersion of ["2024-11-05", "2025-03-26", "2025-06-18"] as const) {
  ...
  it.effect(
    "should expose implementation metadata and descriptor icons when supported by the protocol",
    () =>
      Effect.gen(function* () {
        const fixture = yield* makeLowLevelFixture();
        ...
        const prompt = (prompts.prompts as Array<Record<string, unknown>>).find(
          (_) => _.name === "metadata-prompt",
        );
        assert.isDefined(prompt);
        assert.deepStrictEqual(prompt.icons, [{ src: "https://example.com/prompt.svg" }]);

        const tool = listedTools(yield* november.request("tools/list")).find(
          (_) => _.name === "title-precedence",
        );
        assert.isDefined(tool);
  ...
  it.effect(
    "should hide and reject a tool when its declaration is incompatible with the revision",
    () =>
      Effect.gen(function* () {
        const fixture = yield* makeFixture();
        const currentClient = yield* initialize(fixture.post, "2025-06-18");
  ...
  it.effect("should reject after invocation when a tool result cannot be projected", () =>
    Effect.gen(function* () {
      ...
      assert.property(yield* oldClient.request("tools/call", { name: "audio" }), "error");
      assert.property(yield* oldClient.request("tools/call", { name: "resource-link" }), "error");
  ...
  it.effect("should list and invoke the latest tool when a name is registered twice", () =>
    Effect.gen(function* () {
      ...
      for (const client of [currentClient, oldClient]) {
        const duplicates = listedTools(yield* client.request("tools/list")).filter(
          (tool) => tool.name === "duplicate",
        );
        assert.lengthOf(duplicates, 1);
        assert.strictEqual(duplicates[0]?.description, "second descriptor");

        const result = resultOf(yield* client.request("tools/call", { name: "duplicate" }));
