...
const callTool = (name: string, arguments_: Record<string, unknown> = {}) =>
  Effect.gen(function* () {
    ...
    return yield* test
      ...
      .pipe(Effect.flatMap((message) => decodeCallTool(message.result)));
  });
  ...
  it.layer(layer)(`Mcp Conformance (${protocol.protocolVersion})`, (it) => {
    describe("Tools", () => {
      ...
      describe("Capabilities", () => {
        ...
        it.effect(
          "MUST advertise listChanged when tool list change notifications are supported",
          () =>
            Effect.gen(function* () {
              const test = yield* McpConformance;
              const initialized = yield* test.initialize({ server: "features" });
        ...
        it.effect("MUST list every tool visible to the initialized client", () =>
          Effect.gen(function* () {
            ...
            const result = yield* test
              .decodeResult(response)
              .pipe(Effect.flatMap((message) => decodeTools(message.result)));
        ...
        it.effect("MUST call a registered tool with valid arguments", () =>
          Effect.gen(function* () {
            ...
            assert.deepStrictEqual(result.content, [
              { type: "text", text: JSON.stringify("called") },
            ]);
          }),
        );
      ...
      describe("List Changed Notification", () => {
        it.effect(
          "SHOULD send a tool list changed notification when the advertised list changes",
          () =>
            Effect.gen(function* () {
