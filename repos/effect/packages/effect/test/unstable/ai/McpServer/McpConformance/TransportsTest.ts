...
const decodeErrorFrame = Schema.decodeUnknownEffect(
  Schema.Struct({
    id: Schema.Null,
      ...
      describe("stdio", () => {
        ...
        it.effect("MUST shut down when the client closes stdin", () =>
          Effect.gen(function* () {
            ...
            assert.isTrue(
              Exit.isSuccess(exit) || (Exit.isFailure(exit) && Cause.hasInterruptsOnly(exit.cause)),
            );
          }),
        );
      });
      ...
      describe(httpTransportSuiteName(protocol), () => {
        describe("Sending Messages to the Server", () => {
            ...
            () =>
              Effect.gen(function* () {
                ...
                const mismatched = yield* test.ping(initialized, {
                  id: 3,
                  protocolVersion:
                    protocol.protocolVersion === "2025-11-25" ? "2025-06-18" : "2025-03-26",
                });
          ...
          it.effect("MUST require the application/json content type for POST requests", () =>
            Effect.gen(function* () {
              const test = yield* McpConformance;
              const accepted = yield* test.request(
                jsonRequest("POST", test.initializeRequest(), {
              ...
              for (const contentType of ["text/plain", "application/json-malicious", ""]) {
                const response = yield* test.request(
                  jsonRequest("POST", test.initializeRequest(), {
          ...
          it.effect("MUST require clients to accept application/json and text/event-stream", () =>
            Effect.gen(function* () {
              ...
              for (const accept of [
                ...
                const response = yield* test.request(
                  jsonRequest("POST", test.initializeRequest(), { accept }),
                );
          ...
          it.effect(
            "MUST return an empty 202 response for accepted notifications and responses",
            () =>
              Effect.gen(function* () {
                const test = yield* McpConformance;
                const initialized = yield* test.initialize();
        ...
        describe("Session Management", () => {
          ...
          it.effect(
            "MUST require the returned session identifier on subsequent HTTP requests",
            () =>
              Effect.gen(function* () {
                const test = yield* McpConformance;
                yield* test.initialize();
                const response = yield* test.request(
                  jsonRequest("POST", test.pingRequest(), {
          ...
          it.effect(
            "SCENARIO declines client session termination without invalidating the session",
            () =>
              Effect.gen(function* () {
                const test = yield* McpConformance;
                const initialized = yield* test.initialize();
          ...
          it.effect("MUST reject initialize requests carrying a session identifier", () =>
            Effect.gen(function* () {
              ...
              const unknown = yield* test.request(
                jsonRequest("POST", test.initializeRequest({ id: 3 }), {
                  "Mcp-Session-Id": "unknown-session",
                }),
              );
        ...
        describe("Security", () => {
          it.effect("MUST validate the Origin header before every MCP route", () =>
            Effect.gen(function* () {
              ...
              assert.strictEqual(
                (yield* test.request(
                  jsonRequest("POST", test.initializeRequest({ id: 2 }), {
