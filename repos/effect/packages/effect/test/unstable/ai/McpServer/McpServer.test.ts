...
const makeRouterTestClient = (router: Layer.Layer<never, never, HttpRouter.HttpRouter>) =>
...
describe("McpServer", () => {
  describe("direct service", () => {
    it.effect("should fail when a resource URI is unknown", () =>
      Effect.gen(function* () {
        ...
        const error = yield* server
          .findResource("file:///unknown")
          .pipe(Effect.provideService(McpSchema.McpServerClient, directClient), Effect.flip);
    ...
    it.effect("should pass undefined to a low-level tool handler when arguments are omitted", () =>
      Effect.gen(function* () {
        ...
        yield* server
          .callTool({ name: "arguments-omitted" })
  ...
  it.effect(
    "returns an empty 202 for notifications and responses and remains successful for request POSTs",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("drops server notifications from buffered JSON-RPC responses", () =>
    Effect.gen(function* () {
      ...
      ).pipe(Layer.provideMerge(HttpRouter.layer), Layer.provide(RpcSerialization.layerJsonRpc()));
          ...
          Deferred.succeed(writeRequest, write).pipe(
            Effect.as({
              ...
              send: (clientId, message) =>
                Queue.offer(clientId === 1 ? client1Outbound : client2Outbound, message).pipe(
                  Effect.asVoid,
                ),
