...
const BatchResponse = Schema.Array(
  Schema.Struct({
    id: Schema.Number,
...
export interface McpConformanceShape {
  ...
  readonly makePeer: (options?: McpTestPeerOptions) => ReturnType<typeof makeMcpTestPeer>;
  readonly observations: Effect.Effect<Observations>;
  readonly resetObservations: Effect.Effect<void>;
  readonly decodeError: (
    response: Response,
  ) => Effect.Effect<typeof ErrorResponse.Type, Schema.SchemaError>;
  readonly decodeResult: (
    response: Response,
  ) => Effect.Effect<typeof ResultResponse.Type, Schema.SchemaError>;
...
export const layer = (protocol: McpProtocol.ProtocolAdapter) =>
  Layer.effect(
    McpConformance,
    Effect.gen(function* () {
      const defaultHarness = yield* makeHttpHarness(
        makeServerLayer({
          name: SERVER_NAME,
          protocols: [protocol],
        }),
      ...
      const featuresHarness = yield* makeHttpHarness(
        makeFeaturesServerLayer(protocol, observations),
      );
      ...
      const ping: McpConformanceShape["ping"] = (session, options) =>
        send(session, pingRequest(options?.id), options);
      ...
      return McpConformance.of({
        ...
        decodeError: (response) =>
          Effect.promise<unknown>(() => response.json()).pipe(Effect.flatMap(decodeErrorResponse)),
        decodeResult: (response) =>
          Effect.promise<unknown>(() => response.json()).pipe(Effect.flatMap(decodeResultResponse)),
