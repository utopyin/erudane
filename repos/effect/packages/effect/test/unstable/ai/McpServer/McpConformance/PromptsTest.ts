  ...
  it.layer(layer)(`Mcp Conformance (${protocol.protocolVersion})`, (it) => {
    describe("Prompts", () => {
      ...
      describe("Capabilities", () => {
        ...
        it.effect(
          "MUST advertise listChanged when prompt list change notifications are supported",
          () =>
            Effect.gen(function* () {
              const test = yield* McpConformance;
              const initialized = yield* test.initialize({ server: "features" });
              ...
              assert.strictEqual(
                initialized.message.result.capabilities.prompts?.listChanged,
                true,
              );
            }),
        );
        ...
        it.effect("MUST list every prompt visible to the initialized client", () =>
          Effect.gen(function* () {
            ...
            const result = yield* test
              ...
              .pipe(Effect.flatMap((message) => decodePrompts(message.result)));
      ...
      describe("Getting Prompts", () => {
        it.effect("MUST get a registered prompt without arguments", () =>
          Effect.gen(function* () {
            ...
            const result = yield* test
              .decodeResult(response)
              .pipe(Effect.flatMap((message) => decodeGetPrompt(message.result)));
      ...
      describe("List Changed Notification", () => {
        it.effect(
          "SHOULD send a prompt list changed notification when the advertised list changes",
          () =>
            Effect.gen(function* () {
              ...
              assert.strictEqual(initializeResult.capabilities.prompts?.listChanged, true);
              ...
              assert.isTrue(
                result.prompts.some((prompt) => prompt.name === "dynamic-list-changed-prompt"),
              );
            }),
        );
      });
