...
const decodeSamplingResult = Schema.decodeUnknownEffect(
  Schema.Struct({
    role: Schema.String,
    content: Schema.Record(Schema.String, Schema.Unknown),
    model: Schema.String,
          ...
          () =>
            Effect.gen(function* () {
              ...
              for (const testCase of requests) {
                const peer = yield* test.makePeer({ capabilities: { sampling: {} } });
                const request = Schema.decodeUnknownSync(McpSchema.CreateMessage.payloadSchema)(
                  testCase.request,
                );
                const exit = yield* Effect.exit(peer.reverseClient.createMessage(request));
      ...
      describe("Creating Messages", () => {
        ...
        it.effect("MUST surface sampling errors returned by the client", () =>
          Effect.gen(function* () {
            ...
            const error = yield* peer.wireClient["sampling/createMessage"](samplingRequest).pipe(
              Effect.flip,
            );
