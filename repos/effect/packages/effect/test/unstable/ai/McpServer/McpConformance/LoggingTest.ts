    ...
    describe("Logging", () => {
      ...
      describe("Setting Log Level", () => {
            ...
            const result = yield* test
              ...
              .pipe(
                Effect.flatMap((message) =>
                  Schema.decodeUnknownEffect(McpSchema.CallToolResult)(message.result),
                ),
              );
            assert.deepStrictEqual(result.content, [
              { type: "text", text: JSON.stringify("Debug") },
            ]);
          }),
        );
        it.effect("SHOULD send notifications at the selected level and higher", () =>
          Effect.gen(function* () {
            ...
            const response = yield* fixture.sendRequest(
              "logging/setLevel",
              { level: "warning" },
              2,
            );
      ...
      describe("Log Message Notifications", () => {
        ...
        it.effect("MUST allow arbitrary JSON-compatible log data", () =>
          Effect.forEach(
            ["message", 42, true, null, ["one", { nested: "two" }], { nested: { value: 1 } }],
            (data) =>
              Schema.decodeUnknownEffect(McpSchema.LoggingMessageNotification.payloadSchema)({
                level: "info",
                data,
              }).pipe(Effect.map((payload) => assert.deepStrictEqual(payload.data, data))),
          ),
        );
        it.effect("MUST emit log messages as notifications without an identifier", () =>
          Effect.gen(function* () {
            ...
            yield* Schema.decodeUnknownEffect(McpSchema.LoggingMessageNotification.payloadSchema)(
              notification.params,
            );
