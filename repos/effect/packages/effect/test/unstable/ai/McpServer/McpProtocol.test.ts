...
const makeTestProtocol = <const Version extends string, const Discriminator extends string>(
...
describe("McpProtocolRegistry", () => {
  it.effect(
    "should reject an empty declaration when runtime input bypasses the non-empty type",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "should reject duplicate versions when distinct adapters declare the same version",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "should select the first declared adapter when the offered version is unsupported",
    () =>
      Effect.gen(function* () {
        const first = makeTestProtocol("test-a", "a");
        const second = makeTestProtocol("test-b", "b");
  ...
  it.effect(
    "should preserve declaration order when the source array mutates after registry creation",
    () =>
      Effect.gen(function* () {
        const first = makeTestProtocol("test-a", "a");
        const second = makeTestProtocol("test-b", "b");
        const protocols: [
          PublicMcpProtocol.AnyProtocolAdapter,
          PublicMcpProtocol.AnyProtocolAdapter,
        ] = [first, second];
  ...
  it.effect(
    "should route to only the selected namespace when protocol schemas are incompatible",
    () =>
      Effect.gen(function* () {
        const first = makeTestProtocol("test-a", "a");
        const second = makeTestProtocol("test-b", "b");
