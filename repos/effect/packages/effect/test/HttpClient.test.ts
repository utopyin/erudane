...
const TestRoutes = HttpRouter.serve(
  HttpRouter.use(
...
const DenoHttpServerUrl = new URL("../../platform/deno/src/DenoHttpServer.ts", import.meta.url)
  .href;
const TestServerLayer = Layer.unwrap(
  Effect.promise(() =>
    "Deno" in globalThis
      ? (
          ...
          DenoHttpServer.layerServer({ hostname: "127.0.0.1", port: 0, onListen: () => {} }),
        )
      : Promise.all([import("@effect/platform-node/NodeHttpServer"), import("node:http")]).then(
          ([NodeHttpServer, Http]) => NodeHttpServer.layerServer(Http.createServer, { port: 0 }),
        ),
  ),
);
[
  ...
  it.layer(layer)(name, (it) => {
    it.effect("get", () =>
      Effect.gen(function* () {
        const response = yield* HttpClient.get("/").pipe(Effect.flatMap((_) => _.text));
    ...
    it.effect("followRedirects", () =>
      Effect.gen(function* () {
        ...
        const response = yield* client.get("/redirect").pipe(Effect.flatMap((_) => _.text));
        expect(response).toBe("test");
      }).pipe(Effect.provide(testLayer)),
    );
