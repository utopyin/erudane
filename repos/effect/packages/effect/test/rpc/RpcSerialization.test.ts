...
const codecForJsonString = (<S extends Schema.Top>(schema: S) =>
  Schema.fromJsonString(Schema.toCodecJson(schema as any))) as RpcSerialization.CodecFor;

const serialization: RpcSerialization.RpcSerialization["Service"] =
...
const Server = RpcServer.layerHttp({
  ...
  path: "/rpc",
  protocol: "http",
}).pipe(Layer.provide(Handlers), Layer.provide(layerSerialization));
...
describe("RpcSerialization", () => {
  ...
  it.effect("layerNdjsonWith forwards maxBufferSize to its decoder", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(parser.decode("34"), []);
      assertMaxBufferSizeExceeded(() => parser.decode("5"), 4);
    }).pipe(Effect.provide(RpcSerialization.layerNdjsonWith({ maxBufferSize: 4 }))),
  );
  ...
  it("jsonRpc preserves id 0 across decode and encode", () => {
    ...
    assert.strictEqual(encoded, '{"jsonrpc":"2.0","method":"users.get","params":null,"id":0}');
  });
  ...
  it("jsonRpc preserves empty string id across decode and encode", () => {
    ...
    assert.strictEqual(encoded, '{"jsonrpc":"2.0","method":"users.get","params":null,"id":""}');
  });
  ...
  it.effect("layerMsgPackWith forwards maxBufferSize to its decoder", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(parser.decode(incompleteFrame), []);
      assertMaxBufferSizeExceeded(() => parser.decode(incompleteFrame), 2);
    }).pipe(Effect.provide(RpcSerialization.layerMsgPackWith({ maxBufferSize: 2 }))),
  );
    ...
    it.effect("fills the request payload hole with the serialization's codec", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          ...
          "string",
          "the payload hole must carry the codec's output",
        );
