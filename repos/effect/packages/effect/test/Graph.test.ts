...
const assertPath = <E>(
  actual: Option.Option<Graph.PathResult<E>>,
...
const assertComponents = (
  ...
  const canonicalize = (components: ReadonlyArray<ReadonlyArray<number>>) =>
    components
      .map((component) => Array.from(component).sort((a, b) => a - b))
      .sort((a, b) => a[0] - b[0]);
...
describe("Graph", () => {
  describe("construction and protocols", () => {
    ...
    it("supports stringification, piping, and node iteration in graph order", () => {
      ...
      assert.deepStrictEqual(Array.from(graph), [
        [0, "A"],
        [1, "B"],
      ]);
  ...
  describe("snapshots", () => {
        ...
        [
          () =>
            Graph.fromSnapshot({
              ...
              nodes: [],
              edges: [],
            } as unknown as Graph.Snapshot<never, never, Graph.Kind>),
        ...
        [
          () =>
            Graph.fromSnapshot({
              ...
              nodes: [null],
              edges: [],
            } as unknown as Graph.Snapshot<never, never, "directed">),
          "Node at position 0 must be defined",
        ],
        [
          () =>
    ...
    it("rejects allocation after safe indexes are exhausted", () => {
      const nodes = Graph.beginMutation(
        Graph.fromSnapshot({
          type: "directed",
      ...
      const edges = Graph.beginMutation(
        Graph.fromSnapshot({
          type: "directed",
          nodes: [{ index: 0, data: "A" }],
      ...
      assertGraphError(
        () => Graph.addEdge(edges, 0, 0, 2),
        "Graph has exhausted safe edge indexes",
      );
    });
  });
  ...
  describe("equality and hashing", () => {
    ...
    it("uses reference equality for mutable graphs and structural equality after finalization", () => {
      ...
      assert.strictEqual(
        Equal.equals(finalized, directed(["A", "B", "C"], [[0, 1, "edge"]])),
        true,
      );
    });
    ...
    it("supports undefined node and edge payloads structurally", () => {
      const make = () =>
        directed<undefined, undefined>([undefined, undefined], [[0, 1, undefined]]);
  ...
  describe("mutation lifecycle", () => {
    ...
    it("guards read callbacks against nested mutation", () => {
      ...
      const operations: ReadonlyArray<() => unknown> = [
        ...
        () =>
          Graph.maximumFlow(mutable, {
            source: 0,
            target: 1,
            capacity: (edge) => (mutate(), edge),
          }),
        () =>
        ...
        () =>
          Array.from(
            Graph.allShortestPaths(mutable, {
              source: 0,
              target: 1,
              cost: (edge) => (mutate(), edge),
            }),
  ...
  describe("node operations", () => {
    ...
    it("updates existing payloads and ignores missing indexes", () => {
      const mutable = Graph.beginMutation(
        directed<string | undefined, never>([undefined, "B"], []),
      );
    ...
    it("finds the first and all matching nodes in graph order", () => {
      ...
      assert.deepStrictEqual(
        Graph.findNode(graph, (node) => node === undefined),
        Option.some(0),
      );
      assert.deepStrictEqual(
        Graph.findNode((node: string | undefined) => node === "missing")(graph),
        Option.none(),
      );
      assert.deepStrictEqual(
        Graph.findNodes(graph, (node) => node === undefined),
        [0, 2],
      );
    });
  });
  ...
  describe("edge operations", () => {
    ...
    it("gets and updates undefined edge data without exposing stored records", () => {
      const mutable = Graph.beginMutation(
        directed<string, number | undefined>(["A", "B"], [[0, 1, undefined]]),
      );
      ...
      assert.deepStrictEqual(
        Graph.getEdge(mutable, 0),
        Option.some({ source: 0, target: 1, data: 2 }),
      );
  ...
  describe("transformations", () => {
    ...
    it("filter-maps nodes and removes incident edges", () => {
      ...
      Graph.filterMapNodes(mutable, (node) =>
        node === 2 ? Option.none() : Option.some(node * 10),
      );
    ...
    it("filter-maps edges without removing nodes", () => {
      ...
      Graph.filterMapEdges(mutable, (edge) =>
        edge % 2 === 0 ? Option.none() : Option.some(edge * 10),
      );
      ...
      assertSnapshot(mutable, {
        ...
        edges: [
          ...
          { index: 2, source: 2, target: 0, data: 30 },
        ],
      });
    });
    ...
    it("filters edges without removing nodes", () => {
      ...
      assertSnapshot(edges, {
        ...
        edges: [
          { index: 0, source: 0, target: 1, data: 1 },
          { index: 2, source: 0, target: 2, data: 3 },
        ],
      });
    });
    ...
    it("exposes earlier bulk writes to later callbacks", () => {
      ...
      assert.deepStrictEqual(nodeStates, [
        ["a", "b"],
        ["A", "b"],
      ]);
      ...
      assert.deepStrictEqual(edgeStates, [
        [1, 2],
        [2, 2],
      ]);
    });
    ...
    it("publishes updated callback values to subsequent graph reads", () => {
      ...
      Graph.updateEdge(edges, 0, () => {
        assert.deepStrictEqual(
          Array.from(Graph.simplePaths(edges, { source: 0, target: 1 }))[0].costs,
          [1],
        );
        return 2;
      });
    ...
    it("reverses directed edges and leaves undirected stored orientation unchanged", () => {
      const directedMutable = Graph.beginMutation(
        directed(
          ["A", "B", "C"],
          [
            [0, 1, 1],
      ...
      assert.deepStrictEqual(Graph.toSnapshot(undirectedMutable).edges, [
        { index: 0, source: 1, target: 0, data: 1 },
      ]);
    });
  });
  ...
  describe("set operations", () => {
    ...
    it("composes, intersects, differs, and symmetrically differs by projected identity", () => {
      ...
      assert.deepStrictEqual(
        semantic(Graph.symmetricDifference(left(), right(), identity)),
        symmetric,
      );
      assert.deepStrictEqual(
        semantic(Graph.symmetricDifference(right(), identity)(left())),
        symmetric,
      );
    });
    ...
    it("supports Effect Equal and Hash node identities", () => {
      ...
      const result = Graph.compose(left(), right(), {
        nodeIdentity: (node) => new NodeKey(node.id),
      });
    ...
    it("coalesces duplicate identities to the last payload and redirects edges", () => {
      const graph = directed<Node, string>(
        [
          { id: "a", label: "first" },
          { id: "a", label: "last" },
        ],
      ...
      assert.deepStrictEqual(Array.from(Graph.values(Graph.nodes(result))), [
        { id: "a", label: "last" },
      ]);
    ...
    it("uses right edge payloads for custom-identity compose and intersection", () => {
      ...
      assert.strictEqual(
        Array.from(Graph.values(Graph.edges(Graph.intersection(left, right, options))))[0].data
          .label,
        "right",
      );
    });
    ...
    it("uses undefined node data as the default node identity", () => {
      assert.strictEqual(
        Graph.nodeCount(
          Graph.compose(directed<undefined, never>([undefined], []), directed([undefined], [])),
        ),
        1,
      );
    });
    ...
    it("builds directed and undirected complements without self-loops", () => {
      assertSnapshot(
        Graph.complement(directed(["A", "B"], [[0, 1, "existing"]]), (a, b) => `${a}-${b}`),
        {
          type: "directed",
          nodes: [
            { index: 0, data: "A" },
      ...
      assertSnapshot(
        Graph.complement(undirected(["A", "B", "C"], [[0, 1, "existing"]]), (a, b) => `${a}-${b}`),
        {
          type: "undirected",
          nodes: [
            { index: 0, data: "A" },
          ...
          edges: [
            { index: 0, source: 0, target: 2, data: "A-C" },
            { index: 1, source: 1, target: 2, data: "B-C" },
          ],
        },
      );
    });
    ...
    it("can ignore edge direction when selecting a neighborhood", () => {
      ...
      assertSnapshot(Graph.neighborhood(graph, 1, { radius: 2, direction: "undirected" }), {
        ...
        edges: [
          { index: 0, source: 1, target: 0, data: "AB" },
          { index: 1, source: 1, target: 2, data: "AC" },
        ],
      });
    });
  ...
  describe("queries", () => {
    ...
    it("preserves edge order when merging directed incidence and scanning undirected adjacency", () => {
      ...
      const undirectedGraph = undirected(
        [0, 1, 2],
        [
          [1, 0, "reverse"],
          [0, 0, "loop"],
    ...
    it("passes stored endpoints to edge finder predicates", () => {
      ...
      assert.deepStrictEqual(
        Graph.findEdges(graph, (_, source, target) => source > target),
        [0],
      );
    });

    it("finds undefined edge payloads", () => {
      const graph = directed<string, number | undefined>(
        ["A", "B", "C"],
        [
          [0, 1, undefined],
          [1, 2, 42],
      ...
      assert.deepStrictEqual(
        Graph.findEdge(graph, (edge) => edge === undefined),
        Option.some(0),
      );
      assert.deepStrictEqual(
        Graph.findEdges(graph, (edge) => edge === undefined),
        [0, 2],
      );
    });
  ...
  describe("serialization", () => {
    ...
    it("serializes representative directed and undirected Mermaid graphs exactly", () => {
      ...
      const directedExpected = ["flowchart TD", '  0["A"]', '  1["B"]', '  0 -->|"edge"| 1'].join(
        "\n",
      );
  ...
  describe("cycles and connectivity", () => {
    it("returns exact sparse cycle witnesses including self-loops and parallel edges", () => {
      ...
      assert.deepStrictEqual(
        Graph.findCycle(directedCycle),
        Option.some({ path: [2, 5, 9, 2], edges: [3, 7, 11] }),
      );
      assert.deepStrictEqual(
        Graph.findCycle(undirected(["A"], [[0, 0, 1]])),
        Option.some({ path: [0, 0], edges: [0] }),
      );
      assert.deepStrictEqual(
        Graph.findCycle(
          undirected(
    ...
    it("returns complete connected, weak, and strong component partitions", () => {
      ...
      assertComponents(Graph.weaklyConnectedComponents(directedGraph), [
        [0, 1, 2],
        [3, 4],
      ]);
      assertComponents(Graph.stronglyConnectedComponents(directedGraph), [[0, 1], [2], [3], [4]]);
      assertComponents(Graph.connectedComponents(Graph.beginMutation(connected)), [
        [0, 1],
        [2, 3],
        [4],
      ]);
      assertComponents(Graph.stronglyConnectedComponents(Graph.beginMutation(directedGraph)), [
        [0, 1],
        [2],
        [3],
        [4],
    ...
    it("computes reachability in outgoing, incoming, and undirected modes", () => {
      ...
      assert.deepStrictEqual(Array.from(Graph.unweightedDistances(graph, 0)), [
        [0, 0],
        [1, 1],
        [2, 2],
      ]);
      assert.deepStrictEqual(
        Array.from(Graph.unweightedDistances(graph, 2, { direction: "incoming" })),
        [
          [0, 2],
          [1, 1],
          [2, 0],
    ...
    it("rejects runtime connectivity kind mismatches and missing endpoints", () => {
      ...
      const undirectedGraph = Graph.undirected() as unknown as Graph.Graph<
        never,
        never,
        Graph.Kind
      >;
      ...
      assertGraphError(
        () =>
          Graph.maximumBipartiteMatching(
            Graph.directed() as unknown as Graph.UndirectedGraph<never, never>,
          ),
  ...
  describe("low-link connectivity", () => {
    const analyze = <N, E>(
      graph: Graph.UndirectedGraph<N, E> | Graph.MutableUndirectedGraph<N, E>,
    ) => ({
    ...
    it("rejects directed graphs at runtime", () => {
      ...
      for (const operation of [
        ...
        assertGraphError(
          () => operation(graph),
          "Cannot analyze undirected connectivity of directed graph",
        );
      }
    });
  });
  ...
  describe("flow", () => {
    ...
    it("enforces capacities, conservation, and max-flow/min-cut equality", () => {
      ...
      for (const candidate of [graph, Graph.beginMutation(graph)]) {
        ...
        assert.strictEqual(
          cut.edges.reduce((total, edge) => total + capacities[edge], 0),
          23,
        );
        assert.deepStrictEqual(Array.from(balance), [-23, 0, 0, 0, 0, 23]);
        assert.deepStrictEqual(
          [...cut.source, ...cut.target].sort((a, b) => a - b),
          [0, 1, 2, 3, 4, 5],
        );
      }
    });
    ...
    it("uses reverse residual edges to reroute flow", () => {
      ...
      assert.strictEqual(
        Graph.maximumFlow(graph, { source: 0, target: 5, capacity: (edge) => edge }).value,
        2,
      );
    });
    ...
    it("handles parallel edges, self-loops, fractions, and disconnected targets", () => {
      ...
      assert.deepStrictEqual(
        Graph.maximumFlow(graph, { source: 0, target: 3, capacity: (edge) => edge }),
        {
          value: 0,
          flows: new Map([
            [0, 0],
    ...
    it("rejects invalid graphs, endpoints, capacities, and finite-range overflow", () => {
      ...
      const overflow = directed<void, number>(
        [undefined, undefined],
        [
  ...
  describe("spanning forests and reductions", () => {
    it("preserves sparse indexes, isolated nodes, and fixed mutable parity in minimum spanning forests", () => {
      ...
      assertSnapshot(
        Graph.minimumSpanningForest(Graph.beginMutation(graph), (edge) => edge),
        expected,
      );
    });
    ...
    it("breaks equal spanning-forest weights by first edge order", () => {
      ...
      assert.deepStrictEqual(
        Graph.toSnapshot(Graph.minimumSpanningForest(graph, (edge) => edge)).edges.map(
          (edge) => edge.index,
        ),
        [0, 1],
      );
      ...
      assertGraphError(
        () =>
          Graph.transitiveReduction(
            Graph.undirected() as unknown as Graph.DirectedGraph<never, never>,
          ),
  ...
  describe("pathfinding", () => {
    ...
    it("handles decreased Dijkstra priorities with fresh order and stale entries", () => {
      const reordered = directed(
        ["source", "improved", "shortcut", "direct", "target"],
        [
          [0, 1, 10],
          [0, 2, 1],
          [0, 3, 2],
      ...
      const stale = directed(
        ["source", "improved", "shortcut", "middle", "target"],
        [
          [0, 1, 10],
          [0, 2, 1],
          [2, 1, 1],
    ...
    it("skips stale A* entries and does not reopen closed nodes", () => {
      ...
      assertPath(
        Graph.astar(stale, { source: 0, target: 4, cost: (edge) => edge, heuristic: () => 0 }),
        {
          path: [0, 2, 1, 3, 4],
          edges: [1, 2, 3, 4],
          distance: 42,
      ...
      const closed = directed(
        ["source", "closed", "later", "target"],
        [
          [0, 1, 10],
          [1, 3, 1],
          [0, 2, 1],
    ...
    it("preserves sparse parallel edge identity across all shortest-path algorithms", () => {
      ...
      for (const candidate of [graph, Graph.beginMutation(graph)]) {
        assertPath(
          Graph.dijkstra(candidate, { source: 2, target: 5, cost: (edge) => edge }),
          expected,
        );
        assertPath(
        ...
        assertPath(
          Graph.bellmanFord(candidate, { source: 2, target: 5, cost: (edge) => edge }),
          expected,
        );
    ...
    it("handles unreachable and same-node paths completely", () => {
      ...
      assertPath(
        Graph.astar(graph, { source: 0, target: 0, cost: (edge) => edge, heuristic: () => 0 }),
        expected,
      );
      assertPath(
    ...
    it("ignores negative cycles that cannot affect the Bellman-Ford target", () => {
      ...
      assertPath(
        Graph.bellmanFord(graph, { source: 0, target: 3, cost: (edge) => edge }),
        expected,
      );
      assertPath(
        Graph.bellmanFord(Graph.beginMutation(graph), {
          source: 0,
          target: 3,
          cost: (edge) => edge,
        }),
    ...
    it("preserves null edge payloads in Floyd-Warshall multihop paths", () => {
      ...
      const graph = directed<string, Edge>(
        ["A", "B", "C"],
        [
          [0, 1, null],
          [1, 2, null],
    ...
    it("reports relevant Bellman-Ford and Floyd-Warshall negative cycles", () => {
      ...
      assertGraphError(
        () => Graph.floydWarshall(cycle, (edge) => edge),
    ...
    it("validates negative Dijkstra and A* weights before early returns", () => {
      ...
      for (const [graph, source, target] of [
        [early, 0, 1],
        [same, 0, 0],
      ] as const) {
        assertGraphError(
    ...
    it("traverses undirected edges against stored orientation", () => {
      ...
      assertPath(
        Graph.astar(graph, { source: 0, target: 2, cost: (edge) => edge, heuristic: () => 0 }),
        expected,
      );
      assertPath(
  ...
  describe("path enumeration", () => {
    it("lazily and repeatably enumerates complete simple paths in edge order", () => {
      ...
      assert.deepStrictEqual(Array.from(paths), expected);
      assert.deepStrictEqual(
        Array.from(Graph.simplePaths({ source: 0, target: 3, limit: 3 })(graph)),
        expected,
      );
    });
    ...
    it("enumerates parallel and structurally tied shortest paths exactly", () => {
      ...
      assert.deepStrictEqual(
        Array.from(
          Graph.allShortestPaths({ source: 0, target: 3, cost: (edge: number) => edge })(graph),
        ),
        expected,
      );
      assert.deepStrictEqual(
        Array.from(
          Graph.allShortestPaths(Graph.beginMutation(graph), {
            source: 0,
            target: 3,
            cost: (edge) => edge,
            limit: 2,
    ...
    it("defers shortest-path cost evaluation until iteration", () => {
      ...
      assert.deepStrictEqual(Array.from(paths), [
        { path: [0, 1], edges: [0], distance: 1, costs: [1] },
      ]);
    ...
    it("revalidates mutable endpoints and isolates active snapshots", () => {
      ...
      const shortestMutable = Graph.beginMutation(
        directed(
          [0, 1, 2, 3],
          [
            [0, 1, 1],
      ...
      const shortest = Graph.allShortestPaths(shortestMutable, {
        source: 0,
        target: 3,
        cost: (edge) => edge,
      });
    ...
    it("validates limits, endpoints, and non-negative shortest-path costs", () => {
      ...
      assertGraphError(
        () =>
          Array.from(
            Graph.allShortestPaths(graph, { source: 0, target: 1, cost: (edge) => edge, limit: 0 }),
          ),
        "All shortest paths requires non-negative edge weights",
      );
      const overflow = directed(
      ...
      assertGraphError(
        () =>
          Array.from(
            Graph.allShortestPaths(overflow, { source: 0, target: 2, cost: (edge) => edge }),
          ),
      ...
      assertGraphError(
        () => Graph.simplePaths(graph, { source: 0, target: 2 }),
        "Node 2 does not exist",
      );
      assert.deepStrictEqual(
        Array.from(Graph.simplePaths(graph, { source: 0, target: 1, limit: 0 })),
        [],
      );
    });
  });
  ...
  describe("traversal", () => {
    ...
    it("preserves postorder with parallel edges, self-loops, and directionless traversal", () => {
      ...
      assertIndices(
        Graph.dfsPostOrder(directionless, { start: [1], direction: "undirected" }),
        [3, 0, 2, 1],
      );
    });
    ...
    it("uses shortest distance for radius membership and supports traversal directions", () => {
      ...
      assertIndices(
        Graph.bfs(graph, { start: [1], direction: "undirected", radius: 1 }),
        [1, 2, 0],
      );
      assertIndices(
        Graph.dfsPostOrder(
    ...
    it("preserves bounded undirected DFS order for reciprocal neighbors", () => {
      const graph = directed<void, number>(new Array(5).fill(undefined), [
        [0, 1, 1],
        [4, 1, 2],
        [1, 4, 3],
      ]);
      ...
      assertIndices(
        Graph.dfs(graph, { start: [1], direction: "undirected", radius: 2 }),
        [1, 4, 0],
      );
    });
    ...
    it("copies starts, prioritizes distinct roots, and preserves sparse indexes", () => {
      ...
      const walkers = [
        ...
        Graph.dfsPostOrder(graph, { start }),
      ];
      start[0] = 3;
    ...
    it("revalidates missing starts for fresh DFS, BFS, and postorder iterators", () => {
      ...
      for (const walker of walkers)
        assertGraphError(() => walker[Symbol.iterator](), "Node 0 does not exist");
    });
    ...
    it("rejects invalid topological graphs and initials", () => {
      ...
      const graph = directed([0, 1], [[0, 1, 1]]);
      assertGraphError(
        () => Array.from(Graph.topo(graph, { initials: [1] })),
  ...
  describe("Walker", () => {
    it("projects representative walkers through indices, values, entries, and visit", () => {
      ...
      assert.deepStrictEqual(Array.from(walker.visit((index, value) => `${index}:${value}`)), [
        "2:A",
        "5:B",
      ]);
    });
    ...
    it("preserves receivers supplied by iterable iterator methods", () => {
      const walker = new Graph.Walker<number, string>(
        (visit) => new Set([visit(0, "A"), visit(1, "B")]),
      );
      assert.deepStrictEqual(Array.from(walker), [
        [0, "A"],
        [1, "B"],
