...
describe("SchemaParser", () => {
  ...
  describe("decodeUnknownExit", () => {
    ...
    it("reads property and index values once", () => {
      ...
      const property = SchemaParser.decodeUnknownExit(Schema.Struct({ value: Schema.String }))(
        propertyInput,
      );
      ...
      const index = SchemaParser.decodeUnknownExit(Schema.Record(Schema.String, Schema.String))(
        indexInput,
      );
      ...
      let rejectedReads = 0;
      const rejectedKey = Schema.String.pipe(
        Schema.decode({
      ...
      const rejected = SchemaParser.decodeUnknownExit(Schema.Record(rejectedKey, Schema.String))(
        rejectedInput,
      );
    ...
    it("rejects a missing root output after parsing structural schemas", () => {
      ...
      const struct = Schema.Struct({ value: Schema.String }).pipe(
        Schema.decode({
          decode: missing,
          encode: missing,
        }),
    ...
    it("stops outer checks after an aborting check in a FilterGroup", () => {
      let innerRuns = 0;
      let outerRuns = 0;
      const group = Schema.makeFilter(() => false)
        .abort()
        .and(
          Schema.makeFilter(() => {
            innerRuns++;
    ...
    it.effect("wraps an asynchronous failure from a uniquely selected union member", () =>
      Effect.gen(function* () {
        const failing = Schema.String.pipe(
          Schema.decode({
            decode: new SchemaGetter.Getter(() =>
              Effect.yieldNow.pipe(Effect.andThen(Effect.fail(new SchemaIssue.InvalidValue()))),
            ),
            encode: SchemaGetter.passthrough(),
          }),
        );
    ...
    it.effect("resolves an unchanged concurrent union candidate", () =>
      Effect.gen(function* () {
        const delayedFailure = Schema.String.pipe(
          Schema.decode({
            decode: new SchemaGetter.Getter(() =>
              Effect.yieldNow.pipe(Effect.andThen(Effect.fail(new SchemaIssue.InvalidValue()))),
            ),
        ...
        Schema.String.pipe(
          Schema.decode({
            decode: new SchemaGetter.Getter((input) => {
              calls.push(name);
              return suspended
                ? Effect.suspend(() => Effect.succeed(input))
    ...
    it("supports mutually recursive Suspend parsers", () => {
      ...
      strictEqual(
        decode({ _tag: "A", next: { _tag: "B", next: { _tag: "A", next: "end" } } })._tag,
        "Success",
      );
    });
    ...
    it("keeps untried Declaration union members lazy", () => {
      ...
      const decode = SchemaParser.decodeUnknownExit(
        Schema.Union([Schema.Literal("a"), declaration]),
      );
    ...
    it("should preserve mixed causes in union candidates instead of trying later candidates", () => {
      const failure = Schema.String.pipe(
        Schema.decode({
          decode: new SchemaGetter.Getter(() => Effect.failCause(makeMixedCause())),
          encode: SchemaGetter.passthrough(),
        }),
      );
      const schema = Schema.Union([failure, Schema.Literal("a")]);
