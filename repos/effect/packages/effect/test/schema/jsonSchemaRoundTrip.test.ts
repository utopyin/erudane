...
function assertJsonSchemaImportRoundTrip(
  ...
  const decode = Schema.decodeUnknownExit(
    imported as unknown as Schema.ConstraintDecoder<unknown>,
    {
...
function assertRepresentationRoundTrip(
  ...
  const decodeImported = Schema.decodeUnknownExit(
    imported as unknown as Schema.ConstraintDecoder<unknown>,
    {
      onExcessProperty: "error",
    },
...
describe("JSON Schema round-trip laws", () => {
  describe("toJsonSchema(fromJsonSchema(A))", () => {
    it("preserves empty closed objects", () => {
      assertJsonSchemaImportRoundTrip({ type: "object", additionalProperties: false }, [
        {},
        { a: 1 },
        [],
        [1],
  ...
  describe("fromJsonSchema(toJsonSchema(X))", () => {
    it("preserves structs", () => {
      assertRepresentationRoundTrip(Schema.Struct({ a: Schema.String }), [
        {},
        { a: "a" },
        { a: 1 },
        { a: "a", b: 1 },
