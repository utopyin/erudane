...
describe("SchemaAST", () => {
  ...
  describe("getCandidates", () => {
    ...
    it("should handle function-valued declarations with sentinels", () => {
      const a = Schema.declare((input): input is () => void => typeof input === "function", {
        "~sentinels": [{ key: "kind", literal: "a" }],
      });
      const b = Schema.declare((input): input is () => void => typeof input === "function", {
    ...
    it("should preserve duplicate candidates with a common discriminator", () => {
      ...
      const ast = schema.ast;
      deepStrictEqual(SchemaAST.getCandidates({ kind: "a" }, ast.types), [
        ast.types[0],
        ast.types[2],
      ]);
    ...
    it("should exclude members whose sentinel the input contradicts", () => {
      const schema = Schema.Union([
        Schema.Struct({
          kind: Schema.Literal("a"),
          variant: Schema.Literal("x"),
          value: Schema.String,
        }),
        Schema.Struct({
          kind: Schema.Literal("a"),
          variant: Schema.Literal("y"),
          value: Schema.Number,
        }),
      ]);
      const ast = schema.ast;
      deepStrictEqual(SchemaAST.getCandidates({ kind: "a", variant: "x" }, ast.types), [
        ast.types[0],
      ]);
      ...
      deepStrictEqual(
        SchemaAST.getCandidates({ kind: "a", variant: undefined }, ast.types, true),
        ast.types,
      );
    });
  });
  ...
  describe("IndexSignature", () => {
    it("accepts valid parameters on both type and encoded side", () => {
      ...
      doesNotThrow(
        () => new SchemaAST.IndexSignature(Schema.NumberFromString.ast, Schema.Number.ast),
      );
      doesNotThrow(
        () =>
