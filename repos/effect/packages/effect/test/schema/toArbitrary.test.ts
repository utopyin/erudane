...
describe("Arbitrary generation", () => {
  ...
  it("should enforce opaque filters", () => {
    ...
    FastCheck.assert(
      FastCheck.property(arbitrary, (a) => a.a.length > 0),
      { numRuns: 5 },
    );
  });
  ...
  describe("candidates", () => {
    it("should use filter candidates with the merged constraint context", () => {
      ...
      FastCheck.assert(
        FastCheck.property(arbitrary, (s) => s === "candidate"),
        { numRuns: 20 },
      );
    });
    ...
    it("should use filter group candidates", () => {
      ...
      FastCheck.assert(
        FastCheck.property(arbitrary, (s) => s.startsWith("a") && s.endsWith("a")),
        { numRuns: 20 },
      );
    });
  ...
  describe("object property counts", () => {
    ...
    it("enforces minProperties with symbol optional keys", () => {
      ...
      FastCheck.assert(
        FastCheck.property(
          toArbitrary(schema),
          (o) => globalThis.Reflect.ownKeys(o).length >= 2 && globalThis.Object.hasOwn(o, key),
        ),
        { numRuns: 100 },
      );
      verifyGeneration(schema);
