...
describe("Serializers", () => {
  describe("toCodecJson", () => {
    ...
    it("is idempotent", () => {
      const once = Schema.toCodecJson(
        Schema.suspend(() => Schema.Struct({ value: Schema.Number })),
      );
    ...
    describe("schemas without encoding", () => {
      ...
      describe("Declaration", () => {
        it("instanceOf without annotations", async () => {
          ...
          await asserts.encoding().fail(new URL("https://example.com"), "Expected JSON value");
      ...
      it("Literals", async () => {
        ...
        const decoding = asserts.decoding();
        await decoding.fail("-", `Expected "a" | 1 | "2" | true`);
      });
      ...
      it("URL", async () => {
        ...
        await decoding.succeed("https://example.com/", new URL("https://example.com"));
        await decoding.fail("not a url", "Expected a valid URL string");
      });
      ...
      it("RegExp", async () => {
        ...
        await decoding.fail({ source: "(", flags: "" }, "Expected valid RegExp source and flags");
        await decoding.fail({ source: "a", flags: "x" }, "Expected valid RegExp source and flags");
      });
      ...
      it("Uint8Array", async () => {
        ...
        await decoding.fail("not a base64 string", "Expected a valid Base64 string");
      });
  ...
  describe("toCodecStringTree", () => {
    ...
    describe("should memoize the result", () => {
      ...
      it("Suspend", () => {
        const serializer = Schema.toCodecStringTree(
          Schema.suspend(() => Schema.Array(Schema.Finite)),
        );
        strictEqual(serializer.ast, Schema.toCodecStringTree(serializer).ast);
      });
    });
    ...
    describe("schemas without encoding", () => {
      ...
      it("Declaration", () => {
        const schema = Schema.instanceOf(URL);
        throws(() => Schema.toCodecStringTree(schema), "Missing structural codec for StringTree");
      });
    ...
    it("preserves union fallback when a singleton fails array element checks", async () => {
      const schema = Schema.toCodecArrayFromSingle(
        Schema.toCodecStringTree(
          Schema.Union([Schema.Array(Schema.String.check(Schema.isMinLength(2))), Schema.String]),
        ),
      );
