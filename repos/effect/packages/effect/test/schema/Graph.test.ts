...
const undirectedCodec = Schema.toCodecJson(
  Schema.Graph("undirected", Schema.String, Schema.Number),
);
...
describe("Schema.Graph", () => {
  it("derives valid arbitrary graphs", () => {
    new TestSchema.Asserts(Schema.Graph("directed", Schema.String, Schema.Number))
      .arbitrary()
      .verifyGeneration();
    new TestSchema.Asserts(Schema.Graph("undirected", Schema.String, Schema.Number))
      .arbitrary()
      .verifyGeneration();
  });
  ...
  it("derives active-structure equivalence from payload schemas", () => {
    ...
    const undirectedEquivalence = Schema.toEquivalence(
      Schema.Graph("undirected", Schema.Number, Schema.Number),
    );
  ...
  it("encodes and decodes empty directed and undirected graphs", () => {
    ...
    assert.deepStrictEqual(
      Schema.encodeSync(undirectedCodec)(Graph.undirected()),
      empty("undirected"),
    );
    assert.strictEqual(decodeDirected(empty("directed")).type, "directed");
    assert.strictEqual(
      Schema.decodeUnknownSync(undirectedCodec)(empty("undirected")).type,
      "undirected",
    );
  });
  ...
  it("preserves indexed directed structure, isolated nodes, loops, parallel edges, and gaps", () => {
    ...
    assert.deepStrictEqual(
      Array.from(decoded),
      encoded.nodes.map(({ index, data }) => [index, data] as const),
    );
    assert.deepStrictEqual(
  ...
  it("applies node and edge payload transformations", () => {
    const codec = Schema.toCodecJson(
      Schema.Graph("directed", Schema.FiniteFromString, Schema.FiniteFromString),
    );
    const encoded = {
      type: "directed",
      nodes: [
