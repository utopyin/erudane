...
const InnerCategory = Schema.Struct({
  ...
  children: Schema.Array(
    Schema.suspend((): Schema.Codec<Category> =>
...
describe("toCodeDocument", () => {
  ...
  const templateType = (...types: ReadonlyArray<string>) =>
  ...
  describe("Declaration", () => {
    ...
    it("Error with stack", () => {
      assertSchema(
        { schema: Schema.ErrorInstance({ includeStack: true }) },
        {
    ...
    it("Error with excluded cause", () => {
      assertSchema(
        { schema: Schema.ErrorInstance({ excludeCause: true }) },
        {
    ...
    describe("checks", () => {
      ...
      it("isGUID with annotations", () => {
        assertSchema(
          ...
          {
            codes: makeCode(
              `Schema.String.check(Schema.isGUID().annotate({ "message": "message" }))`,
              "string",
            ),
          },
        );
  ...
  describe("Enum", () => {
    it("string values", () => {
      ...
      assertSchema(
        ...
        {
          codes: makeCode(`Schema.Enum(_Enum).annotate({ "description": "a" })`, `_Enum`),
          artifacts: [
            {
              _tag: "Enum",
              identifier: "_Enum",
        ...
        },
        {
          ...
          references: {
            nonRecursives: [
              {
                $ref: "A",
                code: makeCode(`Schema.String.annotate({ "identifier": "A" })`, "string"),
              },
            ],
          },
        },
