...
describe("SchemaRepresentation.fromJson", () => {
  ...
  it("does not coerce strings resembling symbols", () => {
    const input = {
      representation: {
        _tag: "Literal",
        literal: { type: "string", value: "Symbol(a)" },
        checks: [],
      },
      references: {},
    } as const;
  ...
  it("rejects non-JSON roots", () => {
    ...
    throws(() => SchemaRepresentation.fromJson(input as never), `Expected object`);
  });
  ...
  it("does not invoke toJSON", () => {
    ...
    assert.deepStrictEqual(SchemaRepresentation.fromJson(input as never), {
