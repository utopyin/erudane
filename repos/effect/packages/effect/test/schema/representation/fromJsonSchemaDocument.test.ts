...
function fromJsonSchemaRepresentation(
  ...
  return SchemaRepresentation.toRepresentation(
    toSchemaFromJsonSchemaDocument(document, options).ast,
  );
}
  ...
  describe("$ref", () => {
    ...
    it("reports the full reference when a direct definition is missing", () => {
      throws(
        () =>
          toSchemaFromJsonSchemaDocument(
            JsonSchema.fromSchemaDraft2020_12({ $ref: "#/$defs/Missing" }),
          ),
        ...
        assertFromJsonSchema(
          {
            schema: {
              type: "number",
              allOf: [{ minimum: 1, maximum: 2, description: "range" }, { type: "integer" }],
            },
          },
          {
            representation: {
    ...
    describe("patterns", () => {
      ...
      it("ignores pattern property value constraints explicitly", () => {
        ...
        if (representation._tag === "Objects") {
          deepStrictEqual(
            representation.indexSignatures.map(({ parameter }) => parameter),
            [
              {
                _tag: "String",
                checks: [],
