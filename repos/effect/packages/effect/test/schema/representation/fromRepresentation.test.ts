...
function assertRepresentationRoundtrip(
  ...
  const revived = SchemaRepresentation.fromRepresentation(SchemaRepresentation.fromJson(expected), {
    revivers,
  });
...
describe("SchemaRepresentation.fromRepresentation", () => {
  ...
  it("revives Literal without changing its type", () => {
    for (const [literal, differentType] of [
      ["1", 1n],
      ["true", true],
      [1, "1"],
      [1n, "1"],
  ...
  it("revives a reference used as a Record key", () => {
    ...
    assert.deepStrictEqual(Schema.decodeUnknownSync(schema as Schema.Codec<unknown>)({ a: 1 }), {
      a: 1,
    });
  });
  ...
  it("revives StructWithRest", () => {
    assertRepresentationRoundtrip(
      Schema.StructWithRest(Schema.Struct({ value: Schema.Number }), [
        Schema.Record(Schema.Symbol, Schema.String),
      ]),
    );
  });
  ...
  it("revives Suspend", () => {
    ...
    assert.strictEqual(schema.ast._tag, "Objects");
    if (schema.ast._tag === "Objects") {
      const children = schema.ast.propertySignatures.find(
    ...
    assert.strictEqual(
      SchemaRepresentation.toRepresentation(schema.ast).representation._tag,
      "Reference",
    );
  });
  ...
  it("restores node annotations before checks", () => {
    const schema = assertRepresentationRoundtrip(
      Schema.String.annotate({ title: "node" }).check(minLengthCheck(2, { description: "check" })),
      [minLengthReviver],
    );
  ...
  it("restores property annotations", () => {
    const schema = revive(
      Schema.Struct({ value: Schema.String.annotateKey({ description: "property" }) }),
    );
    const representation = SchemaRepresentation.toRepresentation(schema.ast).representation;
  ...
  it("revives a Filter", () => {
    ...
    assert.strictEqual(
      Schema.decodeUnknownResult(schema as Schema.Codec<unknown>)("a")._tag,
      "Failure",
    );
  ...
  it("revives a FilterGroup without an identity from its children", () => {
    const group = Schema.makeFilterGroup([minLengthCheck(2), minLengthCheck(3)], {
      description: "both",
    });
    const schema = assertRepresentationRoundtrip(Schema.String.check(group), [minLengthReviver]);
    assert.strictEqual(
      Schema.decodeUnknownResult(schema as Schema.Codec<unknown>)("ab")._tag,
      "Failure",
    );
  ...
  it("uses an identified FilterGroup reviver instead of its persisted children", () => {
    ...
    const reviver: SchemaRepresentation.FilterGroupReviver<null> = {
      ...
      revive: () =>
        Schema.makeFilterGroup([Schema.makeFilter<string>((value) => value !== "blocked")]),
    };
    const schema = SchemaRepresentation.fromRepresentation(document, {
      revivers: [reviver],
