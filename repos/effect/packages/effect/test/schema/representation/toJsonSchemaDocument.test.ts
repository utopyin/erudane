...
describe("SchemaRepresentation.toJsonSchemaDocument", () => {
  ...
  describe("representation nodes", () => {
    const keywords = [
      ...
      [
        "UniqueSymbol",
        { _tag: "UniqueSymbol", symbol: Symbol.for("value"), checks: [] },
        {
          type: "string",
          allOf: [{ pattern: "^Symbol\\((.*)\\)$" }],
        },
    ...
    it("compiles a Never index-signature value as false", () => {
      assert.deepStrictEqual(
        compile({
          ...
          propertySignatures: [],
          indexSignatures: [
            { parameter: StringRepresentation, type: { _tag: "Never", checks: [] } },
          ],
          checks: [],
        }),
    ...
    it("removes an unconstrained index-signature value", () => {
      assert.deepStrictEqual(
        compile({
          ...
          propertySignatures: [],
          indexSignatures: [
            { parameter: StringRepresentation, type: { _tag: "Unknown", checks: [] } },
          ],
          checks: [],
        }),
        { type: "object" },
    ...
    it("compiles the isPattern vertical slice", () => {
      const pattern = SchemaRepresentation.toJsonSchemaDocument(
        SchemaRepresentation.toRepresentation(
          Schema.String.check(Schema.isPattern(/^[a-z]+$/i)).ast,
        ),
      );
    ...
    it("compiles all supported template-literal parts and rejects other nodes", () => {
      const representation: SchemaRepresentation.Representation = {
        ...
        parts: [
          ...
          {
            _tag: "TemplateLiteral",
            parts: [{ _tag: "Literal", literal: "x", checks: [] }, StringRepresentation],
            checks: [],
          },
          {
            _tag: "Union",
    ...
    describe("conservative check compaction", () => {
      ...
      it("keeps a reference separate from contextual annotations", () => {
        ...
        assert.deepStrictEqual(document.schema, {
          ...
          properties: {
            value: {
              allOf: [{ $ref: "#/$defs/Value" }, { description: "contextual" }],
            },
          },
          required: ["value"],
          additionalProperties: false,
      ...
      const coupledKeywords = [
        ["contains and minContains", { contains: { const: "a" } }, { minContains: 2 }],
        [
          "properties and additionalProperties",
          { properties: { a: true } },
          { additionalProperties: false },
        ],
        ["prefixItems and items", { prefixItems: [true] }, { items: false }],
        ["if and else", { if: { required: ["a"] } }, { else: { required: ["b"] } }],
  ...
  describe("safety and errors", () => {
    ...
    it("captures exceptions from JSON Schema callbacks", () => {
      ...
      expectError(() => SchemaRepresentation.toJsonSchemaDocument(document), cause);
    });
