...
describe("SchemaRepresentation.toRepresentation", () => {
  describe("node conversion", () => {
    ...
    it("global unique symbol", () => {
      ...
      assert.deepStrictEqual(
        SchemaRepresentation.toRepresentation(Schema.UniqueSymbol(symbol).ast),
        {
    ...
    it("enum", () => {
      assert.deepStrictEqual(
        SchemaRepresentation.toRepresentation(Schema.Enum({ A: "a", One: 1 }).ast),
        {
          representation: {
            _tag: "Enum",
            enums: [
    ...
    it("template literal", () => {
      assert.deepStrictEqual(
        SchemaRepresentation.toRepresentation(
          Schema.TemplateLiteral(["prefix-", Schema.String, Schema.Number]).ast,
        ),
        {
          representation: {
    ...
    it("union member order", () => {
      assert.deepStrictEqual(
        SchemaRepresentation.toRepresentation(Schema.Union([Schema.String, Schema.BigInt]).ast),
        {
          representation: {
            _tag: "Union",
            types: [
  ...
  describe("schema annotations and declarations", () => {
    it("preserves brands", () => {
      assert.deepStrictEqual(
        SchemaRepresentation.toRepresentation(
          Schema.String.pipe(Schema.brand("A"), Schema.brand("B")).ast,
        ),
        {
          representation: {
            _tag: "String",
    ...
    it("converts declaration type parameters", () => {
      const representation = SchemaRepresentation.toRepresentation(
        Schema.Option(Schema.Number).ast,
      ).representation;
  ...
  describe("checks", () => {
    it("preserves custom filter callbacks, dependencies and aborted state", () => {
      ...
      assert.deepStrictEqual(
        SchemaRepresentation.toRepresentation(Schema.String.check(filter).ast),
        {
          representation: {
            _tag: "String",
            checks: [
    ...
    it("preserves filter groups", () => {
      ...
      assert.deepStrictEqual(
        SchemaRepresentation.toRepresentation(Schema.String.check(group).ast),
        {
          representation: {
            _tag: "String",
            checks: [
  ...
  describe("contextual annotations", () => {
    it("preserves fromJsonString annotations", () => {
      const schema = SchemaAST.toEncoded(
        Schema.fromJsonString(Schema.Struct({ value: Schema.Number })).ast,
      );
  ...
  describe("reference policies", () => {
    it("extracts only candidates with identifiers by default", () => {
      ...
      const identified = Schema.Struct({ value: Schema.String }).annotate({
      ...
      if (document.representation._tag === "Arrays") {
        assert.deepStrictEqual(
          document.representation.elements.map((element) => element.type._tag),
          ["Objects", "Reference"],
        );
      }
    });
  ...
  describe("reference allocation and recursion", () => {
    ...
    it("converts a non-recursive suspend", () => {
      assert.deepStrictEqual(
        SchemaRepresentation.toRepresentation(Schema.suspend(() => Schema.String).ast),
        {
          representation: {
            _tag: "Suspend",
    ...
    it("does not resolve an identifier below a check", () => {
      const schema = Schema.String.annotate({ identifier: "Text" }).pipe(
      ...
      if (document.representation._tag === "String") {
        ...
        assert.strictEqual(
          document.representation.checks[0].representation?.id,
    ...
    it("reuses a Class identifier across repeated type-side occurrences", () => {
      ...
      const document = SchemaRepresentation.toRepresentation(
        SchemaAST.toType(Schema.Tuple([User, User]).ast),
      );
