...
describe("Context", () => {
  ...
  it("invalidates the fiber cache only for opted-in keys", () => {
    ...
    const context = source.pipe(Context.add(Cached, 2), Context.add(CachedClass, 3));
    ...
    deepStrictEqual(
      [...Context.omit(Cached)(replaced).mapUnsafe],
      [
        [A.key, 1],
        [CachedClass.key, 3],
      ],
  ...
  it("distinguishes an undefined service from an absent service", () => {
    ...
    strictEqual(
      Context.getOrElse(context, Undefined, () => 1),
      undefined,
    );
  ...
  it("bounds deep overlay chains without changing values or order", () => {
    const keys = Array.from({ length: 20 }, (_, i) =>
      Context.Service<number>(`ContextTest/Deep${i}`),
    );
  ...
  it("supports equality and JSON materialization", () => {
    ...
    const right = Context.makeUnsafe(
      new Map([
        [A.key, 1],
        [B.key, 2],
      ]),
  ...
  it("pick and omit retain source ordering", () => {
    ...
    deepStrictEqual(
      [...Context.pick(C, A)(source).mapUnsafe],
      [
        [A.key, 1],
        [C.key, 3],
      ],
    ...
    deepStrictEqual(
      [...Context.omit(B)(source).mapUnsafe],
      [
        [A.key, 1],
        [C.key, 3],
      ],
