...
describe("Struct", () => {
  ...
  describe("evolve", () => {
    ...
    it("all required fields", () => {
      const s = { a: "a", b: 1 };
      deepStrictEqual(pipe(s, Struct.evolve({ a: (s) => s.length, b: (b) => b > 0 })), {
        a: 1,
        b: true,
      });
  ...
  it("evolveKeys", () => {
    deepStrictEqual(pipe({ a: "a", b: 2 }, Struct.evolveKeys({ a: (k) => Str.toUpperCase(k) })), {
      A: "a",
      b: 2,
    });
    deepStrictEqual(Struct.evolveKeys({ a: "a", b: 2 }, { a: (k) => Str.toUpperCase(k) }), {
      A: "a",
      b: 2,
    });
  });
  ...
  it("renameKeys", () => {
    deepStrictEqual(pipe({ a: "a", b: 1, c: true }, Struct.renameKeys({ a: "A", b: "B" })), {
      A: "a",
      B: 1,
      c: true,
    });
    deepStrictEqual(Struct.renameKeys({ a: "a", b: 1, c: true }, { a: "A", b: "B" }), {
      A: "a",
      B: 1,
      c: true,
    });
    ...
    deepStrictEqual(pipe({ [from]: "a", b: 1 }, Struct.renameKeys({ [from]: to })), {
      [to]: "a",
      b: 1,
    });
  ...
  it("evolveEntries transforms a selected key and value together", () => {
    ...
    deepStrictEqual(
      Struct.evolveEntries({ a: "a", b: 2 }, { a: (k, v) => [Str.toUpperCase(k), v.length] }),
      {
        A: 1,
        b: 2,
      },
