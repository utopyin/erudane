...
describe("Formatter", () => {
  describe("format", () => {
    ...
    it("preserves repeated non-circular references", () => {
      ...
      strictEqual(
        format({ first: shared, second: shared }),
  ...
  "b": [Circular]
    ...
    it("preserves formatting options for redacted representations", () => {
      ...
      assertTrue(
        format(value, { space: 2, ignoreToString: true }).startsWith(`{\n  "a": 1,\n  "b": 2,`),
      );
  ...
  describe("Inspectable.toJson", () => {
    it("redacts before toJSON", () => {
      deepStrictEqual(Inspectable.toJson(data), { secret: "[REDACTED]" });
    });
  ...
  describe("SchemaIssue reportInput", () => {
    ...
    it("distinguishes undefined input from missing input", () => {
      const undefinedResult = SchemaParser.decodeUnknownResult(Schema.String)(undefined, {
        reportInput: true,
      });
      ...
      const missingResult = SchemaParser.decodeUnknownResult(
    ...
    it("reports local inputs for structs", () => {
      ...
      const result = SchemaParser.decodeUnknownResult(Schema.Struct({ value: Schema.Number }))(
        input,
        {
          errors: "all",
    ...
    it("reports input for union wrappers", () => {
      const anyOf = SchemaParser.decodeUnknownResult(
    ...
    it("respects annotated parse options", () => {
      ...
      const nestedResult = SchemaParser.decodeUnknownResult(nestedDisabled)(nestedInput, {
        reportInput: true,
      });
    ...
    it.effect("distinguishes present undefined from absent input in forbidden", () =>
      Effect.gen(function* () {
        ...
        const present = yield* getter
          .run(Option.some(undefined), { reportInput: true })
          .pipe(Effect.flip);
    ...
    it("formats reported inputs with the historical templates", () => {
      ...
      strictEqual(
        formatIssue(new SchemaIssue.InvalidType(Schema.String.ast, 1, options)),
        "Expected string, got 1",
      );
      strictEqual(
        formatIssue(new SchemaIssue.InvalidValue(undefined, 1, options)),
        "Invalid data 1",
      );
      strictEqual(
      ...
      strictEqual(
        formatIssue(new SchemaIssue.Forbidden(undefined, 1, options)),
        "Forbidden operation",
      );
      strictEqual(
      ...
      strictEqual(
        formatIssue(new SchemaIssue.AnyOf(union, [], 1, options)),
        "Expected string | number, got 1",
      );
    });
    ...
    it("uses expected annotations only for InvalidValue", () => {
      strictEqual(
        formatIssue(new SchemaIssue.InvalidValue({ expected: "a valid value" })),
        "Expected a valid value",
      );
      strictEqual(
        formatIssue(
          new SchemaIssue.InvalidValue({ expected: "a valid value" }, "secret", {
            reportInput: true,
          }),
        ),
      ...
      strictEqual(
        formatIssue(
          new SchemaIssue.Forbidden({ expected: "a permitted operation" }, "secret", {
            reportInput: true,
          }),
        ),
        "Forbidden operation",
      ...
      const filter = new SchemaIssue.Filter(
        Schema.isMinLength(1),
        new SchemaIssue.InvalidValue({ expected: "a custom value" }, "secret", {
          reportInput: true,
        }),
      );
    ...
    it("uses expected annotations in built-ins", () => {
      ...
      const enabled = SchemaParser.decodeUnknownResult(Schema.URLFromString)("invalid", {
        reportInput: true,
      });
      assertTrue(Result.isFailure(enabled));
    ...
    it("keeps Redacted input redacted", () => {
      ...
      const result = SchemaParser.decodeUnknownResult(Schema.Redacted(Schema.Number))(input, {
        reportInput: true,
      });
    ...
    it("preserves custom messages and hooks", () => {
      ...
      const leafFormatter = SchemaIssue.makeFormatterStandardSchemaV1({
        leafHook: () => "redacted leaf",
      });
      ...
      const checkFormatter = SchemaIssue.makeFormatterStandardSchemaV1({
        checkHook: () => "redacted check",
      });
