    ...
    describe("Effect.withExecutionPlan", () => {
      it.effect("emits start and success on first attempt", () =>
        Effect.gen(function* () {
          ...
          assertTrue(
            events[1]._tag === "AttemptSuccess" && Duration.isDuration(events[1].duration),
          );
        }),
      );
      ...
      it.effect("event attempt matches CurrentMetadata across a failover boundary", () =>
        Effect.gen(function* () {
          ...
          deepStrictEqual(
            events
              .filter((event) => event._tag === "AttemptStart")
      ...
      it.effect("emits AttemptFailure when a step layer fails to build", () =>
        Effect.gen(function* () {
          ...
          const result = yield* Stream.runCollect(
            Stream.unwrap(Effect.map(Service, (_) => _.stream)),
          ).pipe(Effect.withExecutionPlan(plan, { onEvent }));
      ...
      it.effect("emits AttemptFailure when interrupted mid-attempt", () =>
        Effect.gen(function* () {
          ...
          deepStrictEqual(simplify(events[0]), {
            _tag: "AttemptStart",
            attempt: 1,
            stepAttempt: 1,
            stepIndex: 0,
