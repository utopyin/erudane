...
describe("Effect", () => {
  ...
  describe("tracing", () => {
    it("terminal root failure captures stack frame", () => {
      ...
      const exit = Effect.runForkWith(Context.make(References.CurrentStackFrame, frame))(
        failure,
      ).pollUnsafe();
    ...
    it.effect("concurrency interrupts started siblings on failure", () =>
      Effect.gen(function* () {
        ...
        const fiber = yield* Effect.all(
          [make(1), Effect.fail("boom").pipe(Effect.delay(100)), make(3), make(4)],
          { concurrency: 3 },
        ).pipe(Effect.forkChild);
    ...
    it.effect("scoped combines usage and finalizer failures", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(
          result,
          Exit.failCause(
            Cause.combine(Cause.fail("usage failure"), Cause.die("finalizer failure")),
          ),
        );
      }),
    );
  ...
  describe("updateServiceScoped", () => {
    ...
    it.effect("supports merging the current value on reset", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.gen(function* () {
          yield* Effect.updateServiceScoped(CurrentValues, (values) => [...values, "scoped"], {
