...
const main = () => {
  ...
  try {
    ...
    const headRoot =
      options.head === "worktree" ? repoRoot : createWorktree(runRoot, "head", headSha);
    if (headRoot !== repoRoot) worktrees.push(headRoot);

    for (const fixture of selected) {
      const baseFixturePath = materializeFixture(baseRoot, fixture);
      const headFixturePath =
        headRoot === repoRoot ? fixture.fixturePath : materializeFixture(headRoot, fixture);
  ...
  if (
    ...
    results.some((result) => result.comparison.status === "regression")
  ) {
    process.exitCode = 1;
  }
};
