...
const input = Object.fromEntries(
  Array.from({ length: size }, (_, index) => [`field${index}`, `value${index}`]),
);

const makeEffectCheckedSchema = () =>
  Schema.Struct(
    Object.fromEntries(
      Array.from({ length: size }, (_, index) => [`field${index}`, Schema.NonEmptyString]),
    ),
  );
...
export const effectFirstDecodeTemplateLiteral = () => ({
  run: () =>
    SchemaParser.decodeUnknownExit(makeEffectTemplateLiteralSchema())(
