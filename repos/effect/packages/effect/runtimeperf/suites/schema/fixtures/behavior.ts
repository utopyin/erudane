...
const checkedString = Schema.String.check(Schema.isMinLength(2)).check(
...
export const transformationDecodeInvalid = decodeParserCase(
...
const makeEncodingChain = (size) => {
  ...
  for (let i = 1; i < size; i++) {
    schema = Schema.String.pipe(Schema.decodeTo(schema, SchemaTransformation.passthrough()));
  }
  return schema;
};
...
export const optionalPresentInvalid = decodeCase(
  optionalStruct,
  { required: "value", optionalKey: 1 },
  false,
);

const suspendedString = Schema.String.pipe(
  Schema.decode({
...
const suspendedObjectFields = Object.fromEntries(
  Array.from({ length: 32 }, (_, index) => [
    `field${index}`,
    index === 16 ? suspendedString : Schema.String,
  ]),
);
...
const homogeneousUnion100 = Schema.Union(
  Array.from({ length: 100 }, (_, index) =>
    Schema.String.check(Schema.makeFilter((value) => value === `value${index}`)),
  ),
);
...
export const propertyOrderOriginal = decodeCase(propertyOrderSchema, propertyOrderInput, true, {
