...
const bareApp = app.pipe(Effect.provideService(HttpServerRequest.HttpServerRequest, request));
...
bench
  ...
  .add("withSpan constructed per run (noop tracer)", async () => {
    await Effect.runPromise(
      Effect.void.pipe(Effect.withSpan("bench"), Effect.provideService(Tracer.Tracer, noopTracer)),
    );
  })
