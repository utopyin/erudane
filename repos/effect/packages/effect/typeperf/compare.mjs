...
const [selectedSuiteName, selectedFixtureName] =
  selectedTarget === undefined ? [] : selectedTarget.split("/");
...
const measure = (target, suiteName, subjectName, sourcePath) => {
  ...
  const result = run(
    process.execPath,
    [tscPath, "-p", tsconfigPath, "--extendedDiagnostics", "--noEmit"],
    {
...
const formatSigned = (value) =>
  value === 0 ? "0" : `${value > 0 ? "+" : ""}${formatNumber(value)}`;
...
const createWorktree = (runRoot, name, sha) => {
  ...
  try {
    ...
    linkDirectory(
      join(effectDir, "node_modules"),
      join(path, "packages", "effect", "node_modules"),
    );
  } catch (error) {
    removeWorktree(path);
    throw error;
...
const main = () => {
  ...
  try {
    ...
    const matchedSuites = suites.filter(
      (suite) => selectedSuiteName === undefined || suite.name === selectedSuiteName,
    );
    ...
    for (const suite of matchedSuites) {
      const baseTarget =
        suite.name === httpapi.name ? baseTargetRoot : materializeSuite(baseWorktree, suite);
      const headTarget =
        suite.name === httpapi.name ? headTargetRoot : materializeSuite(headWorktree, suite);
      ...
      const fixtures = suite.fixtures.filter(
        (fixture) => selectedFixtureName === undefined || fixture.name === selectedFixtureName,
      );
