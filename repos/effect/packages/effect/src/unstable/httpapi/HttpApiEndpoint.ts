...
type ExtractWithHeadersSuccess<S extends SuccessConstraint> =
  UnwrapReadonlyArray<S> extends infer Success
...
function transformResponseSchema(schema: Schema.Top): Schema.Top {
  ...
  if (HttpApiSchema.isWithHeaders(schema)) {
    ...
    return HttpApiSchema.rebuildWithHeaders(
      schema,
      inner,
...
function validateResponseExclusivity(
  ...
  for (const schema of schemas) {
    ...
    const body = HttpApiSchema.isWithHeaders(schema)
      ? schema.schema
      : (withHeadersAnnotation?.body ?? schema);
...
function applyResponseEncoding(
  schema: Schema.Top,
