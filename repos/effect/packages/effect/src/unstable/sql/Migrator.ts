...
export const fromGlob = (migrations: Record<string, () => Promise<any>>): Loader =>
  pipe(
    ...
    Arr.map(([key, id, name]): ResolvedMigration => [
      Number(id),
      name,
 ...
 * @since 4.0.0
 */
export const fromFileSystem: (directory: string) => Loader<FileSystem | Path> = Effect.fnUntraced(
  function* (directory) {
          ...
          onSome: ([basename, id, name]): ReadonlyArray<ResolvedMigration> =>
            [
              [
                ...
                Effect.flatMap(
                  Effect.orDie(path.toFileUrl(path.join(directory, basename))),
                  (url) =>
                    Effect.promise(
                      () =>
                        import(
