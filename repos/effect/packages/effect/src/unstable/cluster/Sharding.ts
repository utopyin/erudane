...
const make = Effect.gen(function* () {
  ...
  if (initialRunnerAddress) {
    ...
    const forcedShardReleasePending = () =>
      forcedShardReleaseRunning || MutableHashSet.size(forceReleasingShards) > 0;
    ...
    yield* Effect.gen(function* () {
      ...
      while (true) {
        ...
        const oacquired = yield* runnerStorage
          ...
          .pipe(Effect.timeoutOption(shardLockInterval));
        ...
        yield* storage
          ...
          .pipe(Effect.ignore, Effect.timeoutOption(shardLockInterval));
    ...
    yield* Effect.suspend(() =>
      shardLocksHealthyLatch.isOpen() ? refreshShardLocks : probeShardLocks,
    ).pipe(
      ...
      const send = Effect.catchCause(
        Effect.suspend(() => {
          ...
          if (!state) {
            ...
            const timeoutStartMillis =
              entityRegistrationStartMillis ?? (entityRegistrationFallbackStartMillis ??= now);
  ...
  > {
    ...
    const shouldFail =
      !discard && (message._tag === "OutgoingRequest" || message.envelope._tag === "AckChunk");
    const abandon = (error: EntityNotAssignedToRunner) => {
      if (!isPersisted) {
        return shouldFail
          ? Effect.fail(error)
          : Effect.logDebug(
              "Abandoning outgoing message during shutdown",
      ...
      return Effect.catchTag(persist, "MalformedMessage", Effect.die).pipe(
        Effect.andThen(
          shouldFail
            ? Effect.fail(error)
            : Effect.logWarning(
                "Persisting outgoing message abandoned during shutdown",
                message.envelope.address,
              ),
        ),
      );
