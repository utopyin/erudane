 ...
 * @since 4.0.0
 */
export const Theme: Context.Reference<Theme> = Context.Reference(
  "effect/unstable/cli/Prompt/Theme",
  {
...
const separateSymbol = (symbol: string, text: string): string =>
  symbol.length === 0 ? text : symbol + " " + text;
...
const handleFileClear = (options: FileOptionsReq) => {
  return Effect.fnUntraced(function* (state: FileState, _: Action<FileState, string>) {
    ...
    const promptText = isConfirming
      ? renderPrompt("(Y/n)", CONFIRM_MESSAGE, figures.prefix, figures.pointerSmall, {
          plain: true,
        })
      : renderPrompt(
...
const renderMultiSelectChoices = <A>(
  ...
  const selectedCount = Array.from(state.selectedIndices).filter(
    (index) => !choices[index].disabled,
  ).length;
...
const processSpace = <A>(state: MultiSelectState, options: SelectOptionsReq<A>) => {
  ...
  if (state.index === 0) {
    ...
    const selectedCount = Array.from(state.selectedIndices).filter(
      (index) => !options.choices[index].disabled,
    ).length;
...
const handleMultiSelectProcess = <A>(options: SelectOptionsReq<A> & MultiSelectOptionsReq) => {
  return (input: Terminal.UserInput, state: MultiSelectState) => {
    ...
    switch (input.key.name) {
      ...
      case "enter":
      case "return": {
        const selectedIndices = Array.from(state.selectedIndices).filter(
          (index) => !options.choices[index].disabled,
        );
...
const renderClearScreen = Effect.fnUntraced(function* (state: TextState, options: TextOptionsReq) {
  ...
  const clearOutput = clearOutputWithError(
    renderTextOutput(state, figures.prefix, figures.pointerSmall, options, figures, {
      plain: true,
    }),
    columns,
    errorText,
...
const handleToggleClear = Effect.fnUntraced(function* (options: ToggleOptionsReq) {
  ...
  const clearPrompt = Ansi.eraseLine + Ansi.cursorLeft;
  const toggleText =
    options.active + " " + separateSymbol(figures.toggleSeparator, options.inactive);
