...
const escapeFishChoice = (s: string): string =>
  escapeFishString(s.replace(/[^A-Za-z0-9_.,/@%+-]/gu, "\\$&"));
...
const subcommandCondition = (
  ...
  const parentCondition = parentPath
    .map((parent) => `__fish_seen_subcommand_from ${parent}`)
    .join("; and ");
  if (childSubcommandNames.length > 0) {
