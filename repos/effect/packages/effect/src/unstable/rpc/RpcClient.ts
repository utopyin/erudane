...
const isRpcClientError = (u: unknown): u is RpcClientError =>
  Predicate.isTagged(u, "RpcClientError");
  ...
  return (rpc: Rpc.AnyWithProps): RpcSchemas => {
    ...
    entry = {
      decodeChunk: Option.map(streamSchemas, (streamSchemas) =>
        Schema.decodeUnknownEffect(codecFor(Schema.NonEmptyArray(streamSchemas.success))),
      ),
  ...
  Protocol.make(
    Effect.fnUntraced(function* (writeResponse, clientIds) {
      ...
      yield* Effect.suspend(() => {
        ...
        Effect.retryOrElse(options?.retryPolicy ?? defaultRetryPolicy, (error) =>
          broadcastError(new RpcClientError({ reason: error.reason })),
        ),
        Effect.annotateLogs({
          module: "RpcClient",
