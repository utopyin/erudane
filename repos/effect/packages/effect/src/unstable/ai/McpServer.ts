...
const BroadcastServerNotificationRpcs = ServerNotificationRpcs.omit(
...
export class McpServer extends Context.Service<
  McpServer,
  {
    readonly notifications: RpcClient.RpcClient<
      RpcGroup.Rpcs<typeof BroadcastServerNotificationRpcs>
    >;
    ...
    readonly addResourceTemplate: (options: {
      ...
      readonly completions: Record<
        string,
        (input: string, context: CompletionContext) => Effect.Effect<CompleteResult, InternalError>
      >;
      readonly handle: (
        uri: string,
        params: Array<string>,
    ...
    readonly findResource: (
      ...
      typeof ReadResourceResult.Type,
      McpErrorBase | InvalidParams | InternalError,
      McpServerClient
    >;
    ...
    const service = McpServer.of({
      ...
      callTool: (request) =>
        Effect.gen(function* () {
          ...
          const result = yield* internalCore.tools
            .call(request, {
      ...
      addResource: (options) =>
        Effect.gen(function* () {
          const existingIndex = resources.findIndex(
            ({ resource }) => resource.uri === options.resource.uri,
          );
          if (existingIndex === -1) {
      ...
      addResourceTemplate: ({ annotations, completions, handle, routerPath, template }) =>
        Effect.gen(function* () {
          const existingIndex = resourceTemplates.findIndex(
            ({ template: current }) => current.uriTemplate === template.uriTemplate,
          );
          if (existingIndex === -1) {
      ...
      findResource: (uri) =>
        Effect.gen(function* () {
          const client = yield* McpServerClient;
          return yield* internalCore.resources
            .read(uri, {
              clientId: client.clientId,
              protocol: {
                protocolVersion: client.protocolVersion,
      ...
      addPrompt: (options) =>
        Effect.gen(function* () {
          const existingIndex = prompts.findIndex(
            ({ prompt }) => prompt.name === options.prompt.name,
          );
          if (existingIndex === -1) {
          ...
          yield* internalCore.prompts.register({
            ...
            get: (params, invocation) =>
              options
                .handle(params)
                .pipe(Effect.provideService(McpServerClient, invocation.requestContext)),
          });
      ...
      getPromptResult: Effect.fnUntraced(function* ({ arguments: params, name }) {
        const client = yield* McpServerClient;
        return yield* internalCore.prompts
          .get(name, params ?? {}, McpProtocolInternal.invocationFromClient(client))
          .pipe(
            Effect.catchTag(
              "PromptNotFound",
              () => new InvalidParams({ message: `Prompt '${name}' not found` }),
            ),
          );
      }),
...
class McpProtocolState extends Context.Service<
  McpProtocolState,
  {
...
const runWithProtocolState = Effect.fnUntraced(function* (
  options: {
    readonly name: string;
    readonly version: string;
    readonly description?: string | undefined;
  ...
  const clients = yield* RcMap.make({
    lookup: Effect.fnUntraced(function* (key: McpClientKey) {
      ...
      const client = yield* selectedProtocol
        .makeReverseClient(key.profile)
  ...
  const clientMiddleware = McpServerClientMiddleware.of(
    (effect, { client, headers, payload, rpc }) => {
      ...
      const selectedProtocol =
        session?.protocol ?? protocolForInternalTag(protocolRegistry, rpc._tag);
      ...
      const initializePayload =
        session?.initializePayload ?? (payload as typeof Initialize.payloadSchema.Type);
    ...
    run: (f) =>
      protocol.run((clientId, request_) => {
        ...
        switch (request._tag) {
            ...
            if (rpc && selectedProtocol.clientNotificationRpcs.requests.has(request.tag)) {
              ...
              return decode.pipe(
                Effect.flatMap((payload) => {
                  ...
                  const handler = handlers.mapUnsafe.get(rpc.key) as
                    | Rpc.Handler<string>
                    | undefined;
                  return handler
          ...
          case "Defect": {
            const selectedProtocol = getProtocolForClient(
              clientProtocols,
              clientId,
              protocolRegistry,
            );
  ...
  yield* Queue.take(internalState.get(server)!.notifications).pipe(
    Effect.flatMap(
      Effect.fnUntraced(function* ({ notification, targetClientId }) {
...
export const layer = (options: {
  ...
  layerWithProtocolState(options).pipe(Layer.provide(layerMcpProtocolState(options.protocols)));
...
export const layerStdio = (options: {
  ...
  layer(options).pipe(
    ...
    Layer.provide(
      Layer.succeed(RpcSerialization.RpcSerialization, mcpStdioSerialization(options.protocols)),
    ),
  );
        ...
        decode: (data) => {
          ...
          for (const frame of frames.decode(data)) {
            if (Array.isArray(frame)) {
              ...
              selectedProtocol =
                protocols.find((protocol) => protocol.protocolVersion === offered) ?? protocols[0];
            }
  ...
  Layer.effect(RpcServer.Protocol)(
    Effect.gen(function* () {
      ...
      yield* router.add("POST", options.path, (request) => {
        ...
        const session =
          sessionId === undefined ? undefined : state.sessions.bySessionId.get(sessionId);
        if (sessionId !== undefined && session === undefined) {
          return Effect.succeed(HttpServerResponse.empty({ status: 404 }));
        }
        if (
          protocolVersion !== undefined &&
          !state.protocolRegistry.protocols.some(
              ...
              Effect.matchEffect(Schema.decodeUnknownEffect(Schema.UnknownFromJsonString)(body), {
                ...
                onSuccess: (input) => {
                  if (!Array.isArray(input)) {
                    ...
                    const id =
                      hasId && (typeof input.id === "string" || typeof input.id === "number")
                        ? input.id
                        : null;
                    const isJsonRpc =
                      Predicate.hasProperty(input, "jsonrpc") && input.jsonrpc === "2.0";
                    const hasValidRequestId =
                      !hasId || typeof input.id === "string" || typeof input.id === "number";
                    const isRequest =
                      isJsonRpc &&
                      hasValidRequestId &&
                      Predicate.hasProperty(input, "method") &&
...
export type ResourceCompletions<Schemas extends ReadonlyArray<Schema.Constraint>> = {
  readonly [
    K in Extract<keyof Schemas, `${number}`> as Schemas[K] extends Param<infer Id, infer _S>
      ? Id
      : `param${K}`
  ]: (
    input: string,
...
export const registerResource: {
  ...
  return Effect.fnUntraced(function* <E, R>(options: {
    ...
    readonly priority?: number | undefined;
    readonly completion?:
      | Record<string, (input: string, context: CompletionContext) => Effect.Effect<any>>
      | undefined;
    readonly content: (
      uri: string,
      ...params: Array<any>
    ...
    for (const [param, handle] of Object.entries(options.completion ?? {})) {
      ...
      const handler = (input: string, context: CompletionContext) =>
...
> => {
  ...
  const completion: Record<
    string,
    (input: string, context: CompletionContext) => Effect.Effect<any>
  > = options.completion ?? {};
  return Effect.gen(function* () {
    const registry = yield* McpServer;
    const services =
            ...
            return yield* Effect.withFiber((fiber) => {
              ...
              const session: Session = {
                ...
                resourceSubscriptions:
                  httpRequest === undefined && capabilities.resources?.subscribe === true
                    ? new Set()
                    : undefined,
...
const getClientSession = (sessions: Sessions, clientId: number, headers: Headers.Headers) => {
...
const effectLogLevel = (
  logLevel: SessionLogLevel | undefined,
...
const getProtocolForClient = (
  ...
  clientId: number,
  registry: McpProtocolRegistry.ProtocolRegistry<McpProtocol.ProtocolAdapter>,
): McpProtocol.ProtocolAdapter => clientProtocols.get(clientId) ?? registry.protocols[0];
