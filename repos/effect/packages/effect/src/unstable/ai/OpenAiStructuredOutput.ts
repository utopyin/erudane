...
function rewriteOpenAI(schema: JsonSchema.JsonSchema): JsonSchema.JsonSchema {
  return InternalStructuredOutput.walkJsonSchema(schema, (schema) => {
    ...
    for (const [key, value] of Object.entries(normalized)) {
      if (key === "format") {
        if (typeof value === "string" && formats.has(value) && supportsType(key, normalized.type))
          out.format = value;
    ...
    if (unsupportedFormat !== undefined) {
      InternalStructuredOutput.appendDescription(
        out,
        `a value with a format of ${unsupportedFormat}`,
      );
    }
    if (
      out.type === "object" &&
...
function mergeAllOfKeyword(
  ...
  switch (key) {
    ...
    default:
      if (
        !normalizableAllOfKeywords.has(key) ||
        baseKeywords.has(key) ||
        ambiguousKeywords.has(key)
      )
        return;
