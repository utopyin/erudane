...
) => Effect.Effect<Schema.JsonObject | undefined, McpCore.UnsupportedByProtocol> =
  ...
  makeHandlers: (core, lifecycle) =>
    AdapterRpcs.of({
      initialize: (request, { client }) =>
        lifecycle
          .initialize(McpSchema.protocolVersion, profileFromInitialize(request), client.id)
          .pipe(
            Effect.map((result) =>
      ...
      "resources/templates/list": (_pageRequest) =>
        PublicMcpSchema.McpServerClient.use((request) =>
          ...
          Effect.map((resourceTemplates) =>
            McpSchema.ListResourceTemplatesResult.make({ resourceTemplates }),
          ),
        ),
      ...
      "prompts/list": (_pageRequest) =>
        ...
        ).pipe(Effect.map((prompts) => McpSchema.ListPromptsResult.make({ prompts }))),
      ...
      "tools/call": Effect.fnUntraced(function* (call) {
        ...
        const result = yield* core.tools
          ...
          .pipe(Effect.mapError(McpProtocol.ProtocolError.fromTool));
