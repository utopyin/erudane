...
const requiresSamplingTools = (
  request: typeof PublicMcpSchema.CreateMessage.payloadSchema.Type,
): boolean =>
...
const resultRequiresSamplingTools = (
  result: typeof McpSchema.CreateMessage.successSchema.Type,
): boolean =>
...
const projectContent = Effect.fnUntraced(function* (
    ...
    AdapterRpcs.of({
      ...
      "resources/list": () =>
        PublicMcpSchema.McpServerClient.use((request) =>
          core.resources.list(McpProtocol.profileFromClient(request)),
        ).pipe(
      ...
      "resources/read": Effect.fnUntraced(function* ({ uri }) {
        ...
        const result = yield* core.resources
          .read(uri, McpProtocol.invocationFromClient(request))
          .pipe(Effect.mapError(McpProtocol.ProtocolError.fromFeature));
      ...
      "prompts/list": () =>
        PublicMcpSchema.McpServerClient.use((request) =>
          core.prompts.list(McpProtocol.profileFromClient(request)),
        ).pipe(
          Effect.map((prompts) =>
      ...
      "prompts/get": Effect.fnUntraced(function* ({ arguments: args, name }) {
        const request = yield* PublicMcpSchema.McpServerClient;
        const result = yield* core.prompts
          .get(name, args ?? {}, McpProtocol.invocationFromClient(request))
          .pipe(Effect.mapError(McpProtocol.ProtocolError.fromFeature));
      ...
      "completion/complete": Effect.fnUntraced(function* (completeRequest) {
        const request = yield* PublicMcpSchema.McpServerClient;
        const result = yield* core.completions
          .complete(
            {
              reference:
                completeRequest.ref.type === "ref/prompt"
                  ? {
                      type: "prompt",
              ...
              context:
                completeRequest.context?.arguments === undefined
                  ? undefined
        ...
        const result = yield* core.tools
          ...
          .pipe(
            Effect.catchTag("InvalidToolInput", (error) =>
              Effect.succeed(
                PublicMcpSchema.CallToolResult.make({
                  content: [
                    PublicMcpSchema.TextContent.make({
                      type: "text",
                      text: error.message,
                    }),
                  ],
  ...
  toReverseClient: (profile, client) => ({
    listRoots: Effect.fnUntraced(function* (request) {
      ...
      const wireRequest = yield* McpProtocol.transcode(
        ...
        Effect.mapError(() =>
          unsupported("roots/list", "Request is not representable by this protocol"),
        ),
      );
    ...
    createMessage: Effect.fnUntraced(function* (request) {
      ...
      if (
        requiresSamplingTools(request) &&
        profile.clientCapabilities.sampling?.tools == undefined
      ) {
        return yield* unsupported(
          "sampling/createMessage",
          "Client did not advertise the sampling.tools capability",
        );
      }
      if (
        ...
        return yield* unsupported(
          "sampling/createMessage",
          "Client did not advertise the sampling.context capability",
        );
      }
      const wireRequest = yield* McpProtocol.transcode(
      ...
      if (
        resultRequiresSamplingTools(result) &&
        profile.clientCapabilities.sampling?.tools == undefined
      ) {
        return yield* unsupported(
          "sampling/createMessage",
      ...
      return yield* McpProtocol.transcode(
        ...
        Effect.mapError(() =>
          unsupported(
            "sampling/createMessage",
            "Response is not representable by the canonical model",
          ),
        ),
      );
    }),
    elicit: Effect.fnUntraced(function* (request) {
      ...
      if (!hasElicitationModeCapability(profile, mode)) {
        return yield* unsupported(
          "elicitation/create",
          `Client did not advertise the elicitation.${mode} capability`,
        );
      }
      const wireRequest = yield* McpProtocol.transcode(
        ...
        Effect.mapError(() =>
          unsupported("elicitation/create", "Request is not representable by this protocol"),
        ),
      );
