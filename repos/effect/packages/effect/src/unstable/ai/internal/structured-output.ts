...
function tupleToObject(
  ...
  const propertySignatures = ast.elements.map(
    (element, index) => new SchemaAST.PropertySignature(String(index), element),
  );
  if (ast.rest.length === 1) {
    ...
    propertySignatures.push(
      new SchemaAST.PropertySignature(
        REST_PROPERTY_NAME,
        new SchemaAST.Arrays(false, [], [ast.rest[0]]),
      ),
    );
...
function objectToEntries(
  ...
  const key = unionOrSingle([
    ...ast.propertySignatures.map(
      (propertySignature) => new SchemaAST.Literal(propertySignature.name as string),
    ),
    ...
    switch (key) {
      ...
      case "propertyNames": {
        InternalRecord.assignProperty(
          out,
          key,
          isJsonSchema(value) ? walkJsonSchema(value, visitor) : value,
        );
        break;
      }
      case "prefixItems":
      ...
      case "oneOf": {
        InternalRecord.assignProperty(
          ...
          Array.isArray(value)
            ? value.map((member) =>
                isJsonSchema(member) ? walkJsonSchema(member, visitor) : member,
              )
            : value,
        );
        break;
