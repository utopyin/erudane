...
export class ProtocolError extends Data.TaggedError("ProtocolError")<{
  ...
  static fromTool(error: McpCore.ToolError | McpCore.UnsupportedByProtocol): ProtocolError {
...
export const reverseError =
  (operation: PublicMcpSchema.McpReverseOperationError["operation"]) => (cause: unknown) =>
...
/** @internal */
export const transcode = <From extends Schema.Constraint, To extends Schema.Constraint>(
  from: From,
  to: To,
  input: Schema.Schema.Type<From>,
) => Schema.encodeEffect(from)(input).pipe(Effect.flatMap(Schema.decodeUnknownEffect(to)));

/** @internal */
export const transcodeStrict = <From extends Schema.Constraint, To extends Schema.Constraint>(
  from: From,
  to: To,
  input: Schema.Schema.Type<From>,
) =>
...
export interface HandlerInstallationTarget {
  readonly install: <Rpcs extends Rpc.Any, Handlers extends RpcGroup.HandlersFrom<Rpcs>>(
    protocol: {
...
> {
  ...
  readonly makeReverseClient: (
    profile: McpCore.NegotiatedProtocolProfile,
  ) => Effect.Effect<PublicMcpSchema.McpReverseClient, never, RpcClient.Protocol | Scope.Scope>;
  readonly projectNotification: (
    notification: McpCore.ServerNotification,
  ) => Effect.Effect<
    PublicMcpProtocol.ProjectedNotification | undefined,
...
>(options: {
  ...
  readonly makeHandlers?:
    | ((core: McpCore.McpCore, lifecycle: LifecycleRuntime) => Handlers)
    | undefined;
