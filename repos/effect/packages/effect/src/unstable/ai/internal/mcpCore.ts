...
export interface CompletionRequest {
  ...
  readonly context?:
    | Readonly<{ readonly arguments?: Readonly<Record<string, string>> }>
    | undefined;
...
export const make: Effect.Effect<McpCore> = Effect.sync(() => {
  ...
  const resources: Resources = {
    register: (registration) =>
      Effect.sync(() => {
        const index = resourceRegistrations.findIndex(
          (entry) => entry.descriptor.uri === registration.descriptor.uri,
        );
        if (index === -1) {
