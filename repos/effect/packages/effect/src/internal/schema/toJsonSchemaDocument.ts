...
function extractJsonSchemaNumberType(schema: JsonSchema.JsonSchema): {
  ...
  let type: JsonSchemaNumberType | undefined =
    schema.type === "number" || schema.type === "integer" ? schema.type : undefined;
  let out = schema;
  if (type !== undefined) {
    out = { ...schema };
    delete out.type;
...
function compileJsonSchema(
  ...
  const schemas = Arr.map(representations, (representation, index) =>
    finalizeJsonSchema(recur(representation, rootPaths[index])),
  );
  ...
  function compileDefinition(key: string, path: Path): string {
    ...
    if (fallback !== undefined) {
      ...
      const match = candidates?.find((candidate) =>
        Equal.equals(definitionStates.get(candidate), schema),
      );
      if (match === undefined) {
  ...
  function compileCheck(
    ...
    if (callback !== undefined) {
      ...
      const allowed =
        ordinary === undefined ? inlineableCheckKeywords : inlineableAnnotatedCheckKeywords;
    ...
    switch (representation._tag) {
      ...
      case "Objects": {
        ...
        for (let index = 0; index < representation.indexSignatures.length; index++) {
          ...
          let type: JsonSchema.JsonSchema | false = recur(signature.type, [
            ...path,
            "indexSignatures",
            index,
            "type",
