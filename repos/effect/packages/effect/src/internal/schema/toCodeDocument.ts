...
type CheckRepresentationAnnotation =
  SchemaRepresentation.CheckRepresentationAnnotation<SchemaRepresentation.Representation>;
...
/** @internal */
export function topologicalSort(references: SchemaRepresentation.References): TopologicalSort {
  ...
  function collectRefs(root: SchemaRepresentation.Representation): ReadonlySet<string> {
    ...
    function pushRepresentationSchemas(
  ...
  for (const identifier of recursive)
    InternalRecord.assignProperty(recursives, identifier, references[identifier]);
...
export function toCodeDocument(
  ...
  for (const [$ref, representation] of Object.entries(sorted.recursives)) {
    ...
    InternalRecord.assignProperty(
      recursives,
      ensureUniqueIdentifier($ref),
      recur(representation, ["references", $ref]),
    );
  ...
  function addImport(importDeclaration: string): void {
    if (
      !artifacts.some(
        (artifact) =>
          artifact._tag === "Import" && artifact.importDeclaration === importDeclaration,
      )
    ) {
  ...
  function annotationSchemas(
    ...
    return (
      representation?.schemas?.map((schema, index) => recur(schema, [...path, "schemas", index])) ??
      []
    );
  }

  function checkBrands(check: SchemaRepresentation.Check): ReadonlyArray<string> {
  ...
  function compileCheck(check: SchemaRepresentation.Check, path: Path): string {
    ...
    if (callback !== undefined) {
      ...
      runtime = `Schema.makeFilterGroup([${check.checks
        .map((child, index) => compileCheck(child, [...path, "checks", index]))
        .join(", ")}])`;
    }
  ...
  function applyNode(
    ...
    let runtime =
      base.runtime + runtimeAnnotate(representation.annotations) + runtimeBrands(nodeBrands);
  ...
  function recur(
    ...
    if (representation._tag === "Reference") {
      ...
      if (
        compilingRecursiveDefinition &&
        explicitSuspendDepth === 0 &&
        Object.hasOwn(sorted.recursives, representation.$ref)
      ) {
        return makeCode(
          `Schema.suspend((): Schema.Codec<${identifier}> => ${identifier})`,
          identifier,
        );
      }
      const Type = includeTypeBrands
        ? identifier
        : recur(
            document.references[representation.$ref],
            ["references", representation.$ref],
            false,
          ).Type;
      return makeCode(identifier, Type);
    }
    return applyNode(
      on(representation, path, includeTypeBrands),
      representation,
      path,
      includeTypeBrands,
    );
  ...
  function on(
    ...
    switch (representation._tag) {
      case "Declaration": {
        ...
        const output = (callback as SchemaRepresentation.Generation.Declaration)({
          typeParameters,
        });
        for (const importDeclaration of output.importDeclarations ?? [])
      ...
      case "Enum": {
        ...
        artifacts.push({
          ...
          identifier,
          code: makeCode(
            `enum ${identifier} { ${representation.enums
              .map(([name, value]) => `${format(name)} = ${format(value)}`)
              .join(", ")} }`,
            `typeof ${identifier}`,
          ),
        });
      ...
      case "TemplateLiteral": {
        const parts = representation.parts.map((part, index) =>
          recur(part, [...path, "parts", index], false),
        );
        const Type = `\`${parts.map((part) => `\${${part.Type}}`).join("")}\``;
        return makeCode(
          `Schema.TemplateLiteral([${parts.map((part) => part.runtime).join(", ")}])`,
          Type,
        );
      }
      case "Arrays": {
        const elements = representation.elements.map((element, index) => {
          ...
          return makeCode(
            `${element.isOptional ? "Schema.optionalKey(" : ""}${type.runtime}${element.isOptional ? ")" : ""}${runtimeAnnotate(
              element.annotations,
              "annotateKey",
            )}`,
        ...
        const rest = representation.rest.map((item, index) =>
          recur(item, [...path, "rest", index]),
        );
        if (Arr.isArrayNonEmpty(rest)) {
          ...
          return makeCode(
            `Schema.TupleWithRest(Schema.Tuple([${elements.map((element) => element.runtime).join(", ")}]), [${rest
      ...
      case "Objects": {
        ...
        const indexRuntimes = indexSignatures
          .map(
            (signature) =>
              `Schema.Record(${signature.parameter.runtime}, ${signature.type.runtime})`,
          )
        ...
        const indexTypes = indexSignatures.map(
          (signature) => `readonly [x: ${signature.parameter.Type}]: ${signature.type.Type}`,
        );
