...
function revivePersisted(
  ...
  function reviveDeclaration(
    ...
    const reviver = resolveReviver<SchemaRepresentation.DeclarationReviver<any>>(
      representation,
      representationPath,
    );
  ...
  function reviveFilter(filter: SchemaRepresentation.Filter, path: Path): SchemaAST.Filter<any> {
    const representationPath = [...path, "representation"];
    const representation = filter.representation;
    if (representation === undefined) {
      throw errorWithPath("Missing representation annotation", representationPath);
    }
    const reviver = resolveReviver<SchemaRepresentation.FilterReviver<any>>(
      representation,
      representationPath,
    );
    const payload = decodePayload(representation, reviver, [...representationPath, "payload"]);
  ...
  function reviveFilterGroup(
    ...
    if (representation === undefined) {
      const checks = group.checks.map((check, index) =>
        reviveCheck(check, [...path, "checks", index]),
      );
    ...
    const reviver = resolveReviver<SchemaRepresentation.FilterGroupReviver<any>>(
      representation,
      representationPath,
    );
    const payload = decodePayload(representation, reviver, [...representationPath, "payload"]);
  ...
  function reviveCheck(check: SchemaRepresentation.Check, path: Path): SchemaAST.Check<any> {
  ...
  function finishStructural(
    ...
    return appendChecks(annotate(schema, representation.annotations), representation.checks, [
      ...path,
      "checks",
    ]);
  }

  function recur(representation: SchemaRepresentation.Representation, path: Path): Schema.Top {
    switch (representation._tag) {
      ...
      case "Objects": {
        ...
        const records = representation.indexSignatures.map((indexSignature, index) =>
          Schema.Record(
            recur(indexSignature.parameter, [
              ...path,
              "indexSignatures",
              index,
              "parameter",
            ]) as Schema.Record.Key,
      ...
      case "Union": {
        const members = representation.types.map((member, index) =>
          recur(member, [...path, "types", index]),
        );
        return finishStructural(
          Schema.Union(members, { mode: representation.mode }),
          representation,
          path,
        );
      }
