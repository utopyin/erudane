...
type ImportedJsonSchemaRepresentation = Extract<
  Representation,
  {
    readonly _tag:
      | "Reference"
...
function addNumberCheck(checks: Array<Check>, value: unknown, id: string, key: string): void {
...
function jsonSchemaAnnotations(
  ...
  if (Array.isArray(schema.examples))
    annotations.examples = schema.examples as ReadonlyArray<Schema.Json>;
  ...
  if (typeof schema.contentEncoding === "string")
    annotations.contentEncoding = schema.contentEncoding;
  if (typeof schema.contentMediaType === "string")
    annotations.contentMediaType = schema.contentMediaType;
...
function jsonDeclaration(annotations: Schema.Annotations.Annotations | undefined): Representation {
  return {
    _tag: "Declaration",
    representation: {
...
function translateJsonSchemaMultiDocument(
  ...
  function translateDefinition(
    ...
    if (cached !== undefined) {
      if (cached === null) {
        throw errorWithPath(recursiveReferenceError ?? `Invalid reference ${key}`, [
          ...path,
          "$ref",
        ]);
      }
  ...
  function resolveReference(
    ...
    const representation = translateDefinition(
      reference.$ref,
      path,
      options?.recursiveReferenceError,
    );
    if (representation._tag === "Reference") {
  ...
  function satisfiesLiteral(
    representation: SchemaRepresentation.String | SchemaRepresentation.Number,
    ...
    return representation.checks.every((check) =>
      satisfiesPrimitiveCheck(check, value as string | number),
    );
  }
  ...
  function lowerObject(
    ...
    for (const scope of scopes) {
      ...
      hasFiniteKeyDomain ||=
        scope.additionalProperties._tag === "Never" && scope.patterns.length === 0;
      requiresFiniteKeyDomain ||=
        scope.additionalProperties._tag === "Never" ||
        (scope.additionalProperties._tag !== "Unknown" &&
          (scope.hasProperties || scope.patterns.length > 0));
    }
        ...
        for (const pattern of scope.patterns) {
          ...
          patterns.set(
            ...
            previous === undefined
              ...
              : {
                  ...pattern,
                  type: combine(previous.type, pattern.type, [
                    ...path,
                    "patternProperties",
                    pattern.source,
                  ]),
  ...
  function combine(
    ...
    if (left._tag === "Union") {
      const types = left.types
        .map((type, index) =>
          combine(type as ImportedJsonSchemaRepresentation, right, [...path, "types", index]),
        )
    ...
    switch (left._tag) {
      case "Null":
        return right._tag === "Null"
          ? combinedAnnotations(
              { _tag: "Null", checks: [...left.checks, ...right.checks] },
              left,
              right,
            )
          : never;
      case "String":
        ...
        return annotate(
          ...
          mergeAnnotations(
            left.annotations,
            stringChecks === undefined ? right.annotations : undefined,
          ),
        );
      case "Number":
        if (right._tag === "Literal") {
        ...
        return annotate(
          ...
          mergeAnnotations(
            left.annotations,
            numberChecks === undefined ? right.annotations : undefined,
          ),
        );
      case "Boolean":
        if (right._tag === "Literal") {
      ...
      case "Arrays": {
        ...
        const arrayChecks = combineChecks(left.checks, right.checks, right.annotations, [
        ...
        return annotate(
          ...
          mergeAnnotations(
            left.annotations,
            arrayChecks === undefined ? right.annotations : undefined,
          ),
        );
      }
      case "Objects": {
        ...
        return lowerObject(
          ...
          mergeAnnotations(
            left.annotations,
            objectChecks === undefined ? right.annotations : undefined,
          ),
          path,
        );
      }
  ...
  function on(schema: JsonSchema.JsonSchema, path: Path): ImportedJsonSchemaRepresentation {
    if (typeof schema.$ref === "string") {
      ...
      if ($ref === undefined) {
        throw errorWithPath(`Unsupported reference ${JSON.stringify(schema.$ref)}`, [
          ...path,
          "$ref",
        ]);
      }
    ...
    switch (type) {
      ...
      case "integer":
        return {
          ...
          checks: [
            jsonSchemaFilter(
              type === "number" ? "effect/schema/isFinite" : "effect/schema/isInt",
              null,
            ),
  ...
  function collectObjectScope(schema: JsonSchema.JsonSchema, path: Path): ImportedObjectScope {
    ...
    if (
      ...
      for (const [pattern, value] of Object.entries(schema.patternProperties)) {
        ...
        if (checks.length === 0)
          return { properties, hasProperties, patterns: [], additionalProperties: unknown };
        patterns.push({
          source: pattern,
          parameter: {
            _tag: "String",
  ...
  function collectObjectChecks(schema: JsonSchema.JsonSchema, path: Path): Array<Check> {
    const checks: Array<Check> = [];
  ...
  for (const [key, path] of reachableDefinitions) {
    InternalRecord.assignProperty(
      references,
      key,
      unknownJsonSchemas(translateDefinition(key, path)),
    );
  }
...
export function fromJsonSchemaMultiDocument(
  ...
  return fromRepresentations(
    translateJsonSchemaMultiDocument(document, options),
    jsonSchemaRevivers,
  );
}
