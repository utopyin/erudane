...
export const getNodeIndex = (csr: Csr, nodeId: Graph.NodeIndex): number | undefined =>
  csr.indexByNodeId === undefined
    ? Number.isInteger(nodeId) && nodeId >= 0 && nodeId < csr.nodeIds.length
      ? nodeId
      : undefined
  ...
  for (let i = 0; i < csr.nodeIds.length; i++) {
    ...
    for (let j = 0; j < adjacency.length; j++) {
      ...
      columnIndices[offset++] =
        csr.indexByNodeId === undefined ? neighbor : csr.indexByNodeId.get(neighbor)!;
    }
  }
...
export const getOutgoingWithEdges = (csr: Csr): AdjacencyWithEdges => {
  ...
  for (const nodeId of csr.nodeIds) {
    ...
    for (const edgeId of adjacency) {
      edgeIndices[position++] =
        csr.indexByEdgeId === null ? edgeId : csr.indexByEdgeId.get(edgeId)!;
    }
  }
...
/** @internal */
export const getAdjacencies = (csr: Csr, direction: Graph.TraversalDirection): Adjacencies => {
...
export const getEdgeEndpoints = (csr: Csr): EdgeEndpoints => {
  ...
  for (let i = 0; i < edges.length; i++) {
    sources[i] =
      csr.indexByNodeId === undefined ? edges[i].source : csr.indexByNodeId.get(edges[i].source)!;
    targets[i] =
      csr.indexByNodeId === undefined ? edges[i].target : csr.indexByNodeId.get(edges[i].target)!;
  }
