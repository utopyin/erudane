  ...
  [Equal.symbol](this: GraphImpl<any, any, any>, that: Equal.Equal): boolean {
    if (hasProperty(that, TypeId)) {
      ...
      for (const [nodeIndex, nodeData] of this.nodes) {
        if (
          !thatImpl.nodes.has(nodeIndex) ||
          !Equal.equals(nodeData, thatImpl.nodes.get(nodeIndex))
        ) {
          return false;
        }
      }
...
/** @internal */
export const finalize = <N, E, T extends Graph.Kind>(
  source: GraphImpl<N, E, T>,
): GraphImpl<N, E, T> => {
  const graph: GraphImpl<N, E, T> = Object.create(ProtoGraph);
...
/** @internal */
export const snapshot = <N, E, T extends Graph.Kind>(
  graph: Graph.Graph<N, E, T>,
): Graph.Snapshot<N, E, T> => {
  const impl = toImpl(graph);
  return {
    type: graph.type,
    nodes: Array.from(impl.nodes, ([index, data]) => ({ index, data })).sort(
      (a, b) => a.index - b.index,
    ),
...
export const hydrate = <N, E, T extends Graph.Kind>(
  ...
  graph.nextNodeIndex =
    snapshot.nodes.length === 0 ? 0 : snapshot.nodes[snapshot.nodes.length - 1].index + 1;
  graph.nextEdgeIndex =
    snapshot.edges.length === 0 ? 0 : snapshot.edges[snapshot.edges.length - 1].index + 1;
