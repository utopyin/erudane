...
export const withExecutionPlan: {
  ...
  <A, E extends Input, R, Provides, Input, PlanE, PlanR, RX>(
    self: Effect<A, E, R>,
    plan: Api.ExecutionPlan<{
      provides: Provides;
      input: Input;
    ...
    effect.suspend(() => {
      ...
      const instrument: (attempt: Effect<A, any, any>) => Effect<A, any, any> =
        emitter === undefined
          ? identity
          : (attempt) =>
              effect.uninterruptibleMask((restore) =>
                effect.flatMap(emitter.begin, (state) =>
                  effect.onExit(restore(attempt), (exit) => emitter.end(state, exit)),
                ),
              );
