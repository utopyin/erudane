...
export class FiberImpl<A = any, E = any> implements Fiber.Fiber<A, E> {
  ...
  setContext(context: Context.Context<never>): void {
    ...
    this.runtimeMetrics = Context.getOrUndefinedUnsafe(
      context,
...
} = dual(2, <A, E, B>(self: Exit.Exit<A, E>, f: (a: A) => B): Exit.Exit<B, E> =>
...
} = dual(2, <A, E, B>(self: Exit.Exit<A, E>, b: B): Exit.Exit<B, E> =>
...
const provideServiceImpl = <A, E, R, I, S>(
  ...
  updateContext(self, Context.add(service, implementation)) as any;
...
} = dual(2, <A, E, R>(self: Effect.Effect<A, E, R>, n: number): Array<Effect.Effect<A, E, R>> =>
...
/** @internal */
export const monotonicTimeNanos: Effect.Effect<bigint> = clockWith(
  (clock) => clock.monotonicTimeNanos,
);
...
const prettyLoggerTty = (options: {
  ...
  const processIsBun =
    (globalThis as { readonly process?: { readonly isBun?: boolean } }).process?.isBun === true;
