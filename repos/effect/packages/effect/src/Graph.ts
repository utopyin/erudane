      ...
      for (let i = 0; i < cache.nodeIds.length; i++) {
        ...
        for (let j = start; j < cache.nodeIds.length; j++) {
          ...
          addEdge(
            ...
            newIndices[i],
            newIndices[j],
            createEdge(cache.nodeData[i] as N, cache.nodeData[j] as N),
          );
        }
      }
    });
...
export const outgoingEdges: {
  (
    nodeIndex: NodeIndex,
  ): <N, E>(graph: Graph<N, E, "directed"> | MutableGraph<N, E, "directed">) => Array<EdgeIndex>;
  <N, E>(
...
export const degree: {
  (
    nodeIndex: NodeIndex,
  ): <N, E>(graph: Graph<N, E, "undirected"> | MutableGraph<N, E, "undirected">) => number;
  <N, E>(
...
export const outDegree: {
  (
    nodeIndex: NodeIndex,
  ): <N, E>(graph: Graph<N, E, "directed"> | MutableGraph<N, E, "directed">) => number;
  <N, E>(
    graph: Graph<N, E, "directed"> | MutableGraph<N, E, "directed">,
    nodeIndex: NodeIndex,
  ): number;
...
export const findCycle = <N, E, T extends Kind = "directed">(
  ...
  for (const start of impl.nodes.keys()) {
    ...
    colors.set(start, 1);
    const stack: Array<{ readonly node: NodeIndex; position: number }> = [
...
const getUnweightedDistances = <N, E, T extends Kind>(
  ...
  const adjacencies = csr.getAdjacencies(
    cache,
    graph.type === "undirected" ? "outgoing" : direction,
  );
...
const analyzeLowLinks = <N, E>(
  ...
  return {
    ...
    biconnectedComponents: components.map((component) =>
      component.map((node) => cache.nodeIds[node]),
    ),
  };
};
...
export const maximumFlow: {
  <E>(
    config: MaximumFlowConfig<E>,
  ): <N>(graph: Graph<N, E, "directed"> | MutableGraph<N, E, "directed">) => MaximumFlowResult;
  <N, E>(
    graph: Graph<N, E, "directed"> | MutableGraph<N, E, "directed">,
    config: MaximumFlowConfig<E>,
  ): MaximumFlowResult;
...
export const minimumCut: {
  <E>(
    config: MaximumFlowConfig<E>,
  ): <N>(graph: Graph<N, E, "directed"> | MutableGraph<N, E, "directed">) => MinimumCutResult;
  <N, E>(
    graph: Graph<N, E, "directed"> | MutableGraph<N, E, "directed">,
    config: MaximumFlowConfig<E>,
  ): MinimumCutResult;
...
export const isStronglyConnected = <N, E>(
  ...
  const cache = csr.get(graph);
  return (
    csrReachesAll(cache.nodeIds.length, csr.getOutgoing(cache)) &&
  ...
  while (true) {
    ...
    if (
      ...
      (heap.priorities[right] < heap.priorities[left] ||
        (heap.priorities[right] === heap.priorities[left] &&
          heap.sequences[right] < heap.sequences[left]))
    ) {
      child = right;
    }
    if (
...
export const dijkstra: {
  ...
  <N, E, T extends Kind = "directed">(
    ...
    while (priorityQueue.size > 0) {
      ...
      for (
        let i: number = outgoing.rowOffsets[currentNode];
        i < outgoing.rowOffsets[currentNode + 1];
        i++
      ) {
        const neighbor = outgoing.columnIndices[i];
        const edge = outgoing.edgeIndices[i];
...
export const astar: {
  ...
  <N, E, T extends Kind = "directed">(
    ...
    if (config.source === config.target) {
      if (
        !Number.isFinite(
          withMutationGuard(graph, () => config.heuristic(sourceNodeData, targetNodeData)),
        )
      ) {
    ...
    while (openSet.size > 0) {
      ...
      for (
        let i: number = outgoing.rowOffsets[current];
        i < outgoing.rowOffsets[current + 1];
        i++
      ) {
        const neighbor = outgoing.columnIndices[i];
        if (visited[neighbor] !== 0) {
...
export const allShortestPaths: {
  ...
  <N, E, T extends Kind = "directed">(
    ...
    return pathWalker(function* () {
      ...
      distances.fill(Infinity);
      distances[source] = 0;
      const previous: Array<Array<{ readonly node: number; readonly edge: number }> | undefined> =
        new Array(cache.nodeIds.length);
      ...
      while (queue.size > 0) {
        ...
        for (
          let i = outgoing.rowOffsets[currentNode];
          i < outgoing.rowOffsets[currentNode + 1];
          i++
        ) {
          const edge = outgoing.edgeIndices[i];
          ...
          next() {
            while (stack.length > 0) {
              ...
              return {
                ...
                value: f(cache.nodeIds[current], cache.nodeData[current] as N),
              };
            }
