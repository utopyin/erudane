...
describe("Metrics", () => {
  ...
  it.effect("reports the same delta to every registered reader", () =>
    Effect.gen(function* () {
      ...
      const firstValue = (firstResult.resourceMetrics.scopeMetrics[0]!.metrics[0] as any)
        .dataPoints[0].value;
      const secondValue = (secondResult.resourceMetrics.scopeMetrics[0]!.metrics[0] as any)
        .dataPoints[0].value;
