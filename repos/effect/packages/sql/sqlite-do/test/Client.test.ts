...
class FakeSqlStorage {
  ...
  exec(sql: string, ...params: ReadonlyArray<unknown>) {
        ...
        if (!this.tables.has(table)) {
          ...
          this.columns.set(
            ...
            match[2]
              ...
              .filter((column) => column.length > 0 && column.toUpperCase() !== "PRIMARY"),
          );
        }
      }
    ...
    if (/^INSERT INTO\b/i.test(statement)) {
      ...
      if (match !== null) {
        ...
        for (const group of valueGroups(match[3])) {
          const values = group
            .split(",")
            .map((value) => parseValue(value.trim(), params, () => paramIndex++));
    ...
    if (/^SELECT\b/i.test(statement)) {
      const match =
        /^SELECT\s+(.+)\s+FROM\s+("[^"]+"|\w+)(?:\s+WHERE\s+("[^"]+"|\w+)\s*=\s*(\?|('[^']*')|\d+))?(?:\s+ORDER BY\s+("[^"]+"|\w+)\s+DESC)?$/i.exec(
          statement,
        );
      if (match !== null) {
...
const makeClient = (config: SqliteClient.SqliteClientConfig) =>
  SqliteClient.make(config).pipe(Effect.scoped, Effect.provide(Reactivity.layer));

const hasForbiddenTransactionSql = (db: FakeSqlStorage) =>
  db.statements.some((statement) =>
    /^(BEGIN|COMMIT|ROLLBACK|SAVEPOINT)\b/i.test(normalizeSql(statement)),
  );
  ...
  it.effect("db-only transactions fail clearly without transaction SQL", () =>
    Effect.gen(function* () {
      ...
      const error = yield* Effect.flip(
        sql.withTransaction(sql`INSERT INTO test (name) VALUES ('hello')`),
      );
      const rows = yield* sql`SELECT * FROM test`;
  ...
  it.effect("SqliteMigrator.run works with storage-backed transactions", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(migrations, [
        { migration_id: 1, name: "create", created_at: "current_timestamp" },
      ]);
      assert.strictEqual(storage.transactionCalls, 1);
      assert.strictEqual(hasForbiddenTransactionSql(storage.sql), false);
    }),
