...
describe("Client", () => {
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const [rawRows] = yield* withoutTransforms.batch([
          withoutTransforms<{
            firstName: string;
          }>`SELECT ${withoutTransforms("firstName")} FROM test`,
        ]);
        ...
        const [rawRowsFromTransformedBatch] = yield* transformed.batch([
          withoutTransforms<{
            first_name: string;
          }>`SELECT ${withoutTransforms("first_name")} FROM test`,
        ]);
  ...
  it.effect("should apply statement transformers in a batch", () =>
    Effect.gen(function* () {
      ...
      const results = yield* sql.batch([sql`SELECT ${1} AS value`, sql`SELECT ${2} AS value`]).pipe(
  ...
  it.effect("should defect when batching in a transaction", () =>
    Effect.gen(function* () {
      ...
      yield* sql`CREATE TABLE test (id INTEGER PRIMARY KEY, name TEXT)`;
      const res = yield* sql
        .batch([sql`INSERT INTO test ${sql.insert({ name: "hello" })}`])
        .pipe(sql.withTransaction, Effect.sandbox, Effect.flip);
      const rows = yield* sql`SELECT * FROM test`;
      assert.deepStrictEqual(rows, []);
      assert.equal(Cause.hasDies(res), true);
    }).pipe(Effect.provide(D1Miniflare.layerClient)),
