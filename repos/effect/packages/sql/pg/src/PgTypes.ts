 ...
 * @since 4.0.0
 */
export const arrayOidFor = (elementOid: number): number | undefined =>
...
const parseIPv6 = (text: string): Uint8Array | undefined => {
  ...
  const trailing =
    tail.length > 0 ? tail[tail.length - 1] : head.length > 0 ? head[head.length - 1] : "";
...
const decodeInet = (bytes: Uint8Array, offset: number, size: number): string => {
  ...
  const text =
    family === PGSQL_AF_INET ? formatIPv4(bytes, offset + 4) : formatIPv6(bytes, offset + 4);
...
const parameter =
  (oid: number) =>
  (value: unknown): Parameter => ({ oid, value });
