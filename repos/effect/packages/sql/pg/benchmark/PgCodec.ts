...
const payload = [
  ...
  { oid: PgTypes.OID.float8, value: 1234.5, text: "1234.5" },
  {
    ...
    value: "codec benchmark: hello \\ world",
    text: "codec benchmark: hello \\ world",
  },
  { oid: PgTypes.OID.jsonb, value: jsonValue, text: JSON.stringify(jsonValue) },
  {
    oid: PgTypes.OID.bytea,
    value: byteaValue,
    text: `\\x${Buffer.from(byteaValue).toString("hex")}`,
  },
] as const;
...
const postgresTextParsers = payload.map(
  ({ oid }) => postgresParsers[oid] ?? ((value: string) => value),
);
...
const encodePostgresJs = () =>
  payload.map(({ oid, value }) => postgresSerializers[oid]?.(value) ?? String(value));

const decodeEffect = () =>
  payload.map(({ oid }, index) => success(PgTypes.decode(effectEncoded[index], oid, 1)));
...
const makeDataRow = (fields: ReadonlyArray<string | Uint8Array>): Uint8Array => {
  const encoded = fields.map((field) =>
    typeof field === "string" ? Buffer.from(field, "utf8") : Buffer.from(field),
  );
...
const parserBuffer = Buffer.from(
  parserPayload.buffer,
  parserPayload.byteOffset,
  parserPayload.byteLength,
);
const parserChunks = Array.from({ length: Math.ceil(parserPayload.length / 64) }, (_, index) =>
  parserPayload.subarray(index * 64, (index + 1) * 64),
);
const pgParserChunks = parserChunks.map((chunk) =>
...
const bindEffectFused = () =>
  success(encodeBindFused({ portal: "", statement: "s1", parameters: bindParameters }));
...
const bindArrayEffectFused = () =>
  success(encodeBindFused({ portal: "", statement: "s2", parameters: arrayRow }));
...
const fusedRowParser = PgProtocol.makeParser({
  readField: success(PgTypes.makeFieldReader(binaryColumns)),
});
...
const wideEncoded = Array.from({ length: wideColumnCount }, (_, index) =>
  success(PgTypes.encode(index * 7, wideOid)),
);
...
const wideFusedParser = PgProtocol.makeParser({
  readField: success(
    PgTypes.makeFieldReader(wideEncoded.map(() => ({ dataTypeOid: wideOid, format: 1 }))),
  ),
});
...
const pgArrayParser = (
  pgTypes.getTypeParser as (oid: number, format: "text") => (value: string) => unknown
)(PgTypes.OID.int4Array, "text");
...
const numericEncoded = numericValues.map((value) =>
  success(PgTypes.encode(value, PgTypes.OID.numeric)),
);
const decodeNumericEffect = () =>
  numericEncoded.map((bytes) => success(PgTypes.decode(bytes, PgTypes.OID.numeric, 1)));
...
const batch =
  (run: () => unknown, rows = codecRowsPerRun) =>
  () => {
    let value: unknown;
...
await runSuite(
  "DataRow frames to JavaScript values, wide rows",
  rowsPerParserRun,
  [
...
await runSuite(
  "protocol RowDescription parser",
  descriptionsPerRun,
  [
