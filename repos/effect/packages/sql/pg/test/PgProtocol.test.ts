...
describe("PgProtocol", () => {
  describe("special messages", () => {
    ...
    it("decodes the SSLRequest response byte", () => {
      ...
      if (Result.isFailure(invalid))
        assert.strictEqual(invalid.failure._tag, "PgProtocolParseError");
    });
  ...
  describe("frontend messages", () => {
    ...
    it("encodes strings past the ASCII fast path", () => {
      ...
      for (const query of [
        ...
        const encoded = success(
          PgProtocol.encodeParse({ name: "s1", query, parameterTypes: [23] }),
        );
    ...
    it("encodes Bind through a value sink", () => {
      ...
      assert.strictEqual(
        hex(
          success(
            encodeBind({ portal: "p1", statement: "s1", parameters: [bytes("00000001"), null] }),
          ),
        ),
        frontend.bind,
      );
    ...
    it("keeps sink-written parameters intact when the pool grows mid-message", () => {
      ...
      const encoded = success(
        encodeBind({ portal: "", statement: "", parameters: ["ab", long, "cd"] }),
      );
    ...
    it("encodes Execute", () => {
      assert.strictEqual(
        hex(PgProtocol.encodeExecute({ portal: "p1", maxRows: 5 })),
        frontend.execute,
      );
    });
    ...
    it("encodes Describe and Close", () => {
      ...
      assert.strictEqual(
        hex(PgProtocol.encodeClose({ target: "portal", name: "p1" })),
        frontend.closePortal,
      );
    });
    ...
    it("encodes the password messages", () => {
      ...
      assert.strictEqual(
        hex(PgProtocol.encodeSASLResponse({ data: bytes("010203") })),
    ...
    it("dispatches encode over the tagged union", () => {
      const cases: ReadonlyArray<readonly [PgProtocol.FrontendMessage, Uint8Array]> = [
        ...
        [
          { _tag: "Execute", portal: "p1", maxRows: 5 },
        ...
        [
          { _tag: "SASLResponse", data: bytes("010203") },
    ...
    it("accepts the maximum signed parameter count and rejects the next value", () => {
      ...
      const parse = success(
        PgProtocol.encodeParse({ name: "", query: "", parameterTypes: maxOids }),
      );
      const bind = success(
        PgProtocol.encodeBind({ portal: "", statement: "", parameters: maxParameters }),
      );
      assert.strictEqual(
        new DataView(parse.buffer, parse.byteOffset, parse.byteLength).getInt16(7),
        0x7fff,
      );
      assert.strictEqual(
        new DataView(bind.buffer, bind.byteOffset, bind.byteLength).getInt16(11),
        0x7fff,
      );

      const tooManyOids = new Array<number>(0x8000).fill(23);
      const tooManyParameters = new Array<Uint8Array | null>(0x8000).fill(null);
      const parseFailure = PgProtocol.encodeParse({
        name: "",
        query: "",
        parameterTypes: tooManyOids,
      });
      const bindFailure = PgProtocol.encodeBind({
        portal: "",
        statement: "",
        parameters: tooManyParameters,
      });
  ...
  describe("backend messages", () => {
    ...
    it("decodes ReadyForQuery", () => {
      assert.deepStrictEqual(parseOne(backend.readyForQuery), {
        _tag: "ReadyForQuery",
        status: "I",
      });
    });
  ...
  describe("incremental parsing", () => {
    ...
    it("returns both messages when one chunk holds two", () => {
      ...
      assert.deepStrictEqual(parser.push(chunk), [
        { _tag: "ParseComplete" },
        { _tag: "BindComplete" },
      ]);
    });
    ...
    it("returns every message when one chunk holds many", () => {
      ...
      assert.strictEqual(
        messages.every((message) => message._tag === "ParseComplete"),
        true,
      );
    });
    ...
    it("keeps the trailing partial message across pushes", () => {
      ...
      assert.deepStrictEqual(parser.push(chunk.subarray(8)), [
        { _tag: "ReadyForQuery", status: "I" },
      ]);
    });
    ...
    it("rejects a length prefix below the minimum", () => {
      ...
      assertThrowsTagged("PgProtocolParseError", () =>
        PgProtocol.makeParser().push(bytes("4480000000")),
      );
    });

    it("accepts a frame exactly at maxMessageSize", () => {
      assert.deepStrictEqual(
        PgProtocol.makeParser({ maxMessageSize: 8 }).push(bytes("5f00000008deadbeef")),
  ...
  describe("buffer reuse", () => {
    ...
    it("keeps DataRow field views valid as the buffer pool grows", () => {
      ...
      const retained: Array<{
        readonly row: PgProtocol.DataRow;
        readonly snapshot: Array<Uint8Array | null>;
      }> = [];
      ...
      for (let index = 0; index < 5000; index++) {
        ...
        if (index % 500 === 0) {
          retained.push({
            row,
            snapshot: row.values.map((value) => (value === null ? null : value.slice())),
          });
        }
      }
