...
const assertFailureTagged = (
  tag: string,
  run: () => Result.Result<unknown, { readonly _tag: string }>,
) => {
  const result = run();
  ...
  describe("SCRAM-SHA-256", () => {
    it("replays a captured PostgreSQL exchange", () => {
      const started = success(
        PgAuth.scramInit({ password: scram.password, nonce: scram.clientNonce }),
      );
      assert.strictEqual(decoder.decode(started.response), scram.clientFirstMessage);

      const continued = success(
        PgAuth.scramContinue(started.state, encoder.encode(scram.serverFirstMessage)),
      );
    ...
    it("rejects a server nonce that does not extend the client nonce", () => {
      ...
      assertFailureTagged("PgAuthError", () =>
        PgAuth.scramContinue(
          started.state,
          encoder.encode("r=other,s=DBRmN4Xi9iMOo1tZfsi+Hg==,i=4096"),
        ),
      );
    });
    ...
    it("rejects a tampered server signature", () => {
      ...
      assertFailureTagged("PgAuthError", () =>
        PgAuth.scramFinish(
          continued.state,
          encoder.encode("v=AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="),
        ),
      );
    });
