...
const PgTypes = {
  ...
  decode: (value: Uint8Array, oid: number, format: number) =>
    success(PgTypesResult.decode(value, oid, format)),
  encodeParameter: (parameter: PgTypesResult.Parameter) =>
    success(PgTypesResult.encodeParameter(parameter)),
  makeFieldReader: (columns: ReadonlyArray<PgTypesResult.Column>) =>
    success(PgTypesResult.makeFieldReader(columns)),
  array: (values: ReadonlyArray<unknown> | null, elementOid: number) =>
    success(PgTypesResult.array(values, elementOid)),
};
...
describe("PgTypes", () => {
  ...
  describe("arrays", () => {
    it("decodes a zero-dimension array as empty", () => {
      assert.deepStrictEqual(
        PgTypes.decode(column(rows.textArrayEmpty), PgTypes.OID.textArray, 1),
        [],
      );
    });
    ...
    it("rejects a truncated array element", () => {
      assertThrowsTagged("PgTypesCodecError", () =>
        PgTypes.decode(
          bytes("000000010000000000000017000000010000000100000004000000"),
          PgTypes.OID.int4Array,
          1,
        ),
      );
    ...
    it("rejects an array element length below the NULL sentinel", () => {
      assertThrowsTagged("PgTypesCodecError", () =>
        PgTypes.decode(
          bytes("0000000100000000000000110000000100000001fffffffe"),
          PgTypes.OID.byteaArray,
          1,
        ),
      );
    });
    ...
    it("rejects an array whose lower bound is not one", () => {
      assertThrowsTagged("PgTypesCodecError", () =>
        PgTypes.decode(
          bytes(
            "0000000100000000000000170000000300000000000000040000000100000004000000020000000400000003",
          ),
          PgTypes.OID.int4Array,
          1,
        ),
  ...
  describe("errors", () => {
    it("rejects the text format", () => {
      assertThrowsTagged("PgTypesCodecError", () =>
        PgTypes.decode(bytes("31"), PgTypes.OID.int4, 0),
      );
    });
    ...
    it("rejects NaN timestamps", () => {
      assertThrowsTagged("PgTypesCodecError", () =>
        PgTypes.encode(Number.NaN, PgTypes.OID.timestamptz),
      );
    });
    ...
    it("rejects timestamps outside the PostgreSQL int64 range", () => {
      ...
      assertThrowsTagged("PgTypesCodecError", () =>
        PgTypes.encode(Number.MAX_VALUE, PgTypes.OID.timestamp),
      );
    });
    ...
    it("rejects host bits outside a cidr netmask", () => {
      ...
      assertThrowsTagged("PgTypesCodecError", () =>
        PgTypes.encode("2001:db8::1/32", PgTypes.OID.cidr),
      );
    });

    it("rejects invalid network masks and CIDR host bits on decode", () => {
      for (const [wire, oid] of [
    ...
    it("rejects a value the wrong size for its OID", () => {
      assertThrowsTagged("PgTypesCodecError", () =>
        PgTypes.decode(bytes("0001"), PgTypes.OID.int4, 1),
      );
    });
    ...
    it("rejects unsupported JSONB versions and unserialisable JSON values", () => {
      for (const wire of ["", "007b7d", "027b7d"]) {
        assertThrowsTagged("PgTypesCodecError", () =>
          PgTypes.decode(bytes(wire), PgTypes.OID.jsonb, 1),
        );
      }
    ...
    it("rejects malformed text, numeric, time, and network values", () => {
      assertThrowsTagged("PgTypesCodecError", () =>
        PgTypes.decode(bytes("ff"), PgTypes.OID.text, 1),
      );
      ...
      for (const wire of [
        ...
        assertThrowsTagged("PgTypesCodecError", () =>
          PgTypes.decode(bytes(wire), PgTypes.OID.inet, 1),
        );
      }
    });
  });
  ...
  describe("scalar boundaries", () => {
    it("round trips integer and time limits and rejects the adjacent values", () => {
      const cases: ReadonlyArray<{
        ...
        {
          oid: PgTypes.OID.int4,
          accepted: [-2147483648, 2147483647],
          rejected: [-2147483649, 2147483648],
        },
    ...
    it("round trips scalar boundary values inside arrays", () => {
      for (const [oid, value] of [
        ...
        [
          PgTypes.OID.int8Array,
          [BigInt("-9223372036854775808"), null, BigInt("9223372036854775807")],
        ],
    ...
    it("round trips deterministic generated integer values", () => {
      ...
      for (let index = 0; index < 512; index++) {
        ...
        values.push(state);
        assert.strictEqual(
          PgTypes.decode(PgTypes.encode(state, PgTypes.OID.int4), PgTypes.OID.int4, 1),
          state,
        );
      }
      assert.deepStrictEqual(
  ...
  describe("timestamps", () => {
    ...
    it("round trips milliseconds through both the float and the BigInt path", () => {
      for (const ms of [
        0, 1, -1, 946684800000, 1717171717123, -62135596800000, 253402300799000, 9007199254740,
      ]) {
        assert.strictEqual(
          PgTypes.decode(PgTypes.encode(ms, PgTypes.OID.timestamp), PgTypes.OID.timestamp, 1),
          ms,
        );
      }
    });
  ...
  describe("dates", () => {
    it("round trips years of every width, negative ones included", () => {
      for (const value of [
        ...
        assert.strictEqual(
          PgTypes.decode(PgTypes.encode(value, PgTypes.OID.date), PgTypes.OID.date, 1),
          value,
        );
      }
    });
  ...
  describe("timetz", () => {
    it("round trips fractional seconds and zone offsets", () => {
      for (const value of [
        ...
        assert.strictEqual(
          PgTypes.decode(PgTypes.encode(value, PgTypes.OID.timetz), PgTypes.OID.timetz, 1),
          value,
        );
      }
      assert.strictEqual(
    ...
    it("enforces PostgreSQL's wire time zone displacement range", () => {
      assert.strictEqual(
        PgTypes.decode(bytes("00000000000000000000e0ff"), PgTypes.OID.timetz, 1),
        "00:00:00-15:59",
      );
      assert.strictEqual(
        PgTypes.decode(bytes("0000000000000000ffff1f01"), PgTypes.OID.timetz, 1),
        "00:00:00+15:59",
      );
      assertThrowsTagged("PgTypesCodecError", () =>
    ...
    it("rejects times past midnight at both ends", () => {
      assertThrowsTagged("PgTypesCodecError", () =>
        PgTypes.encode("24:00:01+00:00", PgTypes.OID.timetz),
      );
      assertThrowsTagged("PgTypesCodecError", () =>
        PgTypes.encode("25:00:00+00:00", PgTypes.OID.timetz),
      );
  ...
  describe("parameters", () => {
    ...
    it("encodes strings past the ASCII fast path", () => {
      ...
      for (const value of ["abc", "é", "x".repeat(200), "héllo ☃ 👋🏽", "y".repeat(70_000)]) {
        ...
        assert.strictEqual(
          PgTypes.decode(PgTypes.encode(value, PgTypes.OID.text), PgTypes.OID.text, 1),
          value,
        );
    ...
    it("rejects a stray high byte at every position of the fast path", () => {
      ...
      for (let length = 1; length <= 16; length++) {
        for (let at = 0; at < length; at++) {
          ...
          bytes[at] = 0xc3;
          assert.throws(
            () => PgTypes.decode(bytes, PgTypes.OID.text, 1),
    ...
    it("recovers from a parameter that fails mid-frame", () => {
      ...
      assertThrowsTagged("PgTypesCodecError", () =>
        encodeBind({
          portal: "",
          statement: "s",
          parameters: [PgTypes.int4(1), PgTypes.uuid("not-a-uuid")],
        }),
      );
    ...
    it("keeps a bytea frame intact when the source is mutated afterwards", () => {
      ...
      const encoded = success(
        encodeBind({ portal: "", statement: "", parameters: [PgTypes.bytea(source)] }),
      );
      source[0] = 0xff;
  ...
  describe("makeFieldReader", () => {
    it("reads every builtin column as decode does", () => {
      ...
      assert.deepStrictEqual(
        row.values,
        fields.map((field, index) => PgTypes.decode(field, columns[index], 1)),
      );
    });
    ...
    it("hands a view to a registered codec that cannot read in place", () => {
      ...
      try {
        const parser = PgProtocol.makeParser({
          readField: PgTypes.makeFieldReader(binary([PgTypes.OID.int4, oid])),
        });
        const messages = parser.push(
          dataRow([PgTypes.encode(1, PgTypes.OID.int4), new Uint8Array([9, 8, 7])]),
        );
    ...
    it("keeps public codec failures typed and makes parser failures terminal", () => {
      ...
      try {
        ...
        assert.isTrue(threw);
        assertThrowsTagged("PgProtocolParseError", () =>
          parser.push(dataRow([new Uint8Array([1])])),
        );
      } finally {
        PgTypes.unregister(oid);
    ...
    it("reads a row split across chunks", () => {
      ...
      const values = [
        7,
        "a longer value than the fast path covers",
