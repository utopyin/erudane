...
Alchemy has no "bind Hyperdrive by id" form: `Cloudflare.Hyperdrive.Connection.read` looks up by **name** and silently adopts a match (`Cloudflare/Hyperdrive/Connection.ts:222-262`), then `reconcile` re-puts the origin (the API never returns the password, so we must supply it). Given id `7986575ea58343eaa80f040ef5c83f27`, phase 0 runs `bunx wrangler hyperdrive get 7986575ea58343eaa80f040ef5c83f27` to read the config's **name**, host, port, database and user; those plus the role password go to `.env` (`HYPERDRIVE_NAME`, `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, `DB_PASSWORD`). After the first deploy alchemy's state holds `hyperdriveId = 7986575e…`.
