...
```ts
export const layer = HttpRouter.add("POST", "/chat", Effect.gen(function* () {
...
Error policy: decode failures → 400 (`HttpServerResponse` via `HttpRouter` default schema-error handling or explicit `catchTag`). `ChatError` *inside* the stream cannot change the status anymore; it becomes a trailing `RUN_ERROR` event (`Stream.catchTag` in `Agui.encode`).
...
```ts
const WireContent = Schema.Union([Schema.String, Schema.Array(Schema.Struct({ type: Schema.Literal("text"), text: Schema.String }))])
...
const WireMessage = Schema.Union([
  ...
  Schema.Struct({ id: Schema.String, role: Schema.Literal("assistant"), content: Schema.optional(Schema.String),
    toolCalls: Schema.optional(Schema.Array(Schema.Struct({
      id: Schema.String, type: Schema.Literal("function"),
      function: Schema.Struct({ name: Schema.String, arguments: Schema.String }) }))) }),
  Schema.Struct({ id: Schema.String, role: Schema.Literal("tool"), toolCallId: Schema.String, content: Schema.String, error: Schema.optional(Schema.String) }),
])
