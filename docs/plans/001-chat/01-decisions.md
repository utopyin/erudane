...
`apps/web` initialises shadcn with `bunx shadcn@latest init --preset b7ClRmgEa`; components live in `apps/web/src/components/ui`. Icons are **not** the shadcn default (lucide): they come from the locally installed Nucleo React package, **glyph (filled) fills only — never outline** (`~/.nucleo/skills`, families core/ui/micro/sharp/pixel), copied as components into `apps/web/src/components/icons/` and customised per the `nucleo-customize` skill (`currentColor`, `strokeWidth` on core/ui outline, `data-color="color-2"` for accents). Chat UI uses the `ui` family, glyph, 18px.

## D11 — Dev-only inference through a ChatGPT subscription

`bun run login:chatgpt` (`scripts/chatgpt/login.ts`) runs the Codex "Sign in with ChatGPT" PKCE flow (client `app_EMoamEEZ73f0CkXaXp7hrann`, callback `localhost:1455`) and stores `{ access, refresh, expires, accountId }` in `.auth/chatgpt.json` (gitignored, mode 600) — the same shape pi uses. Under `alchemy dev`, `alchemy.run.ts` loads the file, refreshes the pair when within 5 minutes of expiry, and injects `{ access, accountId }` as the `CHATGPT_OAUTH` secret; `apps/api/src/model.ts` then points the OpenAI client at `https://chatgpt.com/backend-api/codex` with the bearer + `chatgpt-account-id` headers and `store: false` / `include: ["reasoning.encrypted_content"]`. Deploys never read the file (`ALCHEMY_DEV` gate), so production stays on the API key. This is tolerated by OpenAI for personal developer use today, not guaranteed; it is never a production path.
